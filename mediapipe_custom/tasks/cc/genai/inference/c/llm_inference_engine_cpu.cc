...
#include "mediapipe_custom/tasks/cc/genai/inference/c/llm_inference_engine.h"
#include "mediapipe_custom/tasks/cc/genai/inference/proto/llm_params.pb.h"
#include "mediapipe_custom/tasks/cc/genai/inference/proto/transformer_params.pb.h"
#include "mediapipe_custom/tasks/cc/genai/inference/utils/llm_utils/metadata_utils.h"
#include "mediapipe_custom/tasks/cc/genai/inference/utils/llm_utils/model_data.h"
#include "mediapipe_custom/tasks/cc/genai/inference/utils/xnn_utils/graph_builder.h"
#include "mediapipe_custom/tasks/cc/genai/inference/utils/xnn_utils/llm.h"
#include "mediapipe_custom/tasks/cc/genai/inference/utils/xnn_utils/llm_builder_factory.h"
#include "mediapipe_custom/tasks/cc/genai/inference/utils/xnn_utils/llm_weights.h"
// clang-format off
#include "mediapipe_custom/tasks/cc/genai/inference/utils/llm_utils/scoped_file.h"
// clang-format on
...
using ::mediapipe_custom::tasks::genai::llm_utils::ScopedFile;

constexpr int kCheckLastKChars = 10;

struct TfLiteLlm {
  std::unique_ptr<tflite::Interpreter> interpreter;
  std::unique_ptr<mediapipe_custom::tasks::core::ModelAssetBundleResources> resources;
};
...
struct LlmInferenceEngineCpu_Engine {
  ...
  const std::variant<mediapipe_custom::tasks::genai::xnn_utils::Llm*, TfLiteLlm*> llm;
  ...
  ~LlmInferenceEngineCpu_Engine() {
    ...
    if (std::holds_alternative<mediapipe_custom::tasks::genai::xnn_utils::Llm*>(llm)) {
      delete std::get<mediapipe_custom::tasks::genai::xnn_utils::Llm*>(llm);
    } else {
...
void* next_token_function(void* args) {
  ...
  if (cpu_session->timestep < cpu_session->engine->max_num_tokens) {
    ...
    if (std::holds_alternative<mediapipe_custom::tasks::genai::xnn_utils::Llm*>(
            cpu_session->engine->llm)) {
      auto status = std::get<mediapipe_custom::tasks::genai::xnn_utils::Llm*>(
                        cpu_session->engine->llm)
...
void* start_llm_function(void* args) {
  ...
  if (std::holds_alternative<mediapipe_custom::tasks::genai::xnn_utils::Llm*>(
          cpu_session->engine->llm)) {
    auto llm = std::get<mediapipe_custom::tasks::genai::xnn_utils::Llm*>(
        cpu_session->engine->llm);
...
CreateXnnLlmCpuEngine(const LlmModelSettings* model_settings) {
  ...
  MP_ASSIGN_OR_RETURN(auto model_data,
                      mediapipe_custom::tasks::genai::llm_utils::ModelData::Create(
                          std::move(model_file)));
  ...
  auto llm_params =
      mediapipe_custom::tasks::genai::xnn_utils::LlmParams::FromLLMParametersProto(
          llm_params_proto);
  ...
  MP_ASSIGN_OR_RETURN(auto backend,
                      model_data->ReadMetadata(
                          mediapipe_custom::tasks::genai::llm_utils::kLlmBackendName));
  RET_CHECK_EQ(backend, "cpu");

  // Create directory for tokenizer and model cache file.
  if (model_settings->cache_dir != nullptr) {
    auto s = mediapipe_custom::file::RecursivelyCreateDir(model_settings->cache_dir);
    if (!s.ok()) {
      ABSL_LOG(WARNING) << s;
    }
  }
  ...
  auto weight_loader = std::make_unique<
      mediapipe_custom::tasks::genai::xnn_utils::DefaultLlmWeightsLoader>(
      model_settings->model_path, llm_params);

  auto runtime_configs =
      std::make_unique<mediapipe_custom::tasks::genai::xnn_utils::RuntimeConfigs>();

  MP_ASSIGN_OR_RETURN(auto llm,
                      mediapipe_custom::tasks::genai::xnn_utils::CreateLlm(
...
CreateTfliteLlmCpuEngine(const LlmModelSettings* model_settings) {
  auto external_file =
      std::make_unique<mediapipe_custom::tasks::core::proto::ExternalFile>();
  if (model_settings) {
    external_file->set_file_name(model_settings->model_path);
  }
  MP_ASSIGN_OR_RETURN(auto resources,
                      mediapipe_custom::tasks::core::ModelAssetBundleResources::Create(
  ...
  if (weight_cache_path != ":nocache") {
    if (weight_cache_path.empty()) {
      ...
      weight_cache_path = mediapipe_custom::file::JoinPath(
          weight_cache_path,
          absl::StrCat(mediapipe_custom::file::Basename(model_settings->model_path),
                       ".xnnpack_cache"));
    }
