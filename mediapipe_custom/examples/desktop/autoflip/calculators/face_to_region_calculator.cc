...
#include "mediapipe_custom/examples/desktop/autoflip/calculators/face_to_region_calculator.pb.h"
#include "mediapipe_custom/examples/desktop/autoflip/quality/visual_scorer.h"
...
absl::Status FaceToRegionCalculator::GetContract(
  ...
  cc->Inputs().Tag(kFacesTag).Set<std::vector<mediapipe_custom::Detection>>();
  cc->Outputs().Tag(kRegionsTag).Set<DetectionSet>();
  return absl::OkStatus();
}

absl::Status FaceToRegionCalculator::Open(mediapipe_custom::CalculatorContext* cc) {
...
absl::Status FaceToRegionCalculator::Process(mediapipe_custom::CalculatorContext* cc) {
  if (cc->Inputs().HasTag(kVideoTag) &&
  ...
  cv::Mat frame;
  if (cc->Inputs().HasTag(kVideoTag)) {
    frame = mediapipe_custom::formats::MatView(
  ...
  if (!cc->Inputs().Tag(kFacesTag).Value().IsEmpty()) {
    const auto& input_faces =
        cc->Inputs().Tag(kFacesTag).Get<std::vector<mediapipe_custom::Detection>>();
