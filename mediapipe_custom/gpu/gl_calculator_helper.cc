...
GlTexture GlCalculatorHelper::CreateSourceTexture(
    const mediapipe_custom::Image& image) {
...
template <>
std::unique_ptr<mediapipe_custom::Image> GlTexture::GetFrame<mediapipe_custom::Image>()
    const {
  std::unique_ptr<GpuBuffer> buf = GetFrame<GpuBuffer>();
  auto output = absl::make_unique<mediapipe_custom::Image>(*buf);
  return output;
}
