...
using mediapipe_custom::android::JavaListToStdStringVector;
using mediapipe_custom::android::JStringToStdString;
using mediapipe_custom::android::ThrowIfError;

namespace {
absl::Status AddSidePacketsIntoGraph(mediapipe_custom::android::Graph* mediapipe_graph,
  ...
  for (jsize i = 0; i < num_side_packets; ++i) {
    ...
    mediapipe_graph->SetInputSidePacket(
        ...
        mediapipe_custom::android::Graph::GetPacketFromHandle(packets_array_ref[i]));
...
absl::Status AddStreamHeadersIntoGraph(
    mediapipe_custom::android::Graph* mediapipe_graph, JNIEnv* env,
...
JNIEXPORT jlong JNICALL GRAPH_METHOD(nativeCreateGraph)(JNIEnv* env,
                                                        jobject thiz) {
  if (!mediapipe_custom::java::SetJavaVM(env)) {
    return 0;
  }
  return reinterpret_cast<int64_t>(new mediapipe_custom::android::Graph());
}

JNIEXPORT void JNICALL GRAPH_METHOD(nativeReleaseGraph)(JNIEnv* env,
                                                        jobject thiz,
                                                        jlong context) {
  delete reinterpret_cast<mediapipe_custom::android::Graph*>(context);
}
...
JNIEXPORT void JNICALL GRAPH_METHOD(nativeAddPacketToInputStream)(
  ...
  ThrowIfError(
      env, mediapipe_graph->AddPacketToInputStream(
               ...
               mediapipe_custom::android::Graph::GetPacketFromHandle(packet).At(
                   mediapipe_custom::Timestamp::CreateNoErrorChecking(timestamp))));
}
...
JNIEXPORT void JNICALL GRAPH_METHOD(nativeSetGraphInputStreamBlockingMode)(
  ...
  if (mode) {
    mediapipe_graph->SetGraphInputStreamAddMode(
        mediapipe_custom::CalculatorGraph::GraphInputStreamAddMode::
            WAIT_TILL_NOT_FULL);
  } else {
    mediapipe_graph->SetGraphInputStreamAddMode(
        mediapipe_custom::CalculatorGraph::GraphInputStreamAddMode::ADD_IF_NOT_FULL);
  }
}
...
JNIEXPORT void JNICALL GRAPH_METHOD(nativeUpdatePacketReference)(
  ...
  auto reference =
      mediapipe_custom::android::Graph::GetPacketFromHandle(reference_packet)
          .Get<std::unique_ptr<mediapipe_custom::SyncedPacket>>()
          .get();
  auto new_value = mediapipe_custom::android::Graph::GetPacketFromHandle(new_packet);
