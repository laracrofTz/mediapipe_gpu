...
#include "mediapipe_custom/framework/api3/internal/contract_fields.h"
#include "mediapipe_custom/framework/api3/internal/contract_to_tuple.h"
#include "mediapipe_custom/framework/api3/internal/port_base.h"
#include "mediapipe_custom/framework/api3/internal/specializers.h"
...
class CalculatorContext : public NodeT::template Contract<ContextSpecializer> {
 public:
  explicit CalculatorContext(mediapipe_custom::CalculatorContext& generic_context) {
  ...
  mediapipe_custom::CalculatorContext& GetGenericContext() {
    return *holder_->context;
  }

 private:
  void Reset(mediapipe_custom::CalculatorContext& generic_context) {
...
class Input<ContextSpecializer, PayloadT>
 ...
 public:
  ...
  mediapipe_custom::api3::Packet<PayloadT> Packet() const {
    return mediapipe_custom::api3::Packet<PayloadT>(
 ...
 public:
  ...
  void Send(const PayloadT& payload) const {
    holder_->context->Outputs()
        .Get(Tag(), Index())
        .AddPacket(mediapipe_custom::MakePacket<PayloadT>(payload).At(
  ...
  void Send(PayloadT&& payload) const {
    holder_->context->Outputs()
        .Get(Tag(), Index())
        .AddPacket(
            mediapipe_custom::MakePacket<PayloadT>(std::forward<PayloadT>(payload))
  ...
  void Send(std::unique_ptr<PayloadT> payload) const {
    holder_->context->Outputs()
        .Get(Tag(), Index())
        .AddPacket(mediapipe_custom::Adopt(payload.release())
                       .At(holder_->context->InputTimestamp()));
  }
 ...
 public:
  ...
  void Set(const PayloadT& payload) const {
    holder_->context->OutputSidePackets()
        .Get(Tag(), Index())
        .Set(mediapipe_custom::MakePacket<PayloadT>(payload));
  }

  void Set(PayloadT&& payload) const {
    holder_->context->OutputSidePackets()
        .Get(Tag(), Index())
        .Set(mediapipe_custom::MakePacket<PayloadT>(std::forward<PayloadT>(payload)));
  }
