...
#include "mediapipe_custom/tasks/c/text/language_detector/language_detector.h"
...
#include "mediapipe_custom/tasks/c/text/language_detector/language_detector_result_converter.h"
#include "mediapipe_custom/tasks/cc/text/language_detector/language_detector.h"

namespace mediapipe_custom::tasks::c::text::language_detector {
...
using ::mediapipe_custom::tasks::text::language_detector::LanguageDetector;
...
LanguageDetector* CppLanguageDetectorCreate(
  ...
  auto cpp_options = std::make_unique<
      ::mediapipe_custom::tasks::text::language_detector::LanguageDetectorOptions>();
...
void* language_detector_create(struct LanguageDetectorOptions* options,
                               char** error_msg) {
  return mediapipe_custom::tasks::c::text::language_detector::
...
void language_detector_close_result(LanguageDetectorResult* result) {
  mediapipe_custom::tasks::c::text::language_detector::CppLanguageDetectorCloseResult(
      result);
}

int language_detector_close(void* detector, char** error_ms) {
  return mediapipe_custom::tasks::c::text::language_detector::CppLanguageDetectorClose(
      detector, error_ms);
}
