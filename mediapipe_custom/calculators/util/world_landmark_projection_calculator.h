...
struct WorldLandmarkProjectionNode : Node<kWorldLandmarkProjectionNodeName> {
  ...
  struct Contract {
    ...
    Input<S, mediapipe_custom::LandmarkList> input_landmarks{"LANDMARKS"};

    // An NormalizedRect representing a normalized rectangle in image
    // coordinates.
    Optional<Input<S, mediapipe_custom::NormalizedRect>> input_rect{"NORM_RECT"};
    ...
    Output<S, mediapipe_custom::LandmarkList> output_landmarks{"LANDMARKS"};
  };
};
