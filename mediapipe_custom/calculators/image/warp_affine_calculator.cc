...
#include "mediapipe_custom/calculators/image/warp_affine_calculator.h"
...
#include "mediapipe_custom/calculators/image/affine_transformation.h"
#include "mediapipe_custom/framework/api3/calculator.h"
#include "mediapipe_custom/framework/api3/calculator_context.h"
#include "mediapipe_custom/framework/api3/calculator_contract.h"
#if !MEDIAPIPE_DISABLE_GPU
#include "mediapipe_custom/calculators/image/affine_transformation_runner_gl.h"
#endif  // !MEDIAPIPE_DISABLE_GPU
#include "absl/status/status.h"
#include "absl/status/statusor.h"
#if !MEDIAPIPE_DISABLE_OPENCV
#include "mediapipe_custom/calculators/image/affine_transformation_runner_opencv.h"
...
AffineTransformation::BorderMode GetBorderMode(
    mediapipe_custom::WarpAffineCalculatorOptions::BorderMode border_mode) {
  switch (border_mode) {
    case mediapipe_custom::WarpAffineCalculatorOptions::BORDER_ZERO:
      return AffineTransformation::BorderMode::kZero;
    case mediapipe_custom::WarpAffineCalculatorOptions::BORDER_UNSPECIFIED:
    case mediapipe_custom::WarpAffineCalculatorOptions::BORDER_REPLICATE:
      return AffineTransformation::BorderMode::kReplicate;
  }
}

AffineTransformation::Interpolation GetInterpolation(
    mediapipe_custom::WarpAffineCalculatorOptions::Interpolation interpolation) {
  switch (interpolation) {
    case mediapipe_custom::WarpAffineCalculatorOptions::INTER_UNSPECIFIED:
    case mediapipe_custom::WarpAffineCalculatorOptions::INTER_LINEAR:
      return AffineTransformation::Interpolation::kLinear;
    case mediapipe_custom::WarpAffineCalculatorOptions::INTER_CUBIC:
...
class WarpAffineRunnerHolder<ImageFrame> {
 public:
  ...
  absl::Status Open(mediapipe_custom::CalculatorContext& cc,
                    const mediapipe_custom::WarpAffineCalculatorOptions& options) {
...
template <>
class WarpAffineRunnerHolder<mediapipe_custom::GpuBuffer> {
 public:
  using RunnerType =
      AffineTransformation::Runner<mediapipe_custom::GpuBuffer,
                                   std::unique_ptr<mediapipe_custom::GpuBuffer>>;

  absl::Status Open(mediapipe_custom::CalculatorContext& cc,
                    const mediapipe_custom::WarpAffineCalculatorOptions& options) {
    gpu_origin_ = options.gpu_origin();
    gl_helper_ = std::make_shared<mediapipe_custom::GlCalculatorHelper>();
    interpolation_ = GetInterpolation(options.interpolation());
 ...
 private:
  mediapipe_custom::GpuOrigin::Mode gpu_origin_;
  std::shared_ptr<mediapipe_custom::GlCalculatorHelper> gl_helper_;
  std::unique_ptr<RunnerType> runner_;
  AffineTransformation::Interpolation interpolation_;
};
...
template <>
class WarpAffineRunnerHolder<mediapipe_custom::Image> {
 public:
  using RunnerType =
      AffineTransformation::Runner<mediapipe_custom::Image, mediapipe_custom::Image>;
 ...
 private:
  class Runner : public AffineTransformation::Runner<mediapipe_custom::Image,
                                                     mediapipe_custom::Image> {
   public:
    absl::Status Open(mediapipe_custom::CalculatorContext& cc,
                      const mediapipe_custom::WarpAffineCalculatorOptions& options) {
#if !MEDIAPIPE_DISABLE_OPENCV
    ...
    absl::StatusOr<mediapipe_custom::Image> Run(
        const mediapipe_custom::Image& input, const std::array<float, 16>& matrix,
      ...
      if (input.UsesGpu()) {
        ...
        return mediapipe_custom::Image(*result);
#else
      ...
      return mediapipe_custom::Image(std::make_shared<ImageFrame>(std::move(result)));
#else
...
#endif  // !MEDIAPIPE_DISABLE_OPENCV
#if !MEDIAPIPE_DISABLE_GPU
    WarpAffineRunnerHolder<mediapipe_custom::GpuBuffer> gpu_holder_;
 ...
 public:
  ...
  static absl::Status UpdateContract(
    ...
    if constexpr (std::is_same_v<ImageT, GpuBuffer> ||
      ...
      MP_RETURN_IF_ERROR(mediapipe_custom::GlCalculatorHelper::UpdateContract(
