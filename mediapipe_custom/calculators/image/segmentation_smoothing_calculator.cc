...
#include "mediapipe_custom/calculators/image/segmentation_smoothing_calculator.pb.h"
...
absl::Status SegmentationSmoothingCalculator::Open(CalculatorContext* cc) {
  ...
  auto options =
      cc->Options<mediapipe_custom::SegmentationSmoothingCalculatorOptions>();
...
absl::Status SegmentationSmoothingCalculator::RenderCpu(CalculatorContext* cc) {
  ...
  auto current_mat = mediapipe_custom::formats::MatView(&current_frame);
  ...
  auto previous_mat = mediapipe_custom::formats::MatView(&previous_frame);
  ...
  cv::Mat output_mat = mediapipe_custom::formats::MatView(output_frame.get());
...
absl::Status SegmentationSmoothingCalculator::RenderGpu(CalculatorContext* cc) {
  ...
  const auto& current_frame = cc->Inputs().Tag(kCurrentMaskTag).Get<Image>();
  RET_CHECK(
      (current_frame.format() == mediapipe_custom::GpuBufferFormat::kBGRA32 ||
       current_frame.format() == mediapipe_custom::GpuBufferFormat::kGrayHalf16 ||
       current_frame.format() == mediapipe_custom::GpuBufferFormat::kGrayFloat32 ||
       current_frame.format() == mediapipe_custom::GpuBufferFormat::kRGB24))
...
absl::Status SegmentationSmoothingCalculator::GlSetup(CalculatorContext* cc) {
  ...
  const std::string frag_src =
      absl::StrCat(std::string(mediapipe_custom::kMediaPipeFragmentShaderPreamble),
                   R"(
