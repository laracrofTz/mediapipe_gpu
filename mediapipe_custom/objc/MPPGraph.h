...
@optional
- (void)mediapipeGraph:(MPPGraph*)graph
       didOutputPacket:(const mediapipe_custom::Packet&)packet
            fromStream:(const std::string&)streamName;
...
/// should fail or wait.
@property mediapipe_custom::CalculatorGraph::GraphInputStreamAddMode packetAddMode;

- (instancetype)init NS_UNAVAILABLE;

/// Copies the config and initializes the graph.
/// @param config The configuration describing the graph.
- (instancetype)initWithGraphConfig:(const mediapipe_custom::CalculatorGraphConfig&)config
...
- (void)setHeaderPacket:(const mediapipe_custom::Packet&)packet forStream:(const std::string&)streamName;
...
- (void)setSidePacket:(const mediapipe_custom::Packet&)packet named:(const std::string&)name;
...
- (void)addSidePackets:(const std::map<std::string, mediapipe_custom::Packet>&)extraSidePackets;
...
- (mediapipe_custom::Packet)packetWithPixelBuffer:(CVPixelBufferRef)pixelBuffer
