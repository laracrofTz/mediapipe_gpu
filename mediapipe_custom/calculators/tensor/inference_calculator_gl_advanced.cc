...
#include "mediapipe_custom/calculators/tensor/inference_calculator.h"
#include "mediapipe_custom/calculators/tensor/inference_io_mapper.h"
#include "mediapipe_custom/calculators/tensor/inference_on_disk_cache_helper.h"
#include "mediapipe_custom/calculators/tensor/inference_runner.h"
#include "mediapipe_custom/calculators/tensor/tensor_span.h"
  ...
  class GpuInferenceRunner : public InferenceRunner {
   ...
   private:
    absl::Status InitTFLiteGPURunner(
        ...
        const mediapipe_custom::InferenceCalculatorOptions::Delegate& delegate);
...
absl::Status InferenceCalculatorGlAdvancedImpl::GpuInferenceRunner::Init(
  ...
  const auto& options = cc->Options<mediapipe_custom::InferenceCalculatorOptions>();

  mediapipe_custom::InferenceCalculatorOptions::Delegate delegate = options.delegate();
  if (!kDelegate(cc).IsEmpty()) {
    const mediapipe_custom::InferenceCalculatorOptions::Delegate&
        input_side_packet_delegate = kDelegate(cc).Get();
    RET_CHECK(
        input_side_packet_delegate.has_gpu() ||
        input_side_packet_delegate.delegate_case() ==
            mediapipe_custom::InferenceCalculatorOptions::Delegate::DELEGATE_NOT_SET)
...
InferenceCalculatorGlAdvancedImpl::GpuInferenceRunner::InitTFLiteGPURunner(
  ...
  switch (delegate.gpu().usage()) {
    case mediapipe_custom::InferenceCalculatorOptions::Delegate::Gpu::
        FAST_SINGLE_ANSWER: {
    ...
    case mediapipe_custom::InferenceCalculatorOptions::Delegate::Gpu::UNSPECIFIED: {
  ...
  switch (delegate.gpu().api()) {
    case mediapipe_custom::InferenceCalculatorOptions::Delegate::Gpu::ANY: {
      // Do not need to force any specific API.
      break;
    }
    case mediapipe_custom::InferenceCalculatorOptions::Delegate::Gpu::OPENGL: {
      tflite_gpu_runner_->ForceOpenGL();
      break;
    }
    case mediapipe_custom::InferenceCalculatorOptions::Delegate::Gpu::OPENCL: {
