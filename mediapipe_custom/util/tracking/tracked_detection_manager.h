...
#include "mediapipe_custom/util/tracking/tracked_detection_manager_config.pb.h"
...
class TrackedDetectionManager {
 public:
  ...
  std::vector<int> UpdateDetectionLocation(
      int id, const ::mediapipe_custom::NormalizedRect& bounding_box,
      int64_t timestamp);
  ...
  void SetConfig(const mediapipe_custom::TrackedDetectionManagerConfig& config) {
    config_ = config;
  }
 ...
 private:
  ...
  mediapipe_custom::TrackedDetectionManagerConfig config_;
};
