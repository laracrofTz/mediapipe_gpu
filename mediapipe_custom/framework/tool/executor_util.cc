...
void EnsureMinimumDefaultExecutorStackSize(const int32_t min_stack_size,
  ...
  mediapipe_custom::ExecutorConfig* default_executor_config = nullptr;
  for (mediapipe_custom::ExecutorConfig& executor_config :
  ...
  if (!default_executor_config) {
    ...
    if (config->num_threads()) {
      default_executor_config->mutable_options()
          ->MutableExtension(mediapipe_custom::ThreadPoolExecutorOptions::ext)
  ...
  if (default_executor_config->type().empty() ||
    ...
    mediapipe_custom::ThreadPoolExecutorOptions* extension =
        default_executor_config->mutable_options()->MutableExtension(
            mediapipe_custom::ThreadPoolExecutorOptions::ext);
