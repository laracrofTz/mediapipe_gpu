...
#include "mediapipe_custom/calculators/video/tracked_detection_manager_calculator.pb.h"
...
#include "mediapipe_custom/util/tracking/tracked_detection.h"
#include "mediapipe_custom/util/tracking/tracked_detection_manager.h"
...
int64_t GetInputTimestampMs(::mediapipe_custom::CalculatorContext* cc) {
...
absl::Status TrackedDetectionManagerCalculator::Open(CalculatorContext* cc) {
  mediapipe_custom::TrackedDetectionManagerCalculatorOptions options =
      cc->Options<mediapipe_custom::TrackedDetectionManagerCalculatorOptions>();
    ...
    if (!removed_detection_ids->empty() &&
      ...
      for (int box_id : *removed_detection_ids) {
        ...
        cc->Outputs()
            ...
            .AddPacket(mediapipe_custom::MakePacket<int>(box_id).At(timestamp++));
      }
    }
