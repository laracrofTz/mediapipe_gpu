...
#include "mediapipe_custom/calculators/tensor/feedback_tensors_calculator.pb.h"
...
class FeedbackTensorsCalculator : public Node {
 public:
  ...
  absl::Status Open(CalculatorContext* cc) override {
    ...
    const auto& options =
        cc->Options<mediapipe_custom::FeedbackTensorsCalculatorOptions>();
  ...
  absl::Status Process(CalculatorContext* cc) override {
    if (feedback_tensors_location_ ==
        mediapipe_custom::FeedbackTensorsCalculatorOptions::NONE) {
    ...
    switch (feedback_tensors_location_) {
      case mediapipe_custom::FeedbackTensorsCalculatorOptions::PREPENDED:
        MP_RETURN_IF_ERROR(AddFeedbackTensors(cc, outputs));
        MP_RETURN_IF_ERROR(AddInputTensors(cc, outputs));
        break;
      case mediapipe_custom::FeedbackTensorsCalculatorOptions::APPENDED:
        MP_RETURN_IF_ERROR(AddInputTensors(cc, outputs));
        MP_RETURN_IF_ERROR(AddFeedbackTensors(cc, outputs));
        break;
      default:
 ...
 private:
  ...
  mediapipe_custom::FeedbackTensorsCalculatorOptions::FeedbackTensorsLocation
