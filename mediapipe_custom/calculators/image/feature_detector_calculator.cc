...
#include "mediapipe_custom/calculators/image/feature_detector_calculator.pb.h"
...
class FeatureDetectorCalculator : public CalculatorBase {
 ...
 private:
  ...
  std::unique_ptr<mediapipe_custom::ThreadPool> pool_;
...
absl::Status FeatureDetectorCalculator::Open(CalculatorContext* cc) {
  ...
  pool_ = absl::make_unique<mediapipe_custom::ThreadPool>("ThreadPool", kNumThreads);
  pool_->StartWorkers();
  return absl::OkStatus();
}
