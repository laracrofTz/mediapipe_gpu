...
#include "mediapipe_custom/java/com/google/mediapipe/framework/jni/graph_gl_sync_token.h"
...
JNIEXPORT void JNICALL GRAPH_GL_SYNC_TOKEN_METHOD(nativeWaitOnCpu)(
  ...
  mediapipe_custom::GlSyncToken& token =
      *reinterpret_cast<mediapipe_custom::GlSyncToken*>(syncToken);
  token->Wait();
}
...
JNIEXPORT void JNICALL GRAPH_GL_SYNC_TOKEN_METHOD(nativeRelease)(
    JNIEnv* env, jclass cls, jlong syncToken) {
  delete reinterpret_cast<mediapipe_custom::GlSyncToken*>(syncToken);
}
