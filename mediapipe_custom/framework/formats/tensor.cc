...
#include "mediapipe_custom/framework/port/aligned_malloc_and_free.h"  // IWYU pragma: keep
...
Tensor::OpenGlTexture2dView Tensor::GetOpenGlTexture2dReadView() const {
  ...
  if (!(valid_ & kValidOpenGlTexture2d)) {
    ...
    if (gl_context_->GetGlVersion() == mediapipe_custom::GlVersion::kGLES2) {
...
void Tensor::AllocateOpenGlTexture2d() const {
  if (opengl_texture2d_ == GL_INVALID_INDEX) {
    gl_context_ = mediapipe_custom::GlContext::GetCurrent();
