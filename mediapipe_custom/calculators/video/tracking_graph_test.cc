...
#include "mediapipe_custom/calculators/video/box_tracker_calculator.pb.h"
...
#include "mediapipe_custom/framework/port/advanced_proto_inc.h"
...
#include "mediapipe_custom/util/tracking/box_tracker.pb.h"
#include "mediapipe_custom/util/tracking/tracking.pb.h"
...
class TrackingGraphTest : public Test {
 protected:
  ...
  void SetUp() override {
    test_dir_ = mediapipe_custom::GetTestDataDir(kTestPackageRoot);
    ...
    mediapipe_custom::CalculatorGraphConfig config_copy = config_;
    mediapipe_custom::tool::AddVectorSink("boxes", &config_copy, &output_packets_);
    mediapipe_custom::tool::AddVectorSink("ra_boxes", &config_copy,
    ...
    mediapipe_custom::tool::AddVectorSink("boxes", &parallel_config, &output_packets_);
    mediapipe_custom::tool::AddVectorSink("ra_boxes", &parallel_config,
                                   &random_access_results_packets_);
  ...
  void RunGraphWithSidePacketsAndInputs(
      const std::map<std::string, mediapipe_custom::Packet>& side_packets,
      const mediapipe_custom::Packet& start_pos_packet);
  ...
  std::vector<mediapipe_custom::Packet> output_packets_;
  std::vector<mediapipe_custom::Packet> random_access_results_packets_;
...
TEST_F(TrackingGraphTest, BasicBoxTrackingSanityCheck) {
  ...
  side_packets.insert(std::make_pair("analysis_downsample_factor",
                                     mediapipe_custom::MakePacket<float>(1.0f)));
  side_packets.insert(std::make_pair(
      "calculator_options",
      mediapipe_custom::MakePacket<CalculatorOptions>(CalculatorOptions())));
...
TEST_F(TrackingGraphTest, BasicQuadTrackingSanityCheck) {
  ...
  side_packets.insert(std::make_pair(
      "calculator_options",
      mediapipe_custom::MakePacket<CalculatorOptions>(calculator_options)));
