...
//    public:
//     static constexpr mediapipe_custom::api2::Input<InputData> kInput{"IN"};
//     static constexpr mediapipe_custom::api2::Output<OutputData> kOutput{"OUT"};
//
...
class CalculatorBaseFactoryFor<
    T,
    typename std::enable_if<std::is_base_of<mediapipe_custom::api2::Node, T>{}>::type>
...
MEDIAPIPE_STATIC_REGISTRATOR_TEMPLATE(
    NodeRegistrator, mediapipe_custom::CalculatorBaseRegistry, T::kCalculatorName,
    std::make_unique<mediapipe_custom::internal::CalculatorBaseFactoryFor<T>>)

MEDIAPIPE_STATIC_REGISTRATOR_TEMPLATE(SubgraphRegistrator,
                                      mediapipe_custom::SubgraphRegistry,
...
#define MEDIAPIPE_NODE_CONTRACT(...)                                          \
  static constexpr auto kContract =                                           \
      mediapipe_custom::api2::internal::MakeContract(__VA_ARGS__);                   \
  using Contract =                                                            \
      typename mediapipe_custom::api2::internal::TaggedContract<decltype(kContract), \
                                                         kContract>;
...
#define MEDIAPIPE_NODE_IMPLEMENTATION(Impl)                       \
  MEDIAPIPE_REGISTER_FACTORY_FUNCTION_QUALIFIED(                  \
      mediapipe_custom::CalculatorBaseRegistry, calculator_registration, \
      Impl::kCalculatorName,                                      \
      std::make_unique<mediapipe_custom::internal::CalculatorBaseFactoryFor<Impl>>)
...
#define MEDIAPIPE_REGISTER_NODE(name)                                    \
  MEDIAPIPE_REGISTER_FACTORY_FUNCTION_QUALIFIED(                         \
      mediapipe_custom::CalculatorBaseRegistry, calculator_registration, #name, \
      std::make_unique<mediapipe_custom::internal::CalculatorBaseFactoryFor<name>>)
...
#define MEDIAPIPE_SUBGRAPH_IMPLEMENTATION(Impl)           \
  MEDIAPIPE_REGISTER_FACTORY_FUNCTION_QUALIFIED(          \
      mediapipe_custom::SubgraphRegistry, subgraph_registration, \
