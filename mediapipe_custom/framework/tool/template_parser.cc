...
#include "mediapipe_custom/framework/tool/template_parser.h"
...
#include "mediapipe_custom/framework/deps/proto_descriptor.pb.h"
...
#include "mediapipe_custom/framework/tool/calculator_graph_template.pb.h"
#include "mediapipe_custom/framework/tool/proto_util_lite.h"

using mediapipe_custom::proto_ns::Descriptor;
using mediapipe_custom::proto_ns::DynamicMessageFactory;
using mediapipe_custom::proto_ns::EnumDescriptor;
using mediapipe_custom::proto_ns::EnumValueDescriptor;
using mediapipe_custom::proto_ns::FieldDescriptor;
using mediapipe_custom::proto_ns::Message;
using mediapipe_custom::proto_ns::OneofDescriptor;
using mediapipe_custom::proto_ns::Reflection;
using mediapipe_custom::proto_ns::TextFormat;
using ProtoPath = mediapipe_custom::tool::ProtoUtilLite::ProtoPath;
using FieldType = mediapipe_custom::tool::ProtoUtilLite::FieldType;
using FieldValue = mediapipe_custom::tool::ProtoUtilLite::FieldValue;
...
TextFormat::ParseLocation TemplateParser::ParseInfoTree::GetLocation(
  ...
  const std::vector<TextFormat::ParseLocation>* locations =
      mediapipe_custom::FindOrNull(locations_, field);
...
TemplateParser::ParseInfoTree* TemplateParser::ParseInfoTree::GetTreeForNested(
  ...
  const std::vector<std::unique_ptr<TemplateParser::ParseInfoTree>>* trees =
      mediapipe_custom::FindOrNull(nested_, field);
...
absl::Status KeyProtoMapEntries(Message* source, MessageMap* stowed_messages) {
  ...
  mediapipe_custom::CalculatorGraphTemplate rules;
 ...
 protected:
  ...
  void RecordFieldPath(const FieldDescriptor& field, const std::string& path) {
    for (int i = template_rules_.rule().size() - 1; i >= 0; --i) {
      ...
      rule->set_path(path);
      rule->set_field_type(
          static_cast<mediapipe_custom::FieldDescriptorProto::Type>(field.type()));
    }
  }

  mediapipe_custom::CalculatorGraphTemplate template_rules_;
