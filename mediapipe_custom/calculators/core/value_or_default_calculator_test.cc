...
class ValueOrDefaultRunner : public mediapipe_custom::CalculatorRunner {
 public:
  ValueOrDefaultRunner()
      : mediapipe_custom::CalculatorRunner(R"pb(
    ...
    MutableSidePackets()->Index(0) = mediapipe_custom::MakePacket<int>(kDefaultValue);
  }
  ...
  void TickAt(int64_t time) {
    ...
    MutableInputs()->Tag("TICK").packets.push_back(
        mediapipe_custom::Adopt(new bool(false)).At(mediapipe_custom::Timestamp(time)));
  }
  ...
  void ProvideInput(int64_t time, int value) {
    MutableInputs()->Tag("IN").packets.push_back(
        mediapipe_custom::Adopt(new int(value)).At(mediapipe_custom::Timestamp(time)));
  }

  // Extracts the timestamps (as int64) of the output stream of the calculator.
  std::vector<int64_t> GetOutputTimestamps() const {
    std::vector<int64_t> timestamps;
    for (const mediapipe_custom::Packet& packet : Outputs().Tag("OUT").packets) {
  ...
  std::vector<int64_t> GetFlagTimestamps() const {
    std::vector<int64_t> timestamps;
    for (const mediapipe_custom::Packet& packet : Outputs().Tag("FLAG").packets) {
      timestamps.emplace_back(packet.Timestamp().Value());
    }
    return timestamps;
  }
