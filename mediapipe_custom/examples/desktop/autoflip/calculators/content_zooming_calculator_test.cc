...
#include "mediapipe_custom/examples/desktop/autoflip/quality/kinematic_path_solver.h"
...
void AddDetectionFrameSize(const cv::Rect_<float>& position, const int64_t time,
  ...
  auto detections = std::make_unique<std::vector<mediapipe_custom::Detection>>();
  ...
  if (flags.animated_zoom.has_value()) {
    runner->MutableInputs()
        ->Tag(kAnimateZoomTag)
        .packets.push_back(
            mediapipe_custom::MakePacket<bool>(flags.animated_zoom.value())
                .At(Timestamp(time)));
  }
  ...
  if (flags.max_zoom_factor_percent.has_value()) {
    runner->MutableInputs()
        ...
        .packets.push_back(
            mediapipe_custom::MakePacket<int>(flags.max_zoom_factor_percent.value())
                .At(Timestamp(time)));
  }
  ...
  if (flags.scale_factor_percent.has_value()) {
    runner->MutableInputs()
        ...
        .packets.push_back(
            mediapipe_custom::MakePacket<int>(flags.scale_factor_percent.value())
                .At(Timestamp(time)));
  }
}
...
void CheckCropRectFloats(const float x_center, const float y_center,
  ...
  auto float_rect = output.Tag("NORMALIZED_CROP_RECT")
                        ...
                        .Get<mediapipe_custom::NormalizedRect>();
...
void CheckCropRect(const int x_center, const int y_center, const int width,
  ...
  const auto& rect = output_packets[frame_number].Get<mediapipe_custom::Rect>();
...
TEST(ContentZoomingCalculatorTest, PanConfigWithCache) {
  mediapipe_custom::autoflip::ContentZoomingCalculatorStateCacheType cache;
  auto config = ParseTextProtoOrDie<CalculatorGraphConfig::Node>(kConfigD);
  ...
  // Now repeat the last frame for a new runner without the cache to see a reset
  {
    auto runner = ::absl::make_unique<CalculatorRunner>(config);
    runner->MutableSidePackets()->Tag(kStateCacheTag) = MakePacket<
        mediapipe_custom::autoflip::ContentZoomingCalculatorStateCacheType*>(nullptr);
...
TEST(ContentZoomingCalculatorTest, ProvidesZeroSizeFirstRectWithoutDetections) {
  ...
  const auto& rect = output_packets[0].Get<mediapipe_custom::NormalizedRect>();
...
TEST(ContentZoomingCalculatorTest, ProvidesConstantFirstRect) {
  ...
  const auto& first_rect = output_packets[0].Get<mediapipe_custom::NormalizedRect>();
  ...
  for (int i = 1; i < 4; ++i) {
    const auto& rect = output_packets[i].Get<mediapipe_custom::NormalizedRect>();
...
TEST(ContentZoomingCalculatorTest, InitialEmptyDetectionDefaultsToNoCrop) {
  ...
  auto empty_detections = std::make_unique<std::vector<mediapipe_custom::Detection>>();
  runner->MutableInputs()
      ->Tag("DETECTIONS")
