...
#include "mediapipe_custom/framework/api2/contract.h"
#include "mediapipe_custom/framework/api2/node_test.pb.h"
#include "mediapipe_custom/framework/api2/packet.h"
#include "mediapipe_custom/framework/api2/port.h"
#include "mediapipe_custom/framework/api2/test_contracts.h"
...
template <typename T>
std::vector<T> PacketValues(const std::vector<mediapipe_custom::Packet>& packets) {
  std::vector<T> result;
...
TEST(NodeTest, GetContract) {
  ...
  const CalculatorGraphConfig::Node node_config =
      mediapipe_custom::ParseTextProtoOrDie<CalculatorGraphConfig::Node>(R"pb(
        calculator: "Foo"
        input_stream: "BASE:base"
        input_stream: "SCALE:scale"
        output_stream: "OUT:out"
      )pb");
  mediapipe_custom::CalculatorContract contract;
...
void RunFooCalculatorInGraph(const std::string& foo_name) {
  CalculatorGraphConfig config =
      mediapipe_custom::ParseTextProtoOrDie<CalculatorGraphConfig>(
          absl::Substitute(R"(
        input_stream: "base"
        input_stream: "scale"
        output_stream: "out"
        node {
          ...
          output_stream: "OUT:out"
                           ...
                           foo_name));
  std::vector<mediapipe_custom::Packet> out_packets;
  tool::AddVectorSink("out", &config, &out_packets);
  mediapipe_custom::CalculatorGraph graph;
  MP_EXPECT_OK(graph.Initialize(config, {}));
  MP_EXPECT_OK(graph.StartRun({}));
  MP_EXPECT_OK(graph.AddPacketToInputStream(
      "base", mediapipe_custom::MakePacket<int>(10).At(Timestamp(1))));
  MP_EXPECT_OK(graph.AddPacketToInputStream(
      "scale", mediapipe_custom::MakePacket<float>(2.0).At(Timestamp(1))));
...
TEST(NodeTest, OptionalStream) {
  CalculatorGraphConfig config =
      mediapipe_custom::ParseTextProtoOrDie<CalculatorGraphConfig>(R"pb(
        input_stream: "base"
  ...
  mediapipe_custom::CalculatorGraph graph;
  MP_EXPECT_OK(graph.Initialize(config, {}));
  MP_EXPECT_OK(graph.StartRun({{"bias", mediapipe_custom::MakePacket<float>(30.0)}}));
  MP_EXPECT_OK(graph.AddPacketToInputStream(
      "base", mediapipe_custom::MakePacket<int>(10).At(Timestamp(1))));
  MP_EXPECT_OK(graph.CloseAllPacketSources());
  MP_EXPECT_OK(graph.WaitUntilDone());
...
TEST(NodeTest, DynamicTypes) {
  ...
  MP_EXPECT_OK(graph.StartRun({}));
  MP_EXPECT_OK(graph.AddPacketToInputStream(
      "in", mediapipe_custom::MakePacket<int>(10).At(Timestamp(1))));
  MP_EXPECT_OK(graph.CloseAllPacketSources());
  MP_EXPECT_OK(graph.WaitUntilDone());
...
TEST(NodeTest, MultiPort) {
  ...
  std::vector<mediapipe_custom::Packet> out0_packets;
  std::vector<mediapipe_custom::Packet> out1_packets;
  ...
  MP_EXPECT_OK(graph.StartRun({}));
  MP_EXPECT_OK(graph.AddPacketToInputStream(
      "in0", mediapipe_custom::MakePacket<int>(10).At(Timestamp(1))));
  MP_EXPECT_OK(graph.AddPacketToInputStream(
      "in1", mediapipe_custom::MakePacket<int>(5).At(Timestamp(1))));
  MP_EXPECT_OK(graph.AddPacketToInputStream(
      "in0", mediapipe_custom::MakePacket<int>(15).At(Timestamp(2))));
  MP_EXPECT_OK(graph.AddPacketToInputStream(
      "in1", mediapipe_custom::MakePacket<int>(7).At(Timestamp(2))));
  MP_EXPECT_OK(graph.CloseAllPacketSources());
  MP_EXPECT_OK(graph.WaitUntilDone());
...
TEST(NodeTest, SideFallbackWithStream) {
  ...
  MP_EXPECT_OK(graph.Initialize(config, {}));
  MP_EXPECT_OK(
      graph.ObserveOutputStream("out", [&outputs](const mediapipe_custom::Packet& p) {
        outputs.push_back(p.Get<int>());
        return absl::OkStatus();
      }));
  MP_EXPECT_OK(graph.StartRun({}));
  MP_EXPECT_OK(graph.AddPacketToInputStream(
      "in", mediapipe_custom::MakePacket<int>(10).At(Timestamp(0))));
  MP_EXPECT_OK(graph.AddPacketToInputStream(
      "factor", mediapipe_custom::MakePacket<int>(2).At(Timestamp(0))));
  MP_EXPECT_OK(graph.CloseAllPacketSources());
  MP_EXPECT_OK(graph.WaitUntilDone());
...
TEST(NodeTest, SideFallbackWithSide) {
  ...
  MP_EXPECT_OK(graph.StartRun({{"factor", mediapipe_custom::MakePacket<int>(2)}}));
  MP_EXPECT_OK(graph.AddPacketToInputStream(
      "in", mediapipe_custom::MakePacket<int>(10).At(Timestamp(0))));
  MP_EXPECT_OK(graph.CloseAllPacketSources());
  MP_EXPECT_OK(graph.WaitUntilDone());
...
TEST(NodeTest, OneOf) {
  ...
  MP_EXPECT_OK(graph.AddPacketToInputStream(
      "in", mediapipe_custom::MakePacket<int>(10).At(Timestamp(1))));
  MP_EXPECT_OK(graph.AddPacketToInputStream(
      "in", mediapipe_custom::MakePacket<float>(5.0).At(Timestamp(2))));
  MP_EXPECT_OK(graph.CloseAllPacketSources());
  MP_EXPECT_OK(graph.WaitUntilDone());
...
TEST(NodeTest, DefaultTimestampChange0) {
  ...
  MP_EXPECT_OK(graph.StartRun({}));
  MP_EXPECT_OK(graph.AddPacketToInputStream(
      "a", mediapipe_custom::MakePacket<int>(10).At(Timestamp(2))));
  MP_EXPECT_OK(graph.AddPacketToInputStream(
      "b", mediapipe_custom::MakePacket<int>(10).At(Timestamp(2))));
...
TEST(NodeTest, ConsumeInputs) {
  ...
  MP_EXPECT_OK(graph.StartRun({}));
  MP_EXPECT_OK(graph.AddPacketToInputStream(
      "int", mediapipe_custom::MakePacket<int>(10).At(Timestamp(0))));
  MP_EXPECT_OK(graph.AddPacketToInputStream(
      "any", mediapipe_custom::MakePacket<float>(10).At(Timestamp(0))));
  MP_EXPECT_OK(graph.AddPacketToInputStream(
      "num", mediapipe_custom::MakePacket<int>(10).At(Timestamp(0))));
  MP_EXPECT_OK(graph.CloseAllPacketSources());
  MP_EXPECT_OK(graph.WaitUntilDone());
}
...
TEST_P(SidePacketConnectionTestSuite, SidePacketConnectionTest) {
  ...
  std::map<std::string, mediapipe_custom::Packet> side_packets;
  if (graph_side_in) {
