...
#include "mediapipe_custom/tasks/cc/genai/inference/utils/xnn_utils/tflite_weight_accessor.h"
  ...
  if (scoped_cache_file && scoped_cache_file->IsValid()) {
    ...
    xnn_weights_cache_ = std::make_shared<PackWeightsCache>(
        params.cache_dir.empty()
            ...
            : mediapipe_custom::file::JoinPath(
                  params.cache_dir,
                  absl::StrCat(mediapipe_custom::file::Basename(weight_path),
                               ".cache")));
  }
