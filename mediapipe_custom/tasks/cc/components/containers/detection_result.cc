...
namespace mediapipe_custom::tasks::components::containers {

constexpr int kDefaultCategoryIndex = -1;

Detection ConvertToDetection(const mediapipe_custom::Detection& detection_proto) {
  Detection detection;
  ...
  if (detection_proto.location_data().has_bounding_box()) {
    mediapipe_custom::LocationData::BoundingBox bounding_box_proto =
...
DetectionResult ConvertToDetectionResult(
    std::vector<mediapipe_custom::Detection> detections_proto) {
