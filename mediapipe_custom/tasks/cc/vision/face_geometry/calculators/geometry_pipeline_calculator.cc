...
#include "mediapipe_custom/tasks/cc/core/external_file_handler.h"
...
#include "mediapipe_custom/tasks/cc/vision/face_geometry/libs/geometry_pipeline.h"
...
#include "mediapipe_custom/tasks/cc/vision/face_geometry/proto/geometry_pipeline_metadata.pb.h"
...
using ::mediapipe_custom::tasks::vision::face_geometry::proto::
  ...
  static absl::Status GetContract(CalculatorContract* cc) {
    ...
    if (cc->Inputs().HasTag(kMultiFaceLandmarksTag)) {
      cc->Inputs()
          ...
          .Set<std::vector<mediapipe_custom::NormalizedLandmarkList>>();
      cc->Outputs().Tag(kMultiFaceGeometryTag).Set<std::vector<FaceGeometry>>();
      return absl::OkStatus();
    } else {
      cc->Inputs()
          .Tag(kFaceLandmarksTag)
          .Set<mediapipe_custom::NormalizedLandmarkList>();
  ...
  absl::Status Process(CalculatorContext* cc) override {
    ...
    if (cc->Inputs().HasTag(kMultiFaceLandmarksTag)) {
      ...
      cc->Outputs()
          ...
          .AddPacket(mediapipe_custom::Adopt<std::vector<FaceGeometry>>(
      ...
      const auto& face_landmarks =
          cc->Inputs()
              .Tag(kFaceLandmarksTag)
              .Get<mediapipe_custom::NormalizedLandmarkList>();
      ...
      cc->Outputs()
          ...
          .AddPacket(mediapipe_custom::MakePacket<FaceGeometry>(multi_face_geometry[0])
                         .At(cc->InputTimestamp()));
    }
...
REGISTER_CALCULATOR(
    ::mediapipe_custom::tasks::vision::face_geometry::FaceGeometryPipelineCalculator);
