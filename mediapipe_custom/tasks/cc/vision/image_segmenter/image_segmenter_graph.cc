...
#include "mediapipe_custom/calculators/image/image_clone_calculator.pb.h"
#include "mediapipe_custom/calculators/image/image_transformation_calculator.pb.h"
#include "mediapipe_custom/calculators/image/set_alpha_calculator.pb.h"
#include "mediapipe_custom/calculators/tensor/tensor_converter_calculator.pb.h"
...
#include "mediapipe_custom/tasks/cc/components/processors/proto/image_preprocessing_graph_options.pb.h"
#include "mediapipe_custom/tasks/cc/core/model_resources.h"
#include "mediapipe_custom/tasks/cc/core/model_task_graph.h"
#include "mediapipe_custom/tasks/cc/core/proto/inference_subgraph.pb.h"
#include "mediapipe_custom/tasks/cc/metadata/metadata_extractor.h"
#include "mediapipe_custom/tasks/cc/vision/image_segmenter/calculators/tensors_to_segmentation_calculator.pb.h"
#include "mediapipe_custom/tasks/cc/vision/image_segmenter/proto/image_segmenter_graph_options.pb.h"
#include "mediapipe_custom/tasks/cc/vision/image_segmenter/proto/segmenter_options.pb.h"
#include "mediapipe_custom/tasks/cc/vision/utils/image_tensor_specs.h"
#include "mediapipe_custom/tasks/metadata/image_segmenter_metadata_schema_generated.h"
#include "mediapipe_custom/tasks/metadata/metadata_schema_generated.h"
#include "mediapipe_custom/util/graph_builder_utils.h"
#include "mediapipe_custom/util/label_map.pb.h"
#include "mediapipe_custom/util/label_map_util.h"
...
using ::mediapipe_custom::api2::builder::MultiSource;
using ::mediapipe_custom::api2::builder::Source;
using ::mediapipe_custom::tasks::metadata::ModelMetadataExtractor;
using ::mediapipe_custom::tasks::vision::image_segmenter::proto::
    ImageSegmenterGraphOptions;
using ::mediapipe_custom::tasks::vision::image_segmenter::proto::SegmenterOptions;
using ::tflite::TensorMetadata;
using LabelItems = mediapipe_custom::proto_ns::Map<int64_t, ::mediapipe_custom::LabelMapItem>;
...
absl::StatusOr<LabelItems> GetLabelItemsIfAny(
  ...
  absl::StatusOr<absl::string_view> labels_file;
  if (!labels_filename.empty()) {
    labels_file = metadata_extractor.GetAssociatedFile(labels_filename);
  }
  if (labels_filename.empty() || !labels_file.ok()) {
  ...
  return mediapipe_custom::BuildLabelMapFromFiles(*labels_file, display_names_file);
}
...
void ConfigureImageTransformationCalculator(
    ...
    mediapipe_custom::ImageTransformationCalculatorOptions& options) {
...
void ConfigureTensorConverterCalculator(
    ...
    mediapipe_custom::TensorConverterCalculatorOptions& options) {
...
absl::StatusOr<ImageAndTensorsOnDevice> ConvertImageToTensors(
  ...
  if (input_tensor_channel != 3 && input_tensor_channel != 4) {
    ...
    image_clone.GetOptions<mediapipe_custom::ImageCloneCalculatorOptions>()
    ...
    if (is_hair_segmentation) {
      ...
      set_alpha.GetOptions<mediapipe_custom::SetAlphaCalculatorOptions>()
          .set_alpha_value(0);
    ...
    ConfigureImageTransformationCalculator(
        *tflite_input_tensor,
        image_transformation
            .GetOptions<mediapipe_custom::ImageTransformationCalculatorOptions>());
    ...
    ConfigureTensorConverterCalculator(
        image_tensor_specs,
        tensor_converter
            .GetOptions<mediapipe_custom::TensorConverterCalculatorOptions>());
...
class ImageSegmenterGraph : public core::ModelTaskGraph {
 public:
  absl::StatusOr<mediapipe_custom::CalculatorGraphConfig> GetConfig(
      mediapipe_custom::SubgraphContext* sc) override {
 ...
 private:
  absl::Status SanityCheck(mediapipe_custom::SubgraphContext* sc) {
  ...
  // (mediapipe_custom::Image) as the input and returns segmented image mask as output.
  //
  ...
  // image_in: (mediapipe_custom::Image) stream to run segmentation on.
...
REGISTER_MEDIAPIPE_GRAPH(
    ::mediapipe_custom::tasks::vision::image_segmenter::ImageSegmenterGraph);
