...
#include "mediapipe_custom/calculators/audio/time_series_framer_calculator.pb.h"
...
void BM_TimeSeriesFramerCalculator(benchmark::State& state) {
  ...
  mediapipe_custom::TimeSeriesFramerCalculatorOptions* options =
      node->mutable_options()->MutableExtension(
          mediapipe_custom::TimeSeriesFramerCalculatorOptions::ext);
  ...
  for (auto _ : state) {
    ...
    std::vector<mediapipe_custom::Packet> input_packets;
    ...
    for (int i = 0; i < 32; ++i) {
      ...
      input_packets.push_back(mediapipe_custom::Adopt(samples.release())
                                  .At(mediapipe_custom::Timestamp::FromSeconds(t)));
    ...
    auto header = std::make_unique<mediapipe_custom::TimeSeriesHeader>();
