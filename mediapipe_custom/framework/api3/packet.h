...
class Packet {
 public:
  ...
  explicit Packet(mediapipe_custom::Packet p) : packet_(std::move(p)) {}
  ...
  mediapipe_custom::Timestamp Timestamp() const { return packet_.Timestamp(); }

  const mediapipe_custom::Packet& AsLegacyPacket() const { return packet_; }

  std::string DebugString() const { return packet_.DebugString(); }

 private:
  mediapipe_custom::Packet packet_;
};
...
Packet<T> MakePacket(Args&&... args) {
  return Packet<T>(mediapipe_custom::MakePacket<T>(std::forward<Args>(args)...));
}
template <typename T>
Packet<T> MakePacket(std::unique_ptr<T> ptr) {
  return Packet<T>(mediapipe_custom::Adopt(ptr.release()));
}
...
Packet<T> PointToForeign(const T* ptr,
  ...
  return Packet<T>(mediapipe_custom::PointToForeign(ptr, std::move(cleanup)));
}
