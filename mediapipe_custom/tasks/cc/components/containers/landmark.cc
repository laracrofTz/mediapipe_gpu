...
Landmark ConvertToLandmark(const mediapipe_custom::Landmark& proto) {
...
NormalizedLandmark ConvertToNormalizedLandmark(
    const mediapipe_custom::NormalizedLandmark& proto) {
  return {/*x=*/proto.x(), /*y=*/proto.y(), /*z=*/proto.z(),
          /*visibility=*/proto.has_visibility()
              ? std::optional<float>(proto.visibility())
              : std::nullopt,
...
Landmarks ConvertToLandmarks(const mediapipe_custom::LandmarkList& proto) {
  Landmarks landmarks;
...
NormalizedLandmarks ConvertToNormalizedLandmarks(
    const mediapipe_custom::NormalizedLandmarkList& proto) {
