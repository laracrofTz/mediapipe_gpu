...
#include "mediapipe_custom/framework/legacy_calculator_support.h"
...
#include "mediapipe_custom/framework/port/topologicalsorter.h"
#include "mediapipe_custom/framework/status_handler.h"
#include "mediapipe_custom/framework/stream_handler.pb.h"
#include "mediapipe_custom/framework/thread_pool_executor.pb.h"
#include "mediapipe_custom/framework/tool/name_util.h"
#include "mediapipe_custom/framework/tool/status_util.h"
#include "mediapipe_custom/framework/tool/subgraph_expansion.h"
#include "mediapipe_custom/framework/tool/validate_name.h"
#include "mediapipe_custom/framework/vlog_utils.h"
...
std::string DebugName(const CalculatorGraphConfig& config,
  ...
  switch (node_type) {
    case NodeTypeInfo::NodeType::CALCULATOR:
      return mediapipe_custom::DebugName(config.node(node_index));
...
absl::Status NodeTypeInfo::Initialize(
  ...
  for (const mediapipe_custom::protobuf::Any& options : node.node_options()) {
  ...
  if (!status.ok()) {
    statuses.push_back(
        mediapipe_custom::StatusBuilder(std::move(status), MEDIAPIPE_LOC).SetPrepend()
        << "For input streams ");
  }
...
absl::Status ValidatedGraphConfig::AddOutputSidePacketsForNode(
  ...
  for (CollectionItemId id = tag_map.BeginId(); id < tag_map.EndId(); ++id) {
    ...
    if (!mediapipe_custom::InsertIfNotPresent(&side_packet_to_producer_, name,
    ...
    if (mediapipe_custom::ContainsKey(required_side_packets_, name)) {
      if (need_sorting_ptr) {
...
absl::Status ValidatedGraphConfig::AddOutputStream(NodeTypeInfo::NodeRef node,
  ...
  if (!mediapipe_custom::InsertIfNotPresent(&stream_to_producer_, name,
  ...
  for (const EdgeInfo& stream : input_streams_) {
    ...
    if (!stream.packet_type->IsConsistentWith(
      ...
      return absl::UnknownError(absl::Substitute(
          ...
          "\"$3\"",
          stream.name,
          mediapipe_custom::DebugName(config_.node(stream.parent_node.index)),
...
absl::Status ValidatedGraphConfig::ValidateRequiredSidePackets(
  ...
  for (const auto& required_item : required_side_packets_) {
    ...
    if (iter == side_packets.end()) {
      ...
      statuses.push_back(mediapipe_custom::InvalidArgumentErrorBuilder(MEDIAPIPE_LOC)
    ...
    for (int index : required_item.second) {
      ...
      if (!status.ok()) {
        statuses.push_back(
            mediapipe_custom::StatusBuilder(std::move(status), MEDIAPIPE_LOC)
                .SetPrepend()
            << "Side packet \"" << required_item.first
            << "\" failed validation: ");
      }
