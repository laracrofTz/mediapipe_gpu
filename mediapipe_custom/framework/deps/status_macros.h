...
#include "mediapipe_custom/framework/deps/source_location.h"
#include "mediapipe_custom/framework/deps/status_builder.h"
...
//   }
//
// The macro ends with a `mediapipe_custom::StatusBuilder` which allows the returned
...
//   }
//
// `mediapipe_custom::StatusBuilder` supports adapting the builder chain using a
...
// interfaces.  See `mediapipe_custom::TaskReturn` and `rpc::RpcSetStatus` for
// details.
//
//   void Read(StringPiece name, mediapipe_custom::Task* task) {
//     int64 id;
...
// to avoid confusion between a `mediapipe_custom::StatusBuilder` and a
...
#define MP_RETURN_IF_ERROR(expr)                                     \
  ...
  if (mediapipe_custom::status_macro_internal::StatusAdaptorForMacros       \
...
// well as a `mediapipe_custom::StatusBuilder` object populated with the error and
...
#define MP_STATUS_MACROS_IMPL_MP_ASSIGN_OR_RETURN_2_(lhs, rexpr)               \
  MP_STATUS_MACROS_IMPL_MP_ASSIGN_OR_RETURN_(                                  \
      ...
      return mediapipe_custom::StatusBuilder(                                         \
...
#define MP_STATUS_MACROS_IMPL_MP_ASSIGN_OR_RETURN_3_(lhs, rexpr,               \
  ...
  MP_STATUS_MACROS_IMPL_MP_ASSIGN_OR_RETURN_(                                  \
      MP_STATUS_MACROS_IMPL_CONCAT_(_status_or_value, __LINE__), lhs, rexpr,   \
      mediapipe_custom::StatusBuilder _(                                              \
          std::move(MP_STATUS_MACROS_IMPL_CONCAT_(_status_or_value, __LINE__)) \
              .status(),                                                       \
          MEDIAPIPE_LOC);                                                      \
