...
struct VisibilityCopyNode : Node<kVisibilityCopyNodeName> {
  ...
  struct Contract {
    ...
    Optional<Input<S, mediapipe_custom::LandmarkList>> in_landmarks_from{
        "LANDMARKS_FROM"};
    // A NormalizedLandmarkList of landmarks to copy from.
    Optional<Input<S, mediapipe_custom::NormalizedLandmarkList>>
        in_norm_landmarks_from{"NORM_LANDMARKS_FROM"};

    // A LandmarkList of landmarks to copy to.
    Optional<Input<S, mediapipe_custom::LandmarkList>> in_landmarks_to{"LANDMARKS_TO"};
    // An output landmarks.
    Optional<Output<S, mediapipe_custom::LandmarkList>> out_landmarks_to{
        "LANDMARKS_TO"};

    // A NormalizedLandmarkList of landmarks to copy to.
    Optional<Input<S, mediapipe_custom::NormalizedLandmarkList>> in_norm_landmarks_to{
        "NORM_LANDMARKS_TO"};
    // An output NormalizedLandmarkList.
    Optional<Output<S, mediapipe_custom::NormalizedLandmarkList>>
        out_norm_landmarks_to{"NORM_LANDMARKS_TO"};

    Options<S, mediapipe_custom::VisibilityCopyCalculatorOptions> options;
  };
