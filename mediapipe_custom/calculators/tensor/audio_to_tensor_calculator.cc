...
#include "mediapipe_custom/util/time_series_util.h"
#include "pffft.h"
...
using Options = ::mediapipe_custom::AudioToTensorCalculatorOptions;
...
//
// Inputs:
//   AUDIO - mediapipe_custom::Matrix
//     The audio data represented as mediapipe_custom::Matrix.
...
absl::Status AudioToTensorCalculator::Open(CalculatorContext* cc) {
  ...
  const auto& options =
      cc->Options<mediapipe_custom::AudioToTensorCalculatorOptions>();
  ...
  if (options.has_source_sample_rate()) {
    ...
    if (!kAudioIn(cc).Header().IsEmpty()) {
      mediapipe_custom::TimeSeriesHeader input_header;
      MP_RETURN_IF_ERROR(
          mediapipe_custom::time_series_util::FillTimeSeriesHeaderIfValid(
...
absl::Status AudioToTensorCalculator::Process(CalculatorContext* cc) {
  if (cc->InputTimestamp() == Timestamp::PreStream()) {
    ...
    if (cc->Options<mediapipe_custom::AudioToTensorCalculatorOptions>()
            .stream_mode()) {
...
absl::Status AudioToTensorCalculator::ProcessStreamingData(
  ...
  if (source_sample_rate_ != -1 && check_inconsistent_timestamps_) {
    mediapipe_custom::time_series_util::LogWarningIfTimestampIsInconsistent(
