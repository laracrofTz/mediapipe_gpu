...
#include "mediapipe_custom/framework/profiler/graph_tracer.h"
...
#include "mediapipe_custom/framework/calculator_profile.pb.h"
...
#include "mediapipe_custom/framework/profiler/test_context_builder.h"
#include "mediapipe_custom/framework/tool/simulation_clock.h"
#include "mediapipe_custom/framework/tool/simulation_clock_executor.h"
...
TEST_F(GraphTracerTest, EmptyTrace) {
  ...
  EXPECT_THAT(GetTrace(),
              EqualsProto(mediapipe_custom::ParseTextProtoOrDie<GraphTrace>(R"pb(
                base_time: 0
                base_timestamp: 0
                stream_name: ""
              )pb")));
...
TEST_F(GraphTracerTest, CalculatorTrace) {
  ...
  // Validate the GraphTrace data.
  EXPECT_THAT(
      GetTrace(), EqualsProto(mediapipe_custom::ParseTextProtoOrDie<GraphTrace>(R"pb(
...
class GraphTracerE2ETest : public ::testing::Test {
 protected:
  ...
  void SetUpRealClock() { clock_ = mediapipe_custom::Clock::RealClock(); }
...
TEST_F(GraphTracerE2ETest, PassThroughGraphProfile) {
  ...
  CalculatorProfile expected =
      mediapipe_custom::ParseTextProtoOrDie<CalculatorProfile>(R"pb(
...
TEST_F(GraphTracerE2ETest, DemuxGraphLog) {
  ...
  EXPECT_THAT(
      trace_2,
      EqualsProto(mediapipe_custom::ParseTextProtoOrDie<GraphTrace>(
          R"pb(
...
TEST_F(GraphTracerE2ETest, DemuxGraphLogFiles) {
  ...
  EXPECT_THAT(profile_2,
              EqualsProto(mediapipe_custom::ParseTextProtoOrDie<GraphProfile>(R"pb(
                graph_trace {
                  base_time: 1544086800000000
                  base_timestamp: 0
...
TEST_F(GraphTracerE2ETest, DisableLoggingToDisk) {
  ...
  EXPECT_TRUE(absl::IsNotFound(
      mediapipe_custom::file::Exists(absl::StrCat(log_path, 0, ".binarypb"))));
}
...
TEST_F(GraphTracerE2ETest, LoggingHappensWithDefaultPath) {
  ...
  RunDemuxInFlightGraph();
  MP_EXPECT_OK(mediapipe_custom::file::Exists(log_path));
}
