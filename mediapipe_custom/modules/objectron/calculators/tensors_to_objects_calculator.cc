...
#include "mediapipe_custom/modules/objectron/calculators/annotation_data.pb.h"
#include "mediapipe_custom/modules/objectron/calculators/belief_decoder_config.pb.h"
#include "mediapipe_custom/modules/objectron/calculators/decoder.h"
#include "mediapipe_custom/modules/objectron/calculators/tensor_util.h"
#include "mediapipe_custom/modules/objectron/calculators/tensors_to_objects_calculator.pb.h"
...
class TensorsToObjectsCalculator : public CalculatorBase {
 ...
 private:
  ...
  ::mediapipe_custom::TensorsToObjectsCalculatorOptions options_;
...
absl::Status TensorsToObjectsCalculator::GetContract(CalculatorContract* cc) {
  ...
  if (cc->Inputs().HasTag(kInputStreamTag)) {
    cc->Inputs().Tag(kInputStreamTag).Set<std::vector<mediapipe_custom::Tensor>>();
  }
...
absl::Status TensorsToObjectsCalculator::ProcessCPU(
  ...
  const auto& input_tensors =
      cc->Inputs().Tag(kInputStreamTag).Get<std::vector<mediapipe_custom::Tensor>>();
...
absl::Status TensorsToObjectsCalculator::LoadOptions(CalculatorContext* cc) {
  ...
  options_ = cc->Options<::mediapipe_custom::TensorsToObjectsCalculatorOptions>();
