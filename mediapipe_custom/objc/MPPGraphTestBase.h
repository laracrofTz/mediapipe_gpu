...
@interface MPPGraphTestBase : XCTestCase <MPPGraphDelegate> {
  ...
  void (^_packetOutputBlock)(MPPGraph* graph, const mediapipe_custom::Packet& packet,
                             const std::string& streamName);
}
...
/// in packets each with timestamp mediapipe_custom::Timestamp(1). Those packets are added to the
...
- (void)testGraphConfig:(const mediapipe_custom::CalculatorGraphConfig&)config
      inputStreamsAndFiles:(NSDictionary<NSString*, NSString*>*)fileInputs
    inputStreamsAndPackets:(const std::map<std::string, mediapipe_custom::Packet>&)packetInputs
               sidePackets:(std::map<std::string, mediapipe_custom::Packet>)sidePackets
                 timestamp:(mediapipe_custom::Timestamp)timestamp
