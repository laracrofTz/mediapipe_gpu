...
#include "mediapipe_custom/calculators/tensor/inference_calculator.pb.h"
...
#include "mediapipe_custom/framework/tool/options_util.h"
#include "mediapipe_custom/framework/tool/test_util.h"
#include "mediapipe_custom/gpu/gpu_origin.pb.h"
#include "mediapipe_custom/modules/face_detection/face_detection.pb.h"

#if !defined(__APPLE__) && !__ANDROID__
#include "mediapipe_custom/gpu/gl_app_texture_support.h"
#include "mediapipe_custom/gpu/gl_calculator_helper.h"
#include "mediapipe_custom/gpu/gpu_test_base.h"
#endif  // !defined(__APPLE__) && !__ANDROID__

namespace mediapipe_custom {
namespace {
using mediapipe_custom::FaceDetectionOptions;

// Ensure protobuf registration.
void RegisterProtobufTypes() {
  MakePacket<mediapipe_custom::InferenceCalculatorOptions::Delegate>();
  MakePacket<mediapipe_custom::FaceDetectionOptions>();
  MakePacket<mediapipe_custom::InferenceCalculatorOptions>();

  MakePacket<mediapipe_custom::ImageToTensorCalculatorOptions>();
  MakePacket<mediapipe_custom::SsdAnchorsCalculatorOptions>();
  MakePacket<mediapipe_custom::TensorsToDetectionsCalculatorOptions>();
  MakePacket<mediapipe_custom::NonMaxSuppressionCalculatorOptions>();
}
...
FaceDetectionOptions GetGpuOptions() {
  FaceDetectionOptions result;
  result.set_gpu_origin(mediapipe_custom::GpuOrigin_Mode::GpuOrigin_Mode_TOP_LEFT);
  result.mutable_delegate()->mutable_gpu()->set_use_advanced_gpu_api(true);
  return result;
}

// Returns an example region of interest rectangle.
mediapipe_custom::NormalizedRect GetTestRoi() {
  mediapipe_custom::NormalizedRect result;
...
TEST_F(FaceDetectionTest, ExpandFaceDetectionShortRangeCpu) {
  ...
  EXPECT_EQ(output.front().Get<std::vector<mediapipe_custom::Detection>>().size(), 1);
}

TEST_F(FaceDetectionTest, ExpandFaceDetection) {
  CalculatorGraphConfig config = GetFaceDetectionGraph();
  mediapipe_custom::FaceDetectionOptions face_options =
...
TEST_F(FaceDetectionTest, FaceDetectionShortRangeByRoiCpu) {
  ...
  MP_ASSERT_OK(graph.AddPacketToInputStream(
      "roi", MakePacket<mediapipe_custom::NormalizedRect>(GetTestRoi())
                 .At(Timestamp(20000))));
  MP_ASSERT_OK(graph.CloseAllPacketSources());
  MP_EXPECT_OK(graph.WaitUntilDone());
  ASSERT_EQ(output.size(), 1);
...
class FaceDetectionGpuTest : public mediapipe_custom::GpuTestBase {
 protected:
  void SetUp() override {}
  void TearDown() override {}
...
TEST_F(FaceDetectionGpuTest, FaceDetectionFullRangeGpu) {
  ...
  MP_ASSERT_OK(mediapipe_custom::SetExternalGlContextForGraph(
