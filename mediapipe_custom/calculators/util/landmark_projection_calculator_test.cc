#include "mediapipe_custom/calculators/util/landmark_projection_calculator.h"
...
TEST(LandmarkProjectionCalculatorTest, ProjectingWithDefaultRect) {
  mediapipe_custom::NormalizedLandmarkList landmarks =
      ParseTextProtoOrDie<mediapipe_custom::NormalizedLandmarkList>(R"pb(
        landmark { x: 10, y: 20, z: -0.5 }
      )pb");
  mediapipe_custom::NormalizedRect rect =
      ParseTextProtoOrDie<mediapipe_custom::NormalizedRect>(
          R"pb(
            x_center: 0.5,
            y_center: 0.5,
            width: 1.0,
  ...
  EXPECT_THAT(
      result,
      EqualsProto(ParseTextProtoOrDie<mediapipe_custom::NormalizedLandmarkList>(R"pb(
        landmark { x: 10, y: 20, z: -0.5 }
      )pb")));
}
...
mediapipe_custom::NormalizedRect GetCroppedRect() {
  return ParseTextProtoOrDie<mediapipe_custom::NormalizedRect>(
      R"pb(
        x_center: 0.5, y_center: 0.5, width: 0.5, height: 2, rotation: 0.0
      )pb");
}

mediapipe_custom::NormalizedLandmarkList GetCroppedRectTestInput() {
  return ParseTextProtoOrDie<mediapipe_custom::NormalizedLandmarkList>(R"pb(
    landmark { x: 1.0, y: 1.0, z: -0.5 }
  )pb");
}

mediapipe_custom::NormalizedLandmarkList GetCroppedRectTestExpectedResult() {
  return ParseTextProtoOrDie<mediapipe_custom::NormalizedLandmarkList>(R"pb(
...
mediapipe_custom::NormalizedRect GetCroppedRectWith90degreeRotation() {
  return ParseTextProtoOrDie<mediapipe_custom::NormalizedRect>(
      R"pb(
        x_center: 0.5,
        y_center: 0.5,
...
mediapipe_custom::NormalizedLandmarkList GetCroppedRectTestInputForRotation() {
  return ParseTextProtoOrDie<mediapipe_custom::NormalizedLandmarkList>(R"pb(
