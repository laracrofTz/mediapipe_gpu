...
#include "mediapipe_custom/calculators/tensor/tensors_to_segmentation_converter.h"
#include "mediapipe_custom/calculators/tensor/tensors_to_segmentation_utils.h"
...
using ::mediapipe_custom::tensors_to_segmentation_utils::GetHwcFromDims;
using ::mediapipe_custom::tensors_to_segmentation_utils::GlRender;
using ::mediapipe_custom::tensors_to_segmentation_utils::NumGroups;
...
// One and only one of FN_SOFTMAX,FN_SIGMOID,FN_NONE will be defined.
  ...
  output_texture.write(out_value, output_coordinate);
    ...
    // Shader defines.
    using Options = ::mediapipe_custom::TensorsToSegmentationCalculatorOptions;
    ...
    mediapipe_custom::GlhCreateProgram(
        mediapipe_custom::kBasicVertexShader, mediapipe_custom::kBasicTexturedFragmentShader,
...
TensorsToSegmentationMetalConverter::Convert(const Tensor& input_tensor,
  ...
  MP_RETURN_IF_ERROR(gpu_helper_.RunInGlContext([this, &input_tensor,
    ...
    mediapipe_custom::GlTexture small_mask_texture;
    ...
    // Run shader, process mask tensor.
    {
      ...
      mediapipe_custom::GpuBuffer small_mask_buffer = [metal_helper_
          mediapipeGpuBufferWithWidth:tensor_width
                               height:tensor_height
                               format:mediapipe_custom::GpuBufferFormat::kBGRA32];
...
CreateMetalConverter(
    CalculatorContext* cc,
    const mediapipe_custom::TensorsToSegmentationCalculatorOptions& options) {
