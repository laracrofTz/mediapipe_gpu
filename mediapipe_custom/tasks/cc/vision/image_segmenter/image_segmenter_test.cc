...
#include "mediapipe_custom/tasks/cc/vision/image_segmenter/image_segmenter.h"
...
#include "mediapipe_custom/tasks/cc/components/containers/rect.h"
...
#include "mediapipe_custom/tasks/cc/vision/core/image_processing_options.h"
#include "mediapipe_custom/tasks/cc/vision/image_segmenter/calculators/tensors_to_segmentation_calculator.pb.h"
#include "mediapipe_custom/tasks/cc/vision/image_segmenter/image_segmenter_result.h"
...
using ::mediapipe_custom::tasks::components::containers::RectF;
using ::mediapipe_custom::tasks::vision::core::ImageProcessingOptions;
...
Image GetSRGBImage(const std::string& image_path) {
  ...
  mediapipe_custom::ImageFrame image_frame(
      mediapipe_custom::ImageFormat::SRGB, image_mat.cols, image_mat.rows,
      image_mat.step, image_mat.data, [image_mat](uint8_t[]) {});
  Image image(std::make_shared<mediapipe_custom::ImageFrame>(std::move(image_frame)));
  return image;
}
...
Image GetSRGBAImage(const std::string& image_path) {
  ...
  mediapipe_custom::ImageFrame image_frame(
      mediapipe_custom::ImageFormat::SRGBA, image_mat.cols, image_mat.rows,
      image_mat.step, image_mat.data, [image_mat](uint8_t[]) {});
  Image image(std::make_shared<mediapipe_custom::ImageFrame>(std::move(image_frame)));
  return image;
}
...
TEST_F(ImageModeTest, SucceedsWithCategoryMask) {
  ...
  cv::Mat actual_mask = mediapipe_custom::formats::MatView(
...
TEST_F(ImageModeTest, SucceedsWithConfidenceMask) {
  ...
  cv::Mat cat_mask = mediapipe_custom::formats::MatView(
...
TEST_F(ImageModeTest, SucceedsSelfie128x128Segmentation) {
  ...
  cv::Mat selfie_mask = mediapipe_custom::formats::MatView(
...
TEST_F(ImageModeTest, SucceedsHairSegmentation) {
  ...
  cv::Mat hair_mask = mediapipe_custom::formats::MatView(
      result.confidence_masks->at(1).GetImageFrameSharedPtr().get());
  MP_ASSERT_OK(segmenter->Close());
  cv::Mat expected_mask = cv::imread(
