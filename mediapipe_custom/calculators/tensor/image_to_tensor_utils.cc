...
BorderMode GetBorderMode(
    const mediapipe_custom::ImageToTensorCalculatorOptions::BorderMode& mode) {
  switch (mode) {
    case mediapipe_custom::
        ImageToTensorCalculatorOptions_BorderMode_BORDER_UNSPECIFIED:
      return BorderMode::kReplicate;
    case mediapipe_custom::ImageToTensorCalculatorOptions_BorderMode_BORDER_ZERO:
      return BorderMode::kZero;
    case mediapipe_custom::ImageToTensorCalculatorOptions_BorderMode_BORDER_REPLICATE:
      return BorderMode::kReplicate;
  }
}
...
int GetNumOutputChannels(const mediapipe_custom::Image& image) {
#if !MEDIAPIPE_DISABLE_GPU
...
absl::StatusOr<std::shared_ptr<const mediapipe_custom::Image>> GetInputImage(
    const api2::Packet<api2::OneOf<Image, mediapipe_custom::ImageFrame>>&
        image_packet) {
  return image_packet.Visit(
      [&image_packet](const mediapipe_custom::Image&) {
        return image_packet.Share<mediapipe_custom::Image>();
      },
      [&image_packet](const mediapipe_custom::ImageFrame&)
          -> absl::StatusOr<std::shared_ptr<const mediapipe_custom::Image>> {
        MP_ASSIGN_OR_RETURN(
            std::shared_ptr<const mediapipe_custom::ImageFrame> image_frame,
            image_packet.Share<mediapipe_custom::ImageFrame>());
        return std::make_shared<const mediapipe_custom::Image>(
            std::const_pointer_cast<mediapipe_custom::ImageFrame>(
...
#if !MEDIAPIPE_DISABLE_GPU
absl::StatusOr<std::shared_ptr<const mediapipe_custom::Image>> GetInputImage(
    const api2::Packet<mediapipe_custom::GpuBuffer>& image_gpu_packet) {
  // A shallow copy is okay since the resulting 'image' object is local in
  // Process(), and thus never outlives 'input'.
  return std::make_shared<const mediapipe_custom::Image>(image_gpu_packet.Get());
}
#endif  // !MEDIAPIPE_DISABLE_GPU
