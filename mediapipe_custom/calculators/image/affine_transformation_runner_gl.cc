...
using mediapipe_custom::GlCalculatorHelper;
using mediapipe_custom::GlhCreateProgram;
using mediapipe_custom::GlTexture;
using mediapipe_custom::GpuBuffer;
using mediapipe_custom::GpuOrigin;
...
bool IsGlClampToBorderSupported(const mediapipe_custom::GlContext& gl_context) {
 ...
 public:
  ...
  absl::Status Init() {
    return gl_helper_->RunInGlContext([this]() -> absl::Status {
      ...
      const std::string vert_src =
          absl::StrCat(mediapipe_custom::kMediaPipeVertexShaderPreamble, kVertShader);
      ...
      auto create_custom_zero_fn = [&]() -> absl::StatusOr<Program> {
        ...
        const std::string frag_custom_zero_src = absl::StrCat(
            mediapipe_custom::kMediaPipeFragmentShaderPreamble,
  ...
  absl::Status RunInternal(const GlTexture& texture,
    ...
    MP_ASSIGN_OR_RETURN(bool is_matrix_vertical_flip_needed,
                        mediapipe_custom::IsGpuOriginAtBottom(gpu_origin_));
