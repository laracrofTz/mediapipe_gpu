...
#include "mediapipe_custom/framework/api3/testing/generator.pb.h"
...
#include "mediapipe_custom/framework/stream_handler/fixed_size_input_stream_handler.pb.h"
#include "mediapipe_custom/framework/testdata/night_light_calculator.pb.h"
#include "mediapipe_custom/framework/testdata/sky_light_calculator.pb.h"
...
TEST(BuilderTest, BuildGraphSettingInputAndOutputStreamHandlers) {
  ...
  auto& foo_ish_opts =
      foo.SetLegacyInputStreamHandler("FixedSizeInputStreamHandler")
          .GetOptions<mediapipe_custom::FixedSizeInputStreamHandlerOptions>();
...
struct FloatGenerator : Node<kFloatGeneratorName> {
  ...
  struct Contract {
    ...
    Options<S, mediapipe_custom::GeneratorOptions> options;
  };
};
...
struct SkyLightProto3Node : Node<kSkyLightProto3NodeName> {
  ...
  struct Contract {
    ...
    Options<S, mediapipe_custom::SkyLightCalculatorOptions> options;
  };
};
...
struct NightLightProto2Node : Node<kNightLightProto3NodeName> {
  ...
  struct Contract {
    ...
    Options<S, mediapipe_custom::NightLightCalculatorOptions> options;
  };
};
...
struct Proto2And3Node : Node<kProto2And3NodeName> {
  ...
  struct Contract {
    ...
    Options<S, mediapipe_custom::SkyLightCalculatorOptions> proto3_options;
    Options<S, mediapipe_custom::NightLightCalculatorOptions> proto2_options;
  };
};
...
TEST(GraphTest, CanAccessGraphInputsOutputs) {
  ...
  EXPECT_THAT(
      config,
      EqualsProto(ParseTextProtoOrDie<mediapipe_custom::CalculatorGraphConfig>(R"pb(
        node {
