...
#include "mediapipe_custom/calculators/util/latency.pb.h"
...
class PacketLatencyCalculatorTest : public ::testing::Test {
 protected:
  ...
  void InitializeSingleStreamGraph() {
    ...
    mediapipe_custom::tool::AddVectorSink("packet_latency_0", &graph_config_,
                                   &out_0_packets_);

    // Create the simulation clock side packet.
    SetupSimulationClock();
    std::map<std::string, ::mediapipe_custom::Packet> side_packet;
    side_packet["clock"] =
        ::mediapipe_custom::MakePacket<std::shared_ptr<::mediapipe_custom::Clock>>(
            simulation_clock_);
  ...
  void InitializeMultipleStreamGraph() {
    ...
    mediapipe_custom::tool::AddVectorSink("packet_latency_0", &graph_config_,
                                   &out_0_packets_);
    mediapipe_custom::tool::AddVectorSink("packet_latency_1", &graph_config_,
                                   &out_1_packets_);
    mediapipe_custom::tool::AddVectorSink("packet_latency_2", &graph_config_,
                                   &out_2_packets_);
    MP_ASSERT_OK(graph_.Initialize(graph_config_, {}));
  ...
  std::shared_ptr<::mediapipe_custom::Clock> simulation_clock_;
