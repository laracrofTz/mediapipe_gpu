...
/**
 * Helper utility for converting `MPPImage` into a `mediapipe_custom::ImageFrame`.
 */
@interface MPPImage (Utils)
/**
 * Converts the `MPPImage` into a `mediapipe_custom::ImageFrame`.
 ...
 * error will be saved.
 *
 * @return An std::unique_ptr<mediapipe_custom::ImageFrame> or `nullptr` in case of errors.
 */
- (std::unique_ptr<mediapipe_custom::ImageFrame>)imageFrameWithError:(NSError **)error;

/**
 * Initializes an `MPPImage` object with the pixels from the given `mediapipe_custom::Image` and source
 * type and orientation from the given source image.
 *
 * Only supports initialization from `mediapipe_custom::Image` of format RGBA.
 ...
 *
 * @param image The `mediapipe_custom::Image` whose pixels are used for creating the `MPPImage`.
 ...
 * error will be saved.
 *
 * @return A new `MPImage` instance with the pixels from the given `mediapipe_custom::Image` and meta data
 * equal to the `sourceImage`. `nil` if there is an error in initializing the `MPPImage`.
 */
- (nullable instancetype)initWithCppImage:(const mediapipe_custom::Image &)image
