...
#include "mediapipe_custom/calculators/tensor/image_to_tensor_converter_gl_utils.h"
...
class ImageToTensorGlTextureConverter : public ImageToTensorConverter {
 public:
  absl::Status Init(CalculatorContext* cc, bool input_starts_at_bottom,
    ...
    return gl_helper_.RunInGlContext([this, input_starts_at_bottom,
      ...
      const std::string extract_sub_rect_vertex_src =
          absl::StrCat(mediapipe_custom::kMediaPipeVertexShaderPreamble,
      ...
      mediapipe_custom::GlhCreateProgram(extract_sub_rect_vertex_src.c_str(),
      ...
      glBufferData(GL_ARRAY_BUFFER, sizeof(mediapipe_custom::kBasicSquareVertices),
                   mediapipe_custom::kBasicSquareVertices, GL_STATIC_DRAW);

      // vbo 1
      glBindBuffer(GL_ARRAY_BUFFER, vbo_[1]);
      glBufferData(GL_ARRAY_BUFFER, sizeof(mediapipe_custom::kBasicTextureVertices),
  ...
  absl::Status Convert(const mediapipe_custom::Image& input, const RotatedRect& roi,
                       float range_min, float range_max,
                       int tensor_buffer_offset,
                       Tensor& output_tensor) override {
    if (input.format() != mediapipe_custom::GpuBufferFormat::kBGRA32 &&
        input.format() != mediapipe_custom::GpuBufferFormat::kRGBAHalf64 &&
        input.format() != mediapipe_custom::GpuBufferFormat::kRGBAFloat128 &&
        input.format() != mediapipe_custom::GpuBufferFormat::kRGB24) {
  ...
  absl::Status ExtractSubRect(const mediapipe_custom::GlTexture& texture,
    ...
    const auto gl_context = mediapipe_custom::GlContext::GetCurrent();
    ABSL_LOG_IF(FATAL, !gl_context) << "GlContext is not bound to the thread.";
    if (gl_context->GetGlVersion() == mediapipe_custom::GlVersion::kGLES2) {
 ...
 private:
  ...
  mediapipe_custom::GlCalculatorHelper gl_helper_;
