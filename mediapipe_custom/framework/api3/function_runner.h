...
class FunctionRunner<OutputPacketT, std::tuple<InputPacketTs...>>
 ...
 public:
  ...
  absl::StatusOr<OutputPacketT> Run(InputPacketTs... inputs) {
    mediapipe_custom::Timestamp timestamp = this->NextTimestamp();
    ...
    MP_ASSIGN_OR_RETURN(mediapipe_custom::Packet packet,
...
class FunctionRunnerBuilder {
 public:
  ...
  FunctionRunnerBuilder& SetService(const GraphService<T>& service,
    ...
    services_[&service] =
        mediapipe_custom::MakePacket<std::shared_ptr<T>>(std::move(object));
    return *this;
  }
 ...
 private:
  ...
  absl::flat_hash_map<const GraphServiceBase*, mediapipe_custom::Packet> services_;
