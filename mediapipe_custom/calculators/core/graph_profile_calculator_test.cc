...
class SleepCalculator : public CalculatorBase {
 public:
  ...
  std::shared_ptr<::mediapipe_custom::Clock> clock_ = nullptr;
};
 ...
 protected:
  ...
  void RunGraph(const std::vector<std::vector<Packet>>& input_sets,
    ...
    MP_ASSERT_OK(graph.StartRun({
        {"sync_clock",
         Adopt(new std::shared_ptr<::mediapipe_custom::Clock>(executor->GetClock()))},
    }));
...
TEST_F(GraphProfileCalculatorTest, GraphProfile) {
  ...
  GraphProfile expected_profile =
      mediapipe_custom::ParseTextProtoOrDie<GraphProfile>(R"pb(
        calculator_profiles {
  ...
  EXPECT_THAT(output_packets[1].Get<GraphProfile>(),
              mediapipe_custom::EqualsProto(expected_profile));
}
