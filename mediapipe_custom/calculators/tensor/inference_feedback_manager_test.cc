...
using ::mediapipe_custom::tool::AddVectorSink;
...
TEST_F(InferenceFeedbackManagerTest, ShouldRunE2ESmokeTest) {
  ...
  for (int n = 0; n < 3; ++n) {
    ...
    MP_ASSERT_OK(graph.AddPacketToInputStream(
        "regular_int_input",
        mediapipe_custom::MakePacket<Tensor>(std::move(input_tensor))
            .At(Timestamp(n))));
  }
...
TEST_F(InferenceFeedbackManagerTest,
  ...
  for (int n = 0; n < 3; ++n) {
    ...
    MP_ASSERT_OK(graph.AddPacketToInputStream(
        "regular_int_input",
        mediapipe_custom::MakePacket<Tensor>(std::move(regular_int_input_tensor))
            .At(Timestamp(n))));
    Tensor feedback_int_input_tensor =
        CreateSingleIntTensor(kFeedbackInputTensorValues[n]);
    MP_ASSERT_OK(graph.AddPacketToInputStream(
        "feedback_int_input",
        mediapipe_custom::MakePacket<Tensor>(std::move(feedback_int_input_tensor))
            .At(Timestamp(n))));
  }
