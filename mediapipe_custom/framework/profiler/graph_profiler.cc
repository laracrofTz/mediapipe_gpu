...
#include "mediapipe_custom/framework/port/advanced_proto_lite_inc.h"
...
#include "mediapipe_custom/framework/port/re2.h"
#include "mediapipe_custom/framework/port/ret_check.h"
#include "mediapipe_custom/framework/port/status.h"
#include "mediapipe_custom/framework/profiler/profiler_resource_util.h"
...
GraphProfiler::GraphProfiler()
  ...
  clock_ = std::shared_ptr<mediapipe_custom::Clock>(
...
void GraphProfiler::SetClock(const std::shared_ptr<mediapipe_custom::Clock>& clock) {
  absl::WriterMutexLock lock(&profiler_mutex_);
  ABSL_CHECK(clock) << "GraphProfiler::SetClock() is called with a nullptr.";
  clock_ = clock;
}

const std::shared_ptr<mediapipe_custom::Clock> GraphProfiler::GetClock() const {
  return clock_;
}
...
absl::Status GraphProfiler::Start(mediapipe_custom::Executor* executor) {
...
std::unique_ptr<GlProfilingHelper> GraphProfiler::CreateGlProfilingHelper() {
  ...
  return absl::make_unique<mediapipe_custom::GlProfilingHelper>(shared_from_this());
}
