...
class SetAlphaCalculator : public CalculatorBase {
 ...
 private:
  ...
  mediapipe_custom::SetAlphaCalculatorOptions options_;
...
absl::Status SetAlphaCalculator::GetContract(CalculatorContract* cc) {
  ...
  if (cc->Inputs().HasTag(kInputAlphaTagGpu)) {
    cc->Inputs().Tag(kInputAlphaTagGpu).Set<mediapipe_custom::GpuBuffer>();
    use_gpu |= true;
  }
#endif  // !MEDIAPIPE_DISABLE_GPU
...
absl::Status SetAlphaCalculator::Open(CalculatorContext* cc) {
  ...
  options_ = cc->Options<mediapipe_custom::SetAlphaCalculatorOptions>();
...
absl::Status SetAlphaCalculator::RenderGpu(CalculatorContext* cc) {
  ...
  if (!(input_frame.format() == mediapipe_custom::GpuBufferFormat::kBGRA32 ||
        input_frame.format() == mediapipe_custom::GpuBufferFormat::kRGB24)) {
  ...
  auto output_texture = gpu_helper_.CreateDestinationTexture(
      width, height, mediapipe_custom::GpuBufferFormat::kBGRA32);
  ...
  if (has_alpha_mask) {
    const auto& alpha_mask =
        cc->Inputs().Tag(kInputAlphaTagGpu).Get<mediapipe_custom::GpuBuffer>();
...
absl::Status SetAlphaCalculator::GlSetup(CalculatorContext* cc) {
  ...
  mediapipe_custom::GlhCreateProgram(mediapipe_custom::kBasicVertexShader, frag_src,
