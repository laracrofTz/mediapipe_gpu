...
#include "mediapipe_custom/calculators/image/image_cropping_calculator.pb.h"
#include "mediapipe_custom/framework/calculator_framework.h"
#include "mediapipe_custom/framework/calculator_runner.h"
#include "mediapipe_custom/framework/calculator_state.h"
...
#include "mediapipe_custom/framework/tool/tag_map_helper.h"
...
std::unique_ptr<mediapipe_custom::ImageFrame> GetInputFrame(int width, int height,
                                                     int channel) {
  const int total_size = width * height * channel;

  auto image_format = channel == 4 ? mediapipe_custom::ImageFormat::SRGBA
                                   : mediapipe_custom::ImageFormat::SRGB;

  auto input_frame = std::make_unique<mediapipe_custom::ImageFrame>(
...
TEST(ImageCroppingCalculatorTest, IdentityFunctionCropWithOriginalSize) {
  auto calculator_node =
      ParseTextProtoOrDie<mediapipe_custom::CalculatorGraphConfig::Node>(
          absl::Substitute(
              R"pb(
  ...
  const auto& output_image =
      outputs.Tag("IMAGE").packets[0].Get<mediapipe_custom::ImageFrame>();
...
TEST(ImageCroppingCalculatorTest, IdentityFunctionCropWithOriginalSizeGPU) {
  mediapipe_custom::CalculatorGraphConfig config =
      ParseTextProtoOrDie<mediapipe_custom::CalculatorGraphConfig>(absl::Substitute(
          R"pb(
  ...
  MP_ASSERT_OK(graph.AddPacketToInputStream(
      "input_frames", input_frame_packet.At(mediapipe_custom::Timestamp(1))));
