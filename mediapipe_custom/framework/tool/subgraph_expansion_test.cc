...
#include "mediapipe_custom/framework/tool/node_chain_subgraph.pb.h"
...
class NodeChainSubgraph : public Subgraph {
 public:
  absl::StatusOr<CalculatorGraphConfig> GetConfig(
      const SubgraphOptions& options) override {
    auto opts =
        Subgraph::GetOptions<mediapipe_custom::NodeChainSubgraphOptions>(options);
...
TEST(SubgraphExpansionTest, TransformStreamNames) {
  ...
  EXPECT_THAT(config, mediapipe_custom::EqualsProto(expected_config));
}
...
TEST(SubgraphExpansionTest, ConnectSubgraphStreams) {
  ...
  EXPECT_THAT(subgraph, mediapipe_custom::EqualsProto(expected_subgraph));
}
...
TEST(SubgraphExpansionTest, ExpandSubgraphs) {
  ...
  EXPECT_THAT(supergraph, mediapipe_custom::EqualsProto(expected_graph));
}
...
TEST(SubgraphExpansionTest, ExecutorFieldOfNodeInSubgraphPreserved) {
  ...
  CalculatorGraphConfig expected_graph = mediapipe_custom::ParseTextProtoOrDie<
...
const mediapipe_custom::GraphService<std::string> kStringTestService{
    "mediapipe_custom::StringTestService"};
...
TEST(SubgraphExpansionTest, SubgraphOptionsUsage) {
  ...
  auto graph_options =
      mediapipe_custom::ParseTextProtoOrDie<Subgraph::SubgraphOptions>(R"pb(
        options {
  ...
  EXPECT_THAT(subgraph, mediapipe_custom::EqualsProto(expected_graph));
}
...
TEST(SubgraphExpansionTest, SimpleSubgraphOptionsUsage) {
  ...
  EXPECT_THAT(moon_subgraph, mediapipe_custom::EqualsProto(expected_graph));
}
...
TEST(SubgraphExpansionTest, SimpleSubgraphOptionsTwice) {
  ...
  auto sky_graph = mediapipe_custom::ParseTextProtoOrDie<CalculatorGraphConfig>(R"pb(
    graph_options: {
      [type.googleapis.com/mediapipe.NodeChainSubgraphOptions] {}
    }
    node: {
  ...
  auto expected_node_options =
      mediapipe_custom::ParseTextProtoOrDie<mediapipe_custom::NodeChainSubgraphOptions>(
          "chain_length: 3");
  mediapipe_custom::NodeChainSubgraphOptions node_options;
  sky_graph.node(0).node_options(0).UnpackTo(&node_options);
  ASSERT_THAT(node_options, mediapipe_custom::EqualsProto(expected_node_options));
  ...
  EXPECT_THAT(expanded_1, mediapipe_custom::EqualsProto(expected_graph));
  EXPECT_THAT(sky_graph, mediapipe_custom::EqualsProto(expected_graph));
}
...
class InternalExecutorSubgraph : public Subgraph {
 public:
  absl::StatusOr<CalculatorGraphConfig> GetConfig(
      const SubgraphOptions& options) override {
    return mediapipe_custom::ParseTextProtoOrDie<CalculatorGraphConfig>(R"pb(
      input_stream: "IN:foo"
