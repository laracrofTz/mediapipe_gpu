...
#include "mediapipe_custom/util/image_test_utils.h"
...
void RunTestWithInputImagePacket(
    ...
    const mediapipe_custom::NormalizedRect& roi, bool output_int_tensor,
  ...
  auto graph_config =
      mediapipe_custom::ParseTextProtoOrDie<CalculatorGraphConfig>(absl::Substitute(
          R"(
  ...
  MP_ASSERT_OK(graph.AddPacketToInputStream(
      "roi",
      MakePacket<mediapipe_custom::NormalizedRect>(std::move(roi)).At(Timestamp(0))));
...
mediapipe_custom::ImageFormat::Format GetImageFormat(int image_channels) {
  if (image_channels == 4) {
...
Packet MakeImagePacket(cv::Mat input) {
  mediapipe_custom::Image input_image(std::make_shared<mediapipe_custom::ImageFrame>(
      GetImageFormat(input.channels()), input.cols, input.rows, input.step,
      input.data, [](uint8_t*) {}));
  return MakePacket<mediapipe_custom::Image>(std::move(input_image)).At(Timestamp(0));
}
...
void RunTest(cv::Mat input, cv::Mat expected_result,
             ...
             const mediapipe_custom::NormalizedRect& roi) {
