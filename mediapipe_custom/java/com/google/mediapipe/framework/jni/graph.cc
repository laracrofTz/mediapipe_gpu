...
#include "mediapipe_custom/framework/tool/executor_util.h"
...
#include "mediapipe_custom/framework/profiler/graph_profiler.h"
#endif  // MEDIAPIPE_PROFILER_AVAILABLE
#if !MEDIAPIPE_DISABLE_GPU
#include "mediapipe_custom/gpu/egl_surface_holder.h"
...
class CallbackHandler {
 public:
  ...
  void PacketCallback(const Packet& packet) {
    context_->CallbackToJava(mediapipe_custom::java::GetJNIEnv(), java_callback_,
                             packet);
  }
...
int64_t Graph::AddSurfaceOutput(const std::string& output_stream_name) {
  ...
  sink_node->set_name(mediapipe_custom::tool::GetUnusedNodeName(
  ...
  const std::string input_side_packet_name =
      mediapipe_custom::tool::GetUnusedSidePacketName(
  ...
  auto it_inserted = output_surface_side_packets_.emplace(
      ...
      AdoptAsUniquePtr(new mediapipe_custom::EglSurfaceHolder()));
...
absl::Status Graph::LoadBinaryGraph(std::string path_to_graph) {
  ...
  absl::Status status =
      mediapipe_custom::file::GetContents(path_to_graph, &graph_config_string);
  if (!status.ok()) {
    return status;
  }
...
void Graph::CallbackToJava(JNIEnv* env, jobject java_callback_obj,
  ...
  std::string packet_class_name = class_registry.GetClassName(
      mediapipe_custom::android::ClassRegistry::kPacketClassName);
  std::string process_method_name = class_registry.GetMethodName(
      mediapipe_custom::android::ClassRegistry::kPacketCallbackClassName, "process");
...
void Graph::CallbackToJava(JNIEnv* env, jobject java_callback_obj,
  ...
  std::string packet_class_name = class_registry.GetClassName(
      mediapipe_custom::android::ClassRegistry::kPacketClassName);
  std::string process_method_name = class_registry.GetMethodName(
      mediapipe_custom::android::ClassRegistry::kPacketWithHeaderCallbackClassName,
      "process");
...
#if !MEDIAPIPE_DISABLE_GPU
mediapipe_custom::GpuResources* Graph::GetGpuResources() const {
...
absl::Status Graph::SetParentGlContext(int64_t java_gl_context) {
  ...
  MP_ASSIGN_OR_RETURN(gpu_resources_,
                      mediapipe_custom::GpuResources::Create(
