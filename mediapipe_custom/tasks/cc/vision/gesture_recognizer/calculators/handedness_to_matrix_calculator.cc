...
#include "mediapipe_custom/tasks/cc/vision/gesture_recognizer/handedness_util.h"
...
using ::mediapipe_custom::tasks::vision::gesture_recognizer::GetRightHandScore;
...
class HandednessToMatrixCalculator : public CalculatorBase {
 public:
  static absl::Status GetContract(CalculatorContract* cc) {
    cc->Inputs().Tag(kHandednessTag).Set<mediapipe_custom::ClassificationList>();
...
absl::Status HandednessToMatrixCalculator::Process(CalculatorContext* cc) {
  ...
  auto handedness =
      cc->Inputs().Tag(kHandednessTag).Get<mediapipe_custom::ClassificationList>();
