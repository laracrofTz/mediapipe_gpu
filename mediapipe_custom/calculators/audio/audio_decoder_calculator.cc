...
#include "mediapipe_custom/util/audio_decoder.h"
...
absl::Status AudioDecoderCalculator::GetContract(CalculatorContract* cc) {
  ...
  if (cc->InputSidePackets().HasTag("OPTIONS")) {
    cc->InputSidePackets().Tag("OPTIONS").Set<mediapipe_custom::AudioDecoderOptions>();
  }
...
absl::Status AudioDecoderCalculator::Open(CalculatorContext* cc) {
  ...
  const auto& decoder_options =
      tool::RetrieveOptions(cc->Options<mediapipe_custom::AudioDecoderOptions>(),
  ...
  std::unique_ptr<mediapipe_custom::TimeSeriesHeader> header =
      absl::make_unique<mediapipe_custom::TimeSeriesHeader>();
