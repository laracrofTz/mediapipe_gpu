...
#include "mediapipe_custom/calculators/core/get_vector_item_calculator.pb.h"
#include "mediapipe_custom/calculators/util/flat_color_image_calculator.pb.h"
#include "mediapipe_custom/calculators/util/landmarks_to_render_data_calculator.h"
...
#include "mediapipe_custom/framework/port/opencv_imgcodecs_inc.h"
#include "mediapipe_custom/framework/port/opencv_imgproc_inc.h"
#include "mediapipe_custom/tasks/cc/core/model_task_graph.h"
#include "mediapipe_custom/tasks/cc/vision/face_detector/proto/face_detector_graph_options.pb.h"
#include "mediapipe_custom/tasks/cc/vision/face_landmarker/face_landmarks_connections.h"
#include "mediapipe_custom/tasks/cc/vision/face_landmarker/proto/face_landmarker_graph_options.pb.h"
#include "mediapipe_custom/tasks/cc/vision/image_generator/proto/conditioned_image_graph_options.pb.h"
#include "mediapipe_custom/tasks/cc/vision/image_segmenter/proto/image_segmenter_graph_options.pb.h"
#include "mediapipe_custom/util/color.pb.h"
#include "mediapipe_custom/util/image_frame_util.h"
...
class DepthImagePostprocessingCalculator : public api2::Node {
 public:
  ...
  absl::Status Process(CalculatorContext* cc) final {
    ...
    cv::Mat raw_depth_mat = mediapipe_custom::formats::MatView(
    ...
    ImageFrameSharedPtr depth_image_frame_ptr = std::make_shared<ImageFrame>(
        mediapipe_custom::ImageFormat::SRGB, depth_mat.cols, depth_mat.rows,
...
// moved to next line.
// clang-format off
MEDIAPIPE_REGISTER_NODE(::mediapipe_custom::tasks::vision::image_generator::internal::DepthImagePostprocessingCalculator);
// clang-format on
// NOLINTEND
 ...
 public:
  ...
  absl::Status Process(CalculatorContext* cc) final {
    ...
    cv::Mat input_image_mat =
        mediapipe_custom::formats::MatView(input_image.GetImageFrameSharedPtr().get());
    ...
    ImageFrameSharedPtr edges_image_frame_ptr = std::make_shared<ImageFrame>(
        mediapipe_custom::ImageFormat::SRGB, edges_mat.cols, edges_mat.rows,
...
// moved to next line.
// clang-format off
MEDIAPIPE_REGISTER_NODE(::mediapipe_custom::tasks::vision::image_generator::internal::CannyEdgeCalculator);
// clang-format on
// NOLINTEND
...
mediapipe_custom::Color GetColor(ColorType color_type) {
  mediapipe_custom::Color color;
  switch (color_type) {
    case WHITE:
      color.set_b(255);
      color.set_g(255);
...
// connections.
mediapipe_custom::LandmarksToRenderDataCalculatorOptions
GetFaceLandmarksRenderDataOptions(
    absl::Span<const std::array<int, 2>> connections, ColorType color_type) {
  mediapipe_custom::LandmarksToRenderDataCalculatorOptions render_options;
...
Source<mediapipe_custom::RenderData> GetFaceLandmarksRenderData(
    Source<mediapipe_custom::NormalizedLandmarkList> face_landmarks,
    const mediapipe_custom::LandmarksToRenderDataCalculatorOptions&
  ...
  return landmarks_to_render_data.Out(kRenderDataTag)
      .Cast<mediapipe_custom::RenderData>();
}
...
absl::StatusOr<Source<Image>> GetFaceLandmarksImage(
  ...
  auto face_landmarks_lists =
      face_landmarker_graph.Out(kNormLandmarksTag)
          .Cast<std::vector<mediapipe_custom::NormalizedLandmarkList>>();

  // Get the single face landmarks.
  auto& get_vector_item =
      graph.AddNode("GetNormalizedLandmarkListVectorItemCalculator");
  get_vector_item.GetOptions<mediapipe_custom::GetVectorItemCalculatorOptions>()
      .set_item_index(0);
  face_landmarks_lists >> get_vector_item.In(kVectorTag);
  auto single_face_landmarks =
      get_vector_item.Out(kItemTag).Cast<mediapipe_custom::NormalizedLandmarkList>();
  ...
  flat_color.GetOptions<mediapipe_custom::FlatColorImageCalculatorOptions>()
      .mutable_color()
      ->set_r(0);
...
REGISTER_MEDIAPIPE_GRAPH(
    ::mediapipe_custom::tasks::vision::image_generator::ConditionedImageGraph);
