...
#include "mediapipe_custom/framework/deps/no_destructor.h"
...
TEST(CalculatorTest, NodeMethods) {
  mediapipe_custom::CalculatorGraphConfig config =
      ParseTextProtoOrDie<mediapipe_custom::CalculatorGraphConfig>(Proto3GraphStr());
...
TEST(CalculatorContextTest, CanLoadResourcesThroughCalculatorContext) {
  mediapipe_custom::CalculatorGraphConfig config =
      ParseTextProtoOrDie<mediapipe_custom::CalculatorGraphConfig>(R"pb(
        node {
          calculator: "NightLightCalculator"
          input_side_packet: "input_value"
          output_stream: "values"
