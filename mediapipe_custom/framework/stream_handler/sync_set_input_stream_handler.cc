...
#include "mediapipe_custom/framework/stream_handler/sync_set_input_stream_handler.h"
...
#include "mediapipe_custom/framework/stream_handler/sync_set_input_stream_handler.pb.h"
...
void SyncSetInputStreamHandler::PrepareForRun(
  ...
  const auto& handler_options =
      options_.GetExtension(mediapipe_custom::SyncSetInputStreamHandlerOptions::ext);
  {
    ...
    for (const auto& sync_set : handler_options.sync_set()) {
      ...
      for (const auto& tag_index : sync_set.tag_index()) {
        ...
        ABSL_CHECK(!mediapipe_custom::ContainsKey(used_ids, id))
    ...
    for (CollectionItemId id = input_stream_managers_.BeginId();
      ...
      if (!mediapipe_custom::ContainsKey(used_ids, id)) {
