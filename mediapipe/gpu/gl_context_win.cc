...
// ANGLE has the same API as EGL, so we can use the same code (?) not sure yet until I test
// #include <thread> //window thread library
// #include <Windows.h> //windows native threads api, case insensitive
#include <memory>
#include <utility>
#include <vector>

#include <EGL/egl.h> // new adds
...
// DWORD is not a c++ type, its defined in Windows.h as unsinged int 32bits
// static DWORD egl_release_tls_index = FLS_OUT_OF_INDEXES;
...
static void EglThreadExitCallback(void* key_value) {
  ...
  if (current_display != EGL_NO_DISPLAY) {
    ...
    //                EGL_NO_CONTEXT); // releasing current context here
    eglMakeCurrent(GetCustomDisplay(), EGL_NO_SURFACE, EGL_NO_SURFACE,
                EGL_NO_CONTEXT); 
  }
  eglReleaseThread();
}
...
//   egl_release_tls_index = FlsAlloc(EglThreadExitCallback);
//   if (egl_release_tls_index == FLS_OUT_OF_INDEXES) {
//     ABSL_LOG(ERROR) << "cannot create FLS key for EGL cleanup.";
//   }
// }
