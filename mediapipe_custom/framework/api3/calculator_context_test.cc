...
#include "mediapipe_custom/framework/api3/port_test_nodes.h"
...
class FooCheckPortsAccessCalculator : public FooApi1Base {
 public:
  ...
  absl::Status Process(mediapipe_custom::CalculatorContext* cc) final {
...
TEST(PortContextTest, CanReadInputWriteOutputPorts) {
  ...
  mediapipe_custom::Packet out;
  MP_ASSERT_OK(
      graph.ObserveOutputStream("out", [&out](const mediapipe_custom::Packet& p) {
        out = p;
        return absl::OkStatus();
      }));
  ...
  MP_ASSERT_OK(graph.StartRun(
      {{"side_in", mediapipe_custom::MakePacket<std::string>("foo_input_side")}}));
  MP_ASSERT_OK(graph.AddPacketToInputStream(
      "in", mediapipe_custom::MakePacket<int>(21).At(Timestamp(0))));
  MP_ASSERT_OK(graph.WaitUntilIdle());

  // Verifying outputs.
  MP_ASSERT_OK_AND_ASSIGN(mediapipe_custom::Packet side_out,
...
TEST(PortContextTest, CanReadInputWriteOutputRepeatedPortsWhenManySpecified) {
  ...
  CalculatorGraph graph;
  MP_ASSERT_OK(graph.Initialize(std::move(config)));
  mediapipe_custom::Packet out0;
  MP_ASSERT_OK(
      graph.ObserveOutputStream("out0", [&out0](const mediapipe_custom::Packet& p) {
        out0 = p;
        return absl::OkStatus();
      }));
  mediapipe_custom::Packet out1;
  MP_ASSERT_OK(
      graph.ObserveOutputStream("out1", [&out1](const mediapipe_custom::Packet& p) {
        out1 = p;
        return absl::OkStatus();
      }));
  mediapipe_custom::Packet out2;
  MP_ASSERT_OK(
      graph.ObserveOutputStream("out2", [&out2](const mediapipe_custom::Packet& p) {
        out2 = p;
        return absl::OkStatus();
      }));
  ...
  MP_ASSERT_OK(graph.StartRun(
      {{"side_in0", mediapipe_custom::MakePacket<std::string>("foo_input_side0")},
       {"side_in1", mediapipe_custom::MakePacket<std::string>("foo_input_side1")},
       {"side_in2", mediapipe_custom::MakePacket<std::string>("foo_input_side2")}}));

  MP_ASSERT_OK(graph.AddPacketToInputStream(
      "in0", mediapipe_custom::MakePacket<int>(21).At(Timestamp(0))));
  MP_ASSERT_OK(graph.AddPacketToInputStream(
      "in1", mediapipe_custom::MakePacket<int>(22).At(Timestamp(0))));

  MP_ASSERT_OK(graph.WaitUntilIdle());

  // Verifying outputs.
  mediapipe_custom::Packet side_out;
