...
#include "mediapipe_custom/calculators/image/recolor_calculator.pb.h"
...
class RecolorCalculator : public CalculatorBase {
 ...
 private:
  ...
  mediapipe_custom::RecolorCalculatorOptions::MaskChannel mask_channel_;
...
absl::Status RecolorCalculator::GetContract(CalculatorContract* cc) {
  ...
  if (cc->Inputs().HasTag(kMaskGpuTag)) {
    cc->Inputs().Tag(kMaskGpuTag).Set<mediapipe_custom::GpuBuffer>();
    use_gpu |= true;
  }
#endif  // !MEDIAPIPE_DISABLE_GPU
...
absl::Status RecolorCalculator::RenderCpu(CalculatorContext* cc) {
  ...
  if (mask_mat.channels() > 1) {
    ...
    if (mask_channel_ == mediapipe_custom::RecolorCalculatorOptions_MaskChannel_ALPHA)
      mask_mat = channels[3];
    else
      mask_mat = channels[0];
  }
  ...
  cv::Mat output_mat = mediapipe_custom::formats::MatView(output_img.get());
...
absl::Status RecolorCalculator::RenderGpu(CalculatorContext* cc) {
  ...
  const auto& input_buffer = input_packet.Get<mediapipe_custom::GpuBuffer>();
  const auto& mask_buffer = mask_packet.Get<mediapipe_custom::GpuBuffer>();
  ...
  auto output = dst_tex.GetFrame<mediapipe_custom::GpuBuffer>();
...
absl::Status RecolorCalculator::LoadOptions(CalculatorContext* cc) {
  const auto& options = cc->Options<mediapipe_custom::RecolorCalculatorOptions>();
...
absl::Status RecolorCalculator::InitGpu(CalculatorContext* cc) {
  ...
  switch (mask_channel_) {
    case mediapipe_custom::RecolorCalculatorOptions_MaskChannel_UNKNOWN:
    case mediapipe_custom::RecolorCalculatorOptions_MaskChannel_RED:
      mask_component = "r";
      break;
    case mediapipe_custom::RecolorCalculatorOptions_MaskChannel_ALPHA:
      mask_component = "a";
      break;
  }
