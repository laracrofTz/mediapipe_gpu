...
#include "mediapipe_custom/tasks/cc/vision/image_generator/diffuser/diffuser_gpu.h"
...
//   PLUGIN_TENSORS - std::vector<mediapipe_custom::Tensor> @Optional
...
//
// Outputs:
//   IMAGE - mediapipe_custom::ImageFrame
...
class StableDiffusionIterateCalculator : public Node {
 public:
  ...
  static constexpr Output<mediapipe_custom::ImageFrame> kImageOut{"IMAGE"};
...
absl::Status StableDiffusionIterateCalculator::Open(CalculatorContext* cc) {
  ...
  MP_RETURN_IF_ERROR(mediapipe_custom::file::Exists(config.model_dir))
      << config.model_dir;
...
absl::Status StableDiffusionIterateCalculator::Process(CalculatorContext* cc) {
  ...
  if (kIterationIn(cc).IsEmpty()) {
    ...
    if (show_result) {
      ...
      kImageOut(cc).Send(Packet<mediapipe_custom::ImageFrame>());
    }
  }
  return absl::OkStatus();
}
