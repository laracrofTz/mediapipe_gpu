...
#include "mediapipe_custom/tasks/cc/components/processors/detection_postprocessing_graph.h"
...
#include "mediapipe_custom/calculators/util/detection_label_id_to_text_calculator.pb.h"
#include "mediapipe_custom/calculators/util/non_max_suppression_calculator.pb.h"
...
#include "mediapipe_custom/framework/formats/object_detection/anchor.pb.h"
#include "mediapipe_custom/framework/formats/tensor.h"
#include "mediapipe_custom/tasks/cc/common.h"
#include "mediapipe_custom/tasks/cc/components/calculators/score_calibration_calculator.pb.h"
#include "mediapipe_custom/tasks/cc/components/calculators/score_calibration_utils.h"
#include "mediapipe_custom/tasks/cc/components/processors/proto/detection_postprocessing_graph_options.pb.h"
#include "mediapipe_custom/tasks/cc/components/processors/proto/detector_options.pb.h"
#include "mediapipe_custom/tasks/cc/core/model_resources.h"
#include "mediapipe_custom/tasks/cc/core/utils.h"
...
#include "mediapipe_custom/tasks/metadata/object_detector_metadata_schema_generated.h"
...
using TensorsSource =
    mediapipe_custom::api2::builder::Source<std::vector<mediapipe_custom::Tensor>>;
...
absl::Status ConfigureOutModelNmsTensorsToDetectionsCalculator(
  ...
  options->set_box_format(
      mediapipe_custom::TensorsToDetectionsCalculatorOptions::YXHW);
  options->set_min_score_thresh(specs.score_threshold);
  if (specs.is_allowlist) {
    options->mutable_allow_classes()->Assign(
        specs.allow_or_deny_categories.begin(),
...
void ConfigureNonMaxSuppressionCalculator(
    ...
    mediapipe_custom::NonMaxSuppressionCalculatorOptions* options) {
  options->set_min_suppression_threshold(
      detector_options.min_suppression_threshold());
  options->set_overlap_type(
      mediapipe_custom::NonMaxSuppressionCalculatorOptions::INTERSECTION_OVER_UNION);
  options->set_algorithm(
      mediapipe_custom::NonMaxSuppressionCalculatorOptions::DEFAULT);
...
void ConfigureDetectionLabelIdToTextCalculator(
    PostProcessingSpecs& specs,
    mediapipe_custom::DetectionLabelIdToTextCalculatorOptions* options) {
...
class DetectionPostprocessingGraph : public mediapipe_custom::Subgraph {
 public:
 ...
 private:
  ...
  // tensors (std::vector<mediapipe_custom::Tensor>) as input and returns one output
  // stream:
  //  - Detection results as a std::vector<Detection>.
  //
  // graph_options: the on-device DetectionPostprocessingGraphOptions.
  // tensors_in: (std::vector<mediapipe_custom::Tensor>>) tensors to postprocess.
  ...
  BuildDetectionPostprocessing(
    ...
    if (!graph_options.has_non_max_suppression_options()) {
      ...
      tensors_to_detections
          .GetOptions<mediapipe_custom::TensorsToDetectionsCalculatorOptions>()
      ...
      ssd_anchor.GetOptions<mediapipe_custom::SsdAnchorsCalculatorOptions>().Swap(
          graph_options.mutable_ssd_anchors_options());
      auto anchors =
          ssd_anchor.SideOut("").Cast<std::vector<mediapipe_custom::Anchor>>();
      ...
      non_maximum_suppression
          .GetOptions<mediapipe_custom::NonMaxSuppressionCalculatorOptions>()
    ...
    detection_label_id_to_text
        .GetOptions<mediapipe_custom::DetectionLabelIdToTextCalculatorOptions>()
...
REGISTER_MEDIAPIPE_GRAPH(
  ::mediapipe_custom::tasks::components::processors::DetectionPostprocessingGraph); // NOLINT
// clang-format on
