...
#include "mediapipe_custom/framework/graph_test_base.h"
...
#include "mediapipe_custom/framework/port/status_matchers.h"  // NOLINT
...
using mediapipe_custom::Detection;
using mediapipe_custom::InferenceCalculatorOptions_Delegate;
...
class InferenceCalculatorTest : public testing::TestWithParam<Param> {
 protected:
  void SetDelegateForParam(mediapipe_custom::CalculatorGraphConfig_Node* node) {
    auto options_map = tool::MutableOptionsMap().Initialize(*node);
    auto options = options_map.Get<mediapipe_custom::InferenceCalculatorOptions>();
...
TEST_P(InferenceCalculatorTest, TestFaceDetection) {
  ...
  std::vector<mediapipe_custom::Packet> detection_packets;
  tool::AddVectorSink("detections", &config, &detection_packets);
  std::vector<mediapipe_custom::Packet> rendering_packets;
