...
void RunTest(const std::string& graph_text, const std::string& tag,
  ...
  auto graph_config = mediapipe_custom::ParseTextProtoOrDie<CalculatorGraphConfig>(
...
std::array<float, 16> GetMatrix(cv::Mat input, mediapipe_custom::NormalizedRect roi,
                                bool keep_aspect_ratio, int out_width,
                                int out_height) {
  std::array<float, 16> transform_mat;
  mediapipe_custom::RotatedRect roi_absolute =
      mediapipe_custom::GetRoi(input.cols, input.rows, roi);
  mediapipe_custom::PadRoi(out_width, out_height, keep_aspect_ratio, &roi_absolute)
      .IgnoreError();
  mediapipe_custom::GetRotatedSubRectToRectTransformMatrix(
...
TEST(WarpAffineCalculatorTest, MediumSubRectKeepAspect) {
  mediapipe_custom::NormalizedRect roi;
