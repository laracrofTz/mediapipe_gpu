...
absl::Status RunMPPGraph() {
  ...
  mediapipe_custom::CalculatorGraphConfig config =
      mediapipe_custom::ParseTextProtoOrDie<mediapipe_custom::CalculatorGraphConfig>(
          calculator_graph_config_contents);
  std::map<std::string, mediapipe_custom::Packet> input_side_packets;
  ...
  for (const std::string& kv_pair : kv_pairs) {
    ...
    input_side_packets[name_and_value[0]] =
        mediapipe_custom::MakePacket<std::string>(input_side_packet_contents);
  }

  mediapipe_custom::MatrixData inc3_pca_mean_matrix_data,
      inc3_pca_projection_matrix_data, vggish_pca_mean_matrix_data,
      vggish_pca_projection_matrix_data;
  mediapipe_custom::Matrix inc3_pca_mean_matrix, inc3_pca_projection_matrix,
  ...
  for (const std::string& kv_pair : kv_pairs) {
    std::vector<std::string> name_and_value = absl::StrSplit(kv_pair, '=');
    RET_CHECK(name_and_value.size() == 2);
    absl::StatusOr<mediapipe_custom::Packet> output_packet =
    ...
    MP_RETURN_IF_ERROR(
        mediapipe_custom::file::SetContents(name_and_value[1], serialized_string));
  }
