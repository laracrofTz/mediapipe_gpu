...
#include "mediapipe_custom/framework/packet_test.pb.h"
...
MEDIAPIPE_REGISTER_TYPE(mediapipe_custom::RegisteredPairStruct,
                        "::mediapipe_custom::RegisteredPairStruct", nullptr, nullptr);
...
TEST(PacketTest, TypeRegistrationDebugString) {
  ...
  EXPECT_EQ(packet.DebugString(),
            "mediapipe_custom::Packet with timestamp: Timestamp::Unset() and type: "
            "::mediapipe_custom::RegisteredPairStruct");
  ...
  std::string expected_type_name =
      (kHaveUnregisteredTypeNames)
          ? "mediapipe_custom::(anonymous namespace)::UnregisteredPairStruct"
          : "<unknown>";
...
TEST(PacketTest, ReturnGenericProtobufMessage) {
  std::unique_ptr<::mediapipe_custom::PacketTestProto> proto_ptr(
      new ::mediapipe_custom::PacketTestProto);
  proto_ptr->add_x(123);
  Packet packet = Adopt(static_cast<proto_ns::Message*>(proto_ptr.release()));
  EXPECT_EQ(123, dynamic_cast<const ::mediapipe_custom::PacketTestProto&>(
...
TEST(PacketTest, TryWrongProtobufMessageSubType) {
  ...
  EXPECT_FALSE(packet.ValidateAsType<::mediapipe_custom::SimpleProto>().ok());
  EXPECT_TRUE(packet.ValidateAsType<::mediapipe_custom::PacketTestProto>().ok());
}
...
TEST(PacketTest, GetProtoBase) {
  ...
  proto_ptr->add_x(123);
  Packet packet = Adopt(proto_ptr.release());
  ::mediapipe_custom::PacketTestProto proto_copy;
...
TEST(PacketTest, ValidateAsProtoMessageLite) {
  auto proto_ptr = absl::make_unique<::mediapipe_custom::PacketTestProto>();
  proto_ptr->add_x(123);
  Packet packet = Adopt(proto_ptr.release());
...
TEST(PacketTest, GetVectorOfProtos) {
  std::vector<mediapipe_custom::PacketTestProto> protos(2);
  protos[0].add_x(123);
  protos[1].add_x(456);
  // Normally we'd move here, but we copy to use the protos for comparison.
  const Packet packet =
      MakePacket<std::vector<mediapipe_custom::PacketTestProto>>(protos);
...
TEST(PacketTest, MessageHolderRegistration) {
  ...
  Packet packet = MakePacket<mediapipe_custom::SimpleProto>();
  ASSERT_EQ(mediapipe_custom::SimpleProto{}.GetTypeName(), "mediapipe.SimpleProto");
...
TEST(PacketTest, PacketFromSerializedProto) {
  mediapipe_custom::SimpleProto original;
  ...
  MP_EXPECT_OK(packet.ValidateAsType<::mediapipe_custom::SimpleProto>());
  EXPECT_FALSE(packet.ValidateAsType<::mediapipe_custom::PacketTestProto>().ok());
}
