...
#include "mediapipe_custom/calculators/image/scale_image_calculator.pb.h"
#include "mediapipe_custom/calculators/image/scale_image_utils.h"
...
#include "mediapipe_custom/framework/formats/yuv_image.h"
#include "mediapipe_custom/framework/port/image_resizer.h"
...
absl::Status ScaleImageCalculator::Open(CalculatorContext* cc) {
  ...
  cc->Outputs().Get(output_data_id_).SetOffset(mediapipe_custom::TimestampDiff(0));
...
absl::Status ScaleImageCalculator::Process(CalculatorContext* cc) {
  ...
  if (input_format_ == ImageFormat::YCBCR420P) {
    ...
    cv::Mat input_mat = ::mediapipe_custom::formats::MatView(image_frame);
    converted_image_frame.Reset(ImageFormat::SRGBA, image_frame->Width(),
                                image_frame->Height(), alignment_boundary_);
    cv::Mat output_mat = ::mediapipe_custom::formats::MatView(&converted_image_frame);
