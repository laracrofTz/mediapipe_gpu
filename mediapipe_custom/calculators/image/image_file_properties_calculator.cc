 ...
 public:
  static absl::Status GetContract(CalculatorContract* cc) {
    ...
    if (cc->Outputs().NumEntries() != 0) {
      ...
      cc->Outputs().Index(0).Set<::mediapipe_custom::ImageFileProperties>();
    } else {
      RET_CHECK(cc->OutputSidePackets().NumEntries() == 1);
      cc->OutputSidePackets().Index(0).Set<::mediapipe_custom::ImageFileProperties>();
    }
  ...
  absl::Status Process(CalculatorContext* cc) override {
    ...
    if (read_properties_) {
      if (cc->Outputs().NumEntries() == 1) {
        ...
        cc->OutputSidePackets().Index(0).Set(
            MakePacket<ImageFileProperties>(properties_)
                .At(mediapipe_custom::Timestamp::Unset()));
      }
    }
