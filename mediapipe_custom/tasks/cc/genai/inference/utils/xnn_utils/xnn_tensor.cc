...
#include "mediapipe_custom/tasks/cc/genai/inference/common/mdspan.h"
#include "mediapipe_custom/tasks/cc/genai/inference/utils/xnn_utils/utils.h"
#include "xnnpack.h"  // from @XNNPACK

namespace mediapipe_custom::tasks::genai {
namespace xnn_utils {
namespace {
...
absl::Status Tensor::DumpToFile(absl::string_view file_path) {
  return mediapipe_custom::file::SetContents(
...
absl::Status Tensor::LoadFromFile(absl::string_view file_path, bool use_mmap,
  ...
  tag = mediapipe_custom::file::Basename(file_path);
...
absl::StatusOr<::mediapipe_custom::Tensor> Tensor::ConvertToMediapipeTensor() {
  RET_CHECK_EQ(datatype, xnn_datatype_fp32) << "Try ConvertToF32 then convert";
  ::mediapipe_custom::Tensor mp_tensor(
      ::mediapipe_custom::Tensor::ElementType::kFloat32,
      ::mediapipe_custom::Tensor::Shape(std::vector<int>(dims.begin(), dims.end())));
...
absl::Status QCTensor::LoadFromFile(absl::string_view quantized_weight_filename,
  ...
  tag = mediapipe_custom::file::Basename(quantized_weight_filename);

  return absl::OkStatus();
}

absl::Status QCTensor::DumpToFile(absl::string_view file_path) {
  MP_RETURN_IF_ERROR(mediapipe_custom::file::SetContents(
      file_path,
