...
namespace mediapipe_custom {
using mediapipe_custom::CalculatorProfile;
using mediapipe_custom::GraphProfile;
using mediapipe_custom::GraphTrace;
...
class GraphProfilerStub {
 public:
  ...
  inline void SetClock(const std::shared_ptr<mediapipe_custom::Clock>& clock) {}
  ...
  inline void Resume() {}
  inline void Reset() {}
  inline absl::Status Start(mediapipe_custom::Executor* executor) {
    return absl::OkStatus();
  }
  ...
  const std::shared_ptr<mediapipe_custom::Clock> GetClock() const { return nullptr; }
};
