...
#include "mediapipe_custom/framework/graph_validation.h"
...
TEST(GraphValidationTest, InitializeTemplateFromProtos) {
  mediapipe_custom::tool::TemplateParser::Parser parser;
...
TEST(GraphValidationTest, OptionalInputNotProvidedForSubgraphCalculator) {
  ...
  side_packets.insert({"foo_in", mediapipe_custom::Adopt(new std::string("input"))});
