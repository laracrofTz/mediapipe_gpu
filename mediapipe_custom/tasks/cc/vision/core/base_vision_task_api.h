...
class BaseVisionTaskApi : public tasks::core::BaseTaskApi {
      ...
      if (node.calculator() == "ImageToTensorCalculator") {
        ...
        mediapipe_custom::ImageToTensorCalculatorOptions options =
            node.options().GetExtension(
                mediapipe_custom::ImageToTensorCalculatorOptions::ext);
 ...
 protected:
  ...
  static absl::StatusOr<mediapipe_custom::NormalizedRect> ConvertToNormalizedRect(
      std::optional<ImageProcessingOptions> options,
      const mediapipe_custom::Image& image, bool roi_allowed = true) {
    mediapipe_custom::NormalizedRect normalized_rect;
