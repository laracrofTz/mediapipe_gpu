...
struct HasProcessMethod<
    ...
    std::void_t<decltype(absl::Status(std::declval<std::decay_t<T>>().Process(
        std::declval<mediapipe_custom::CalculatorContext*>())))>> : std::true_type {};
...
struct TagTuple {
  ...
  static constexpr auto Make(std::index_sequence<I...> indices) {
    return std::make_tuple(mediapipe_custom::api2::internal::tag_build(S<I>{})...);
  }
...
class Contract {
 public:
  ...
  absl::Status GetContract(mediapipe_custom::CalculatorContract* cc) const {
