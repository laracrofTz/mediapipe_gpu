...
TEST(NestedCalculatorGraphTest, ExecutedNestedGraphWithInferenceCalculator) {
  ...
  MP_ASSERT_OK(graph.AddPacketToInputStream(
      "input", mediapipe_custom::MakePacket<Tensor>(std::move(input_tensor))
                   .At(mediapipe_custom::Timestamp(1))));
...
TEST(NestedCalculatorGraphTest, TestNestedGraphServiceInheriting) {
  ...
  const mediapipe_custom::Timestamp timestamp(1);
  MP_ASSERT_OK(graph.AddPacketToInputStream(
      "test_value", mediapipe_custom::MakePacket<int>(kTestValue).At(timestamp)));
  MP_ASSERT_OK(graph.CloseAllInputStreams());
  MP_ASSERT_OK(graph.WaitUntilDone());
...
TEST(NestedCalculatorGraphTest, NestedGraphsCannotRegisterNewServices) {
  ...
  MP_ASSERT_OK(graph.AddPacketToInputStream(
      "test_value",
      mediapipe_custom::MakePacket<int>(
...
TEST(NestedCalculatorGraphTest, NestedGraphWithGpuServiceRequestShouldFail) {
  ...
  MP_ASSERT_OK(graph.StartRun({}));
  MP_ASSERT_OK(graph.AddPacketToInputStream(
      "tick", mediapipe_custom::MakePacket<int>(0).At(mediapipe_custom::Timestamp(1))));
  MP_ASSERT_OK(graph.CloseAllInputStreams());
  MP_ASSERT_OK(graph.WaitUntilDone());
}
