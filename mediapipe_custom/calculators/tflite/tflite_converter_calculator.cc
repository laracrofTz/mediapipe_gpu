...
#include "mediapipe_custom/calculators/tflite/tflite_converter_calculator.pb.h"
...
absl::Status TfLiteConverterCalculator::ProcessCPU(CalculatorContext* cc) {
  if (cc->Inputs().HasTag(kImageFrameTag)) {
    ...
    const mediapipe_custom::ImageFormat::Format format = image_frame.Format();
    ...
    if (!initialized_) {
      if (!(format == mediapipe_custom::ImageFormat::SRGBA ||
            ...
            format == mediapipe_custom::ImageFormat::VEC32F1))
        RET_CHECK_FAIL() << "Unsupported CPU input format.";
      TfLiteQuantization quant;
      if (use_quantized_tensors_) {
        RET_CHECK(format != mediapipe_custom::ImageFormat::VEC32F1)
...
absl::Status TfLiteConverterCalculator::InitGpu(CalculatorContext* cc) {
  ...
  mediapipe_custom::ImageFormat::Format format =
      mediapipe_custom::ImageFormatForGpuBufferFormat(input.format());
  ...
  if (include_alpha && (format != mediapipe_custom::ImageFormat::SRGBA))
...
absl::Status TfLiteConverterCalculator::LoadOptions(CalculatorContext* cc) {
  ...
  const auto& options =
      cc->Options<::mediapipe_custom::TfLiteConverterCalculatorOptions>();
