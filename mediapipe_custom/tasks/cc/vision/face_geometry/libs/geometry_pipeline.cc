...
#include "mediapipe_custom/tasks/cc/vision/face_geometry/libs/mesh_3d_utils.h"
#include "mediapipe_custom/tasks/cc/vision/face_geometry/libs/procrustes_solver.h"
...
#include "mediapipe_custom/tasks/cc/vision/face_geometry/proto/mesh_3d.pb.h"
...
class ScreenToMetricSpaceConverter {
 public:
  ...
  absl::Status Convert(
      const mediapipe_custom::NormalizedLandmarkList& screen_landmark_list,  //
      const PerspectiveCameraFrustum& pcf,                            //
      mediapipe_custom::LandmarkList& metric_landmark_list,                  //
 ...
 private:
  ...
  static void ConvertLandmarkListToEigenMatrix(
      const mediapipe_custom::NormalizedLandmarkList& landmark_list,
  ...
  static void ConvertEigenMatrixToLandmarkList(
      ...
      mediapipe_custom::LandmarkList& landmark_list) {
    landmark_list.Clear();
...
class GeometryPipelineImpl : public GeometryPipeline {
 public:
  ...
  absl::StatusOr<std::vector<proto::FaceGeometry>> EstimateFaceGeometry(
      const std::vector<mediapipe_custom::NormalizedLandmarkList>&
    ...
    for (const mediapipe_custom::NormalizedLandmarkList& screen_face_landmarks :
         multi_face_landmarks) {
      ...
      mediapipe_custom::LandmarkList metric_face_landmarks;
      ...
      mediapipe_custom::MatrixDataProtoFromMatrix(
 ...
 private:
  static bool IsScreenLandmarkListTooCompact(
      const mediapipe_custom::NormalizedLandmarkList& screen_landmarks) {
    float mean_x = 0.f;
    float mean_y = 0.f;
