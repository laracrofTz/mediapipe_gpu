...
// specific type (T).
template <class T>
mediapipe_custom::api2::builder::GenericNode& AddSplitVectorNode(
  ...
  } else if constexpr (std::is_same_v<T, std::vector<mediapipe_custom::Detection>>) {
    return graph.AddNode("SplitDetectionVectorCalculator");
  } else if constexpr (std::is_same_v<
                           T, std::vector<mediapipe_custom::ClassificationList>>) {
    return graph.AddNode("SplitClassificationListVectorCalculator");
  } else if constexpr (std::is_same_v<T, mediapipe_custom::NormalizedLandmarkList>) {
  ...
  } else if constexpr (std::is_same_v<T, mediapipe_custom::JointList>) {
...
template <>
struct split_result_item<mediapipe_custom::NormalizedLandmarkList,
                         /*kIteratorContainsRanges=*/false> {
  using type = mediapipe_custom::NormalizedLandmark;
};
template <>
struct split_result_item<mediapipe_custom::LandmarkList,
                         /*kIteratorContainsRanges=*/false> {
  using type = mediapipe_custom::Landmark;
};
...
struct split_result_item<mediapipe_custom::NormalizedLandmarkList,
  ...
  using type = mediapipe_custom::NormalizedLandmarkList;
};
template <>
struct split_result_item<mediapipe_custom::LandmarkList,
                         /*kIteratorContainsRanges=*/true> {
  using type = mediapipe_custom::LandmarkList;
};
...
auto Split(Stream<CollectionT> items, I begin, I end,
  ...
  auto& splitter_opts =
      splitter.template GetOptions<mediapipe_custom::SplitVectorCalculatorOptions>();
