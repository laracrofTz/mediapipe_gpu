...
using mediapipe_custom::CalculatorBase;
using mediapipe_custom::CalculatorContext;
using mediapipe_custom::CalculatorContract;
using mediapipe_custom::CalculatorGraphConfig;
using mediapipe_custom::GetFromUniquePtr;
using mediapipe_custom::InputStreamShardSet;
using mediapipe_custom::MakePacket;
using mediapipe_custom::OutputStreamShardSet;
using mediapipe_custom::Timestamp;
namespace proto_ns = mediapipe_custom::proto_ns;

constexpr char kEventTag[] = "EVENT";
constexpr char kOutTag[] = "OUT";

using mediapipe_custom::CalculatorGraph;
using mediapipe_custom::Packet;

class InfiniteSequenceCalculator : public mediapipe_custom::CalculatorBase {
 public:
...
class StoppingPassThroughCalculator : public mediapipe_custom::CalculatorBase {
 public:
  ...
  absl::Status Process(CalculatorContext* cc) override {
    ...
    return (++count_ <= max_count_) ? absl::OkStatus()
                                    : mediapipe_custom::tool::StatusStop();
  }
  absl::Status Close(CalculatorContext* cc) override {
    cc->Outputs().Tag(kEventTag).AddPacket(MakePacket<int>(2).At(Timestamp(2)));
    return absl::OkStatus();
