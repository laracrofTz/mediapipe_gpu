...
#include "mediapipe_custom/framework/stream_handler/fixed_size_input_stream_handler.h"
...
#include "mediapipe_custom/framework/stream_handler/default_input_stream_handler.h"
...
FixedSizeInputStreamHandler::FixedSizeInputStreamHandler(
    ...
    const mediapipe_custom::MediaPipeOptions& options, bool calculator_run_in_parallel)
    : DefaultInputStreamHandler(std::move(tag_map), cc_manager, options,
                                calculator_run_in_parallel) {
  const auto& ext =
      options.GetExtension(mediapipe_custom::FixedSizeInputStreamHandlerOptions::ext);
