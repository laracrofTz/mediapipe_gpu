...
#include "mediapipe_custom/framework/formats/image_multi_pool.h"
...
ImageMultiPool::SimplePoolGpu ImageMultiPool::MakeSimplePoolGpu(
    IBufferSpec spec) {
  OSType cv_format = mediapipe_custom::CVPixelFormatForGpuBufferFormat(
      GpuBufferFormatForImageFormat(spec.format));
  ABSL_CHECK_NE(cv_format, -1) << "unsupported pixel format";
  return MakeCFHolderAdopting(mediapipe_custom::CreateCVPixelBufferPool(
...
Image ImageMultiPool::GetBufferFromSimplePool(
  ...
  OSType cv_format = mediapipe_custom::CVPixelFormatForGpuBufferFormat(
      mediapipe_custom::GpuBufferFormatForImageFormat(spec.format));
  ABSL_CHECK_NE(cv_format, -1) << "unsupported pixel format";
  CVPixelBufferRef buffer;
  CVReturn err = mediapipe_custom::CreateCVPixelBufferWithoutPool(
  ...
  static CFDictionaryRef auxAttributes =
      mediapipe_custom::CreateCVPixelBufferPoolAuxiliaryAttributesForThreshold(
          kKeepCount);
  CVReturn err = mediapipe_custom::CreateCVPixelBufferWithPool(
      *pool, auxAttributes,
      [this]() {
...
ImageMultiPool::SimplePoolGpu ImageMultiPool::MakeSimplePoolGpu(
    IBufferSpec spec) {
  return mediapipe_custom::GlTextureBufferPool::Create(
...
#if !MEDIAPIPE_DISABLE_GPU
#ifdef __APPLE__
void ImageMultiPool::RegisterTextureCache(mediapipe_custom::CVTextureCacheType cache) {
  absl::MutexLock lock(&mutex_gpu_);
...
void ImageMultiPool::UnregisterTextureCache(
    mediapipe_custom::CVTextureCacheType cache) {
  absl::MutexLock lock(&mutex_gpu_);
