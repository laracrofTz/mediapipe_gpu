...
#include "mediapipe_custom/framework/deps/mathutil.h"
...
#include "mediapipe_custom/framework/port/benchmark.h"
...
TEST(MathUtil, Round) {
  // test float rounding
  EXPECT_EQ(mediapipe_custom::MathUtil::FastIntRound(0.7f), 1);
  EXPECT_EQ(mediapipe_custom::MathUtil::FastIntRound(5.7f), 6);
  EXPECT_EQ(mediapipe_custom::MathUtil::FastIntRound(6.3f), 6);
  EXPECT_EQ(mediapipe_custom::MathUtil::FastIntRound(1000000.7f), 1000001);
  ...
  EXPECT_LT(rf, 0.5f);
  EXPECT_EQ(mediapipe_custom::MathUtil::Round<int>(rf), 0);

  // same test for double
  double rd = std::nextafter(0.5, 0.0);
  EXPECT_LT(rd, 0.5);
  EXPECT_EQ(mediapipe_custom::MathUtil::Round<int>(rd), 0);

  // same test for long double
  long double rl = std::nextafter(0.5l, 0.0l);
  EXPECT_LT(rl, 0.5l);
  EXPECT_EQ(mediapipe_custom::MathUtil::Round<int>(rl), 0);
}
...
static void BM_IntRound(benchmark::State& state) {
  ...
  int sum = 0;
  for (auto _ : state) {
    sum += mediapipe_custom::MathUtil::Round<int>(x);
    x += 0.1;
    sum += mediapipe_custom::MathUtil::Round<int>(x);
    x += 0.1;
    sum += mediapipe_custom::MathUtil::Round<int>(x);
...
static void BM_FastIntRound(benchmark::State& state) {
  ...
  int sum = 0;
  for (auto _ : state) {
    sum += mediapipe_custom::MathUtil::FastIntRound(x);
    x += 0.1;
    sum += mediapipe_custom::MathUtil::FastIntRound(x);
    x += 0.1;
    sum += mediapipe_custom::MathUtil::FastIntRound(x);
...
static void BM_Int64Round(benchmark::State& state) {
  ...
  int sum = 0;
  for (auto _ : state) {
    sum += mediapipe_custom::MathUtil::Round<int64_t>(x);
    x += 0.1;
    sum += mediapipe_custom::MathUtil::Round<int64_t>(x);
    x += 0.1;
    sum += mediapipe_custom::MathUtil::Round<int64_t>(x);
...
static void BM_UintRound(benchmark::State& state) {
  ...
  int sum = 0;
  for (auto _ : state) {
    sum += mediapipe_custom::MathUtil::Round<uint32_t>(x);
    x += 0.1;
    sum += mediapipe_custom::MathUtil::Round<uint32_t>(x);
    x += 0.1;
    sum += mediapipe_custom::MathUtil::Round<uint32_t>(x);
...
static void BM_SafeIntCast(benchmark::State& state) {
  ...
  int sum = 0;
  for (auto _ : state) {
    sum += mediapipe_custom::MathUtil::SafeCast<int>(x);
    x += 0.1;
    sum += mediapipe_custom::MathUtil::SafeCast<int>(x);
    x += 0.1;
    sum += mediapipe_custom::MathUtil::SafeCast<int>(x);
...
static void BM_SafeInt64Cast(benchmark::State& state) {
  ...
  int sum = 0;
  for (auto _ : state) {
    sum += mediapipe_custom::MathUtil::SafeCast<int64_t>(x);
    x += 0.1;
    sum += mediapipe_custom::MathUtil::SafeCast<int64_t>(x);
    x += 0.1;
    sum += mediapipe_custom::MathUtil::SafeCast<int64_t>(x);
...
static void BM_SafeIntRound(benchmark::State& state) {
  ...
  int sum = 0;
  for (auto _ : state) {
    sum += mediapipe_custom::MathUtil::SafeRound<int>(x);
    x += 0.1;
    sum += mediapipe_custom::MathUtil::SafeRound<int>(x);
    x += 0.1;
    sum += mediapipe_custom::MathUtil::SafeRound<int>(x);
...
static void BM_SafeInt64Round(benchmark::State& state) {
  ...
  int sum = 0;
  for (auto _ : state) {
    sum += mediapipe_custom::MathUtil::SafeRound<int64_t>(x);
    x += 0.1;
    sum += mediapipe_custom::MathUtil::SafeRound<int64_t>(x);
    x += 0.1;
    sum += mediapipe_custom::MathUtil::SafeRound<int64_t>(x);
...
TEST(MathUtil, IntRound) {
  EXPECT_EQ(mediapipe_custom::MathUtil::Round<int>(0.0), 0);
  EXPECT_EQ(mediapipe_custom::MathUtil::Round<int>(0.49), 0);
  EXPECT_EQ(mediapipe_custom::MathUtil::Round<int>(1.49), 1);
  EXPECT_EQ(mediapipe_custom::MathUtil::Round<int>(-0.49), 0);
  EXPECT_EQ(mediapipe_custom::MathUtil::Round<int>(-1.49), -1);

  // Either adjacent integer is an acceptable result.
  EXPECT_EQ(fabs(mediapipe_custom::MathUtil::Round<int>(0.5) - 0.5), 0.5);
  EXPECT_EQ(fabs(mediapipe_custom::MathUtil::Round<int>(1.5) - 1.5), 0.5);
  EXPECT_EQ(fabs(mediapipe_custom::MathUtil::Round<int>(-0.5) + 0.5), 0.5);
  EXPECT_EQ(fabs(mediapipe_custom::MathUtil::Round<int>(-1.5) + 1.5), 0.5);

  EXPECT_EQ(mediapipe_custom::MathUtil::Round<int>(static_cast<double>(0x76543210)),
            0x76543210);
  ...
  EXPECT_EQ(mediapipe_custom::MathUtil::Round<int64_t>(static_cast<double>(value64)),
            value64);
}
...
class SafeCastTester {
 public:
  static void Run() {
    ...
    // Some basic tests.
    EXPECT_EQ(mediapipe_custom::MathUtil::SafeCast<IntOut>(static_cast<FloatIn>(0.0)),
              0);
    EXPECT_EQ(mediapipe_custom::MathUtil::SafeCast<IntOut>(static_cast<FloatIn>(-0.0)),
              0);
    EXPECT_EQ(mediapipe_custom::MathUtil::SafeCast<IntOut>(static_cast<FloatIn>(0.99)),
              0);
    EXPECT_EQ(mediapipe_custom::MathUtil::SafeCast<IntOut>(static_cast<FloatIn>(1.0)),
              1);
    EXPECT_EQ(mediapipe_custom::MathUtil::SafeCast<IntOut>(static_cast<FloatIn>(1.01)),
              1);
    EXPECT_EQ(mediapipe_custom::MathUtil::SafeCast<IntOut>(static_cast<FloatIn>(1.99)),
              1);
    EXPECT_EQ(mediapipe_custom::MathUtil::SafeCast<IntOut>(static_cast<FloatIn>(2.0)),
              2);
    EXPECT_EQ(mediapipe_custom::MathUtil::SafeCast<IntOut>(static_cast<FloatIn>(2.01)),
              2);
    EXPECT_EQ(
        mediapipe_custom::MathUtil::SafeCast<IntOut>(static_cast<FloatIn>(-0.99)), 0);
    EXPECT_EQ(mediapipe_custom::MathUtil::SafeCast<IntOut>(static_cast<FloatIn>(-1.0)),
              s ? -1 : 0);
    EXPECT_EQ(
        mediapipe_custom::MathUtil::SafeCast<IntOut>(static_cast<FloatIn>(-1.01)),
        s ? -1 : 0);
    EXPECT_EQ(
        mediapipe_custom::MathUtil::SafeCast<IntOut>(static_cast<FloatIn>(-1.99)),
        s ? -1 : 0);
    EXPECT_EQ(mediapipe_custom::MathUtil::SafeCast<IntOut>(static_cast<FloatIn>(-2.0)),
              s ? -2 : 0);
    EXPECT_EQ(
        mediapipe_custom::MathUtil::SafeCast<IntOut>(static_cast<FloatIn>(-2.01)),
        s ? -2 : 0);
    EXPECT_EQ(
        mediapipe_custom::MathUtil::SafeCast<IntOut>(static_cast<FloatIn>(117.9)),
        117);
    EXPECT_EQ(
        mediapipe_custom::MathUtil::SafeCast<IntOut>(static_cast<FloatIn>(118.0)),
        118);
    EXPECT_EQ(
        mediapipe_custom::MathUtil::SafeCast<IntOut>(static_cast<FloatIn>(118.1)),
        118);
    EXPECT_EQ(
        mediapipe_custom::MathUtil::SafeCast<IntOut>(static_cast<FloatIn>(-117.9)),
        s ? -117 : 0);
    ...
    // Some edge cases.
    EXPECT_EQ(mediapipe_custom::MathUtil::SafeCast<IntOut>(
    ...
    EXPECT_EQ(mediapipe_custom::MathUtil::SafeCast<IntOut>(inf_val), imax);
    EXPECT_EQ(mediapipe_custom::MathUtil::SafeCast<IntOut>(-inf_val), imin);
    const FloatIn nan_val = inf_val - inf_val;
    EXPECT_TRUE(std::isnan(nan_val));
    EXPECT_EQ(mediapipe_custom::MathUtil::SafeCast<IntOut>(nan_val), 0);
    ...
    if (sizeof(FloatIn) >= 64) {
      ...
      EXPECT_EQ(
          mediapipe_custom::MathUtil::SafeCast<IntOut>(static_cast<FloatIn>(value64)),
          expected);
    }
...

    {
      ...
      for (int i = 0; i < kLoopCount; i++) {
        EXPECT_EQ(mediapipe_custom::MathUtil::SafeCast<IntOut>(v), imax);
        EXPECT_EQ(mediapipe_custom::MathUtil::SafeCast<IntOut>(
...

    {
      ...
      for (int i = 0; i < kLoopCount; i++) {
        EXPECT_EQ(mediapipe_custom::MathUtil::SafeCast<IntOut>(v), imin);
        EXPECT_EQ(mediapipe_custom::MathUtil::SafeCast<IntOut>(
                      static_cast<FloatIn>(v - 10000.)),
                  imin);
        v = NextAfter(v, -std::numeric_limits<FloatIn>::max());
      ...
      for (int i = 0; i < kLoopCount; i++) {
        v = std::min<IntOut>(v - 1,
                             NextAfter(static_cast<FloatIn>(v),
                                       -std::numeric_limits<FloatIn>::max()));
        EXPECT_EQ(
            mediapipe_custom::MathUtil::SafeCast<IntOut>(static_cast<FloatIn>(v)), v);
      }
    }
    ...
    if (sizeof(FloatIn) > sizeof(IntOut)) {
      {
        ...
        for (int i = 0; i < kLoopCount; i++) {
          v = NextAfter(static_cast<FloatIn>(v),
                        -std::numeric_limits<FloatIn>::max());
          EXPECT_EQ(
              mediapipe_custom::MathUtil::SafeCast<IntOut>(static_cast<FloatIn>(v)),
              imax - 1);
        }
      }
      EXPECT_EQ(mediapipe_custom::MathUtil::SafeCast<IntOut>(
...
TEST(MathUtil, SafeCast) {
  ...
  EXPECT_EQ(mediapipe_custom::MathUtil::SafeCast<int>(static_cast<float>(12345.678)),
            12345);
  EXPECT_EQ(mediapipe_custom::MathUtil::SafeCast<int>(static_cast<float>(12345.4321)),
            12345);
  EXPECT_EQ(mediapipe_custom::MathUtil::SafeCast<int>(static_cast<double>(-12345.678)),
            -12345);
  EXPECT_EQ(
      mediapipe_custom::MathUtil::SafeCast<int>(static_cast<double>(-12345.4321)),
      -12345);
  EXPECT_EQ(mediapipe_custom::MathUtil::SafeCast<int>(1E47), 2147483647);
  EXPECT_EQ(mediapipe_custom::MathUtil::SafeCast<int>(-1E47),
...
class SafeRoundTester {
 public:
  static void Run() {
    ...
    // Some basic tests.
    EXPECT_EQ(mediapipe_custom::MathUtil::SafeRound<IntOut>(static_cast<FloatIn>(0.0)),
              0);
    EXPECT_EQ(
        mediapipe_custom::MathUtil::SafeRound<IntOut>(static_cast<FloatIn>(-0.0)), 0);
    EXPECT_EQ(
        mediapipe_custom::MathUtil::SafeRound<IntOut>(static_cast<FloatIn>(0.49)), 0);
    EXPECT_EQ(
        mediapipe_custom::MathUtil::SafeRound<IntOut>(static_cast<FloatIn>(0.51)), 1);
    EXPECT_EQ(
        mediapipe_custom::MathUtil::SafeRound<IntOut>(static_cast<FloatIn>(1.49)), 1);
    EXPECT_EQ(
        mediapipe_custom::MathUtil::SafeRound<IntOut>(static_cast<FloatIn>(1.51)), 2);
    EXPECT_EQ(
        mediapipe_custom::MathUtil::SafeRound<IntOut>(static_cast<FloatIn>(-0.49)), 0);
    EXPECT_EQ(
        mediapipe_custom::MathUtil::SafeRound<IntOut>(static_cast<FloatIn>(-0.51)),
        s ? -1 : 0);
    EXPECT_EQ(
        mediapipe_custom::MathUtil::SafeRound<IntOut>(static_cast<FloatIn>(-1.49)),
        s ? -1 : 0);
    EXPECT_EQ(
        mediapipe_custom::MathUtil::SafeRound<IntOut>(static_cast<FloatIn>(-1.51)),
        s ? -2 : 0);
    EXPECT_EQ(
        mediapipe_custom::MathUtil::SafeRound<IntOut>(static_cast<FloatIn>(117.4)),
        117);
    EXPECT_EQ(
        mediapipe_custom::MathUtil::SafeRound<IntOut>(static_cast<FloatIn>(117.6)),
        118);
    EXPECT_EQ(
        mediapipe_custom::MathUtil::SafeRound<IntOut>(static_cast<FloatIn>(-117.4)),
        s ? -117 : 0);
    ...
    // result.
    EXPECT_EQ(
        fabs(mediapipe_custom::MathUtil::SafeRound<IntOut>(static_cast<FloatIn>(0.5)) -
             0.5),
        0.5);
    EXPECT_EQ(
        fabs(mediapipe_custom::MathUtil::SafeRound<IntOut>(static_cast<FloatIn>(1.5)) -
             1.5),
        0.5);
    EXPECT_EQ(fabs(mediapipe_custom::MathUtil::SafeRound<IntOut>(
    ...
    if (s) {
      ...
      EXPECT_EQ(
          mediapipe_custom::MathUtil::SafeRound<IntOut>(static_cast<FloatIn>(-0.5)),
          0);
      EXPECT_EQ(
          mediapipe_custom::MathUtil::SafeRound<IntOut>(static_cast<FloatIn>(-1.5)),
          0);
      EXPECT_EQ(
          mediapipe_custom::MathUtil::SafeRound<IntOut>(static_cast<FloatIn>(-117.5)),
          0);
    }

    // Some edge cases.
    EXPECT_EQ(mediapipe_custom::MathUtil::SafeRound<IntOut>(
                  std::numeric_limits<FloatIn>::max()),
              imax);
    EXPECT_EQ(mediapipe_custom::MathUtil::SafeRound<IntOut>(
                  -std::numeric_limits<FloatIn>::max()),
              imin);
    const FloatIn inf_val = std::numeric_limits<FloatIn>::infinity();
    EXPECT_EQ(mediapipe_custom::MathUtil::SafeRound<IntOut>(inf_val), imax);
    EXPECT_EQ(mediapipe_custom::MathUtil::SafeRound<IntOut>(-inf_val), imin);
    const FloatIn nan_val = inf_val - inf_val;
    EXPECT_TRUE(std::isnan(nan_val));
    EXPECT_EQ(mediapipe_custom::MathUtil::SafeRound<IntOut>(nan_val), 0);
    ...
    if (sizeof(FloatIn) >= 64) {
      ...
      const IntOut expected =
          (sizeof(IntOut) >= 64) ? static_cast<IntOut>(value64) : imax;
      EXPECT_EQ(
          mediapipe_custom::MathUtil::SafeRound<IntOut>(static_cast<FloatIn>(value64)),
          expected);
    }
...

    {
      ...
      FloatIn v = static_cast<FloatIn>(imax);
      for (int i = 0; i < kLoopCount; i++) {
        EXPECT_EQ(mediapipe_custom::MathUtil::SafeRound<IntOut>(v), imax);
        EXPECT_EQ(mediapipe_custom::MathUtil::SafeRound<IntOut>(
                      static_cast<FloatIn>(v + 10000.)),
                  imax);
        v = NextAfter(v, std::numeric_limits<FloatIn>::max());
...

    {
      ...
      FloatIn v = static_cast<FloatIn>(imin);
      for (int i = 0; i < kLoopCount; i++) {
        EXPECT_EQ(mediapipe_custom::MathUtil::SafeRound<IntOut>(v), imin);
        EXPECT_EQ(mediapipe_custom::MathUtil::SafeRound<IntOut>(
                      static_cast<FloatIn>(v - 10000.)),
                  imin);
        v = NextAfter(v, -std::numeric_limits<FloatIn>::max());
      ...
      for (int i = 0; i < kLoopCount; i++) {
        v = std::min<IntOut>(v - 1,
                             NextAfter(static_cast<FloatIn>(v),
                                       -std::numeric_limits<FloatIn>::max()));
        EXPECT_EQ(
            mediapipe_custom::MathUtil::SafeRound<IntOut>(static_cast<FloatIn>(v)), v);
      }
    }
    ...
    if (sizeof(FloatIn) > sizeof(IntOut)) {
      {
        ...
        for (int i = 0; i < kLoopCount; i++) {
          v = NextAfter(static_cast<FloatIn>(v),
                        -std::numeric_limits<FloatIn>::max());
          EXPECT_EQ(
              mediapipe_custom::MathUtil::SafeRound<IntOut>(static_cast<FloatIn>(v)),
              imax);
        }
      }
      EXPECT_EQ(mediapipe_custom::MathUtil::SafeRound<IntOut>(
...
TEST(MathUtil, SafeRound) {
  ...
  EXPECT_EQ(mediapipe_custom::MathUtil::SafeRound<int>(static_cast<float>(12345.678)),
            12346);
  EXPECT_EQ(mediapipe_custom::MathUtil::SafeRound<int>(static_cast<float>(12345.4321)),
            12345);
  EXPECT_EQ(
      mediapipe_custom::MathUtil::SafeRound<int>(static_cast<double>(-12345.678)),
      -12346);
  EXPECT_EQ(
      mediapipe_custom::MathUtil::SafeRound<int>(static_cast<double>(-12345.4321)),
      -12345);
  EXPECT_EQ(mediapipe_custom::MathUtil::SafeRound<int>(1E47), 2147483647);
  EXPECT_EQ(mediapipe_custom::MathUtil::SafeRound<int>(-1E47),
            static_cast<int64_t>(-2147483648));
}
