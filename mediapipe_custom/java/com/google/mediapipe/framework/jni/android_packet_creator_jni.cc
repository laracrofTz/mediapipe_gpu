...
std::unique_ptr<mediapipe_custom::ImageFrame> CreateImageFrameFromBitmap(
  ...
  if (format == mediapipe_custom::ImageFormat::SRGBA) {
    ...
    std::memcpy(image_frame->MutablePixelData(), pixel_addr,
                image_frame->PixelDataSize());
  } else if (format == mediapipe_custom::ImageFormat::SRGB) {
    if (stride != width * 4) {
      ABSL_LOG(ERROR) << "Bitmap stride: " << stride
    ...
    mediapipe_custom::android::RgbaToRgb(rgba_data, stride, width, height,
...
JNIEXPORT jlong JNICALL ANDROID_PACKET_CREATOR_METHOD(
  ...
  auto image_frame =
      CreateImageFrameFromBitmap(env, bitmap, info.width, info.height,
                                 info.stride, mediapipe_custom::ImageFormat::SRGB);
  if (nullptr == image_frame) return 0L;

  mediapipe_custom::Packet packet = mediapipe_custom::Adopt(image_frame.release());
...
JNIEXPORT jlong JNICALL ANDROID_PACKET_CREATOR_METHOD(
  ...
  auto image_frame =
      CreateImageFrameFromBitmap(env, bitmap, info.width, info.height,
                                 info.stride, mediapipe_custom::ImageFormat::SRGBA);
  if (nullptr == image_frame) return 0L;
...
JNIEXPORT jlong JNICALL ANDROID_PACKET_CREATOR_METHOD(nativeCreateRgbaImage)(
  ...
  mediapipe_custom::Packet packet =
      mediapipe_custom::MakePacket<mediapipe_custom::Image>(std::move(image_frame));
  return CreatePacketWithContext(context, packet);
}
