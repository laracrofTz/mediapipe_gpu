...
#include "mediapipe_custom/tasks/c/text/text_embedder/text_embedder.h"
...
#include "mediapipe_custom/tasks/cc/text/text_embedder/text_embedder.h"

namespace mediapipe_custom::tasks::c::text::text_embedder {
...
using ::mediapipe_custom::tasks::text::text_embedder::TextEmbedder;
...
TextEmbedder* CppTextEmbedderCreate(const TextEmbedderOptions& options,
  ...
  auto cpp_options = std::make_unique<
      ::mediapipe_custom::tasks::text::text_embedder::TextEmbedderOptions>();
...
int CppTextEmbedderCosineSimilarity(const Embedding* u, const Embedding* v,
  ...
  auto status_or_similarity =
      mediapipe_custom::tasks::text::text_embedder::TextEmbedder::CosineSimilarity(
          cpp_u, cpp_v);
...
void* text_embedder_create(struct TextEmbedderOptions* options,
                           char** error_msg) {
  return mediapipe_custom::tasks::c::text::text_embedder::CppTextEmbedderCreate(
      *options, error_msg);
}

int text_embedder_embed(void* embedder, const char* utf8_str,
                        TextEmbedderResult* result, char** error_msg) {
  return mediapipe_custom::tasks::c::text::text_embedder::CppTextEmbedderEmbed(
      embedder, utf8_str, result, error_msg);
}

void text_embedder_close_result(TextEmbedderResult* result) {
  mediapipe_custom::tasks::c::text::text_embedder::CppTextEmbedderCloseResult(result);
}

int text_embedder_close(void* embedder, char** error_ms) {
  return mediapipe_custom::tasks::c::text::text_embedder::CppTextEmbedderClose(
      embedder, error_ms);
}

int text_embedder_cosine_similarity(const Embedding* u, const Embedding* v,
                                    double* similarity, char** error_msg) {
  return mediapipe_custom::tasks::c::text::text_embedder::
