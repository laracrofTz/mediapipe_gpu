...
#include "mediapipe_custom/framework/calculator_contract_test.pb.h"
...
TEST(CalculatorContractTest, PacketGenerator) {
  const PacketGeneratorConfig node =
      mediapipe_custom::ParseTextProtoOrDie<PacketGeneratorConfig>(R"pb(
...
TEST(CalculatorContractTest, StatusHandler) {
  const StatusHandlerConfig node =
      mediapipe_custom::ParseTextProtoOrDie<StatusHandlerConfig>(R"pb(
