...
absl::flat_hash_map<std::string, std::string> JMapToAbslStringMap(
  ...
  while (env->CallBooleanMethod(iterator, hasNextMethod)) {
    ...
    // Add to the mapping.
    std::string key = mediapipe_custom::android::JStringToStdString(env, javaKey);
    std::string value = mediapipe_custom::android::JStringToStdString(env, javaValue);
    mapping[key] = value;
...
jthrowable CreateMediaPipeException(JNIEnv* env, absl::Status status) {
  ...
  std::string mpe_class_name = class_registry.GetClassName(
      mediapipe_custom::android::ClassRegistry::kMediaPipeExceptionClassName);
...
bool ThrowIfError(JNIEnv* env, absl::Status status) {
  if (!status.ok()) {
    env->Throw(mediapipe_custom::android::CreateMediaPipeException(env, status));
    return true;
  }
  return false;
}
