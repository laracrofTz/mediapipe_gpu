...
#include "mediapipe_custom/framework/api3/internal/graph_builder.h"
#include "mediapipe_custom/framework/api3/side_packet.h"
#include "mediapipe_custom/framework/api3/stream.h"
#include "mediapipe_custom/framework/calculator_base.h"
#include "mediapipe_custom/framework/calculator_contract.h"
#include "mediapipe_custom/framework/deps/registration.h"
#include "mediapipe_custom/framework/mediapipe_options.pb.h"
...
using SourceImplBase =
    std::conditional_t<IsSide, mediapipe_custom::api3::SidePacket<T>,
                       mediapipe_custom::api3::Stream<T>>;
...
class Graph : public mediapipe_custom::api3::GenericGraph {
 public:
  Graph() = default;
  ~Graph() = default;
  Graph(Graph&&) = default;
  ...
  void SetType(std::string type) {
    mediapipe_custom::api3::GenericGraph::builder_.SetType(std::move(type));
  }
  ...
  Node<Calc>& AddNode() {
    auto node = std::make_unique<Node<Calc>>(
        mediapipe_custom::api3::GenericGraph::builder_.AddNode(
  ...
  PacketGenerator& AddPacketGenerator(absl::string_view type) {
    auto node = std::make_unique<PacketGenerator>(
        mediapipe_custom::api3::GenericGraph::builder_.AddPacketGenerator(
            std::string(type.data(), type.size())));
    auto node_p = node.get();
  ...
  Executor& AddExecutor(absl::string_view type) {
    return mediapipe_custom::api3::GenericGraph::builder_.AddExecutor(type);
  }

  // Graph ports, non-typed.
  MultiSource<> In(absl::string_view graph_input) {
    return MultiSource<>(
        mediapipe_custom::api3::GenericGraph::builder_.In(graph_input));
  }

  MultiDestination<> Out(absl::string_view graph_output) {
    return MultiDestination<>(
        mediapipe_custom::api3::GenericGraph::builder_.Out(graph_output));
  }

  MultiSideSource<> SideIn(absl::string_view graph_input) {
    return MultiSideSource<>(
        mediapipe_custom::api3::GenericGraph::builder_.SideIn(graph_input));
  }

  MultiSideDestination<> SideOut(absl::string_view graph_output) {
    return MultiSideDestination<>(
        mediapipe_custom::api3::GenericGraph::builder_.SideOut(graph_output));
  }
  ...
  auto operator[](const PortCommon<B, T, kIsOptional, kIsMultiple>& port) {
    using PayloadT =
        typename PortCommon<B, T, kIsOptional, kIsMultiple>::PayloadT;
    if constexpr (std::is_same_v<B, OutputBase>) {
      auto base = mediapipe_custom::api3::GenericGraph::builder_.Out(port.Tag());
      if constexpr (kIsMultiple) {
        return MultiDestination<PayloadT>(base);
      } else {
        return Destination<PayloadT>(base);
      ...
      auto base = mediapipe_custom::api3::GenericGraph::builder_.In(port.Tag());
      if constexpr (kIsMultiple) {
        return MultiSource<PayloadT>(base);
      } else {
        return Source<PayloadT>(base);
      ...
      auto base = mediapipe_custom::api3::GenericGraph::builder_.SideOut(port.Tag());
      if constexpr (kIsMultiple) {
        return MultiSideDestination<PayloadT>(base);
      } else {
        return SideDestination<PayloadT>(base);
      ...
      auto base = mediapipe_custom::api3::GenericGraph::builder_.SideIn(port.Tag());
      if constexpr (kIsMultiple) {
        return MultiSideSource<PayloadT>(base);
      } else {
        return SideSource<PayloadT>(base);
  ...
  CalculatorGraphConfig GetConfig() {
    auto config = mediapipe_custom::api3::GenericGraph::builder_.GetConfig();
