...
TEST(AnyTest, CanUseAnyForNodeTickInputImplementation) {
  ...
  mediapipe_custom::Packet output;
  MP_ASSERT_OK(calculator_graph.ObserveOutputStream(
      "out", [&](const mediapipe_custom::Packet& p) {
        output = p;
        return absl::OkStatus();
      }));
  ...
  MP_ASSERT_OK(calculator_graph.AddPacketToInputStream(
      "tick", mediapipe_custom::MakePacket<SomeTick>().At(Timestamp(1000))));
  MP_ASSERT_OK(calculator_graph.WaitUntilIdle());

  ASSERT_FALSE(output.IsEmpty());
  EXPECT_EQ(output.Get<int>(), 42);

  output = mediapipe_custom::Packet();  // Reset packet.
  ...
  MP_ASSERT_OK(calculator_graph.AddPacketToInputStream(
      "tick", mediapipe_custom::MakePacket<std::string>(
...
TEST(AnyTest, CalculatorsCanSupportSameAsAny) {
  ...
  CalculatorGraph calculator_graph;
  MP_ASSERT_OK(calculator_graph.Initialize(std::move(config)));
  mediapipe_custom::Packet output_a;
  MP_ASSERT_OK(calculator_graph.ObserveOutputStream(
      "passed_a", [&](const mediapipe_custom::Packet& p) {
        output_a = p;
        return absl::OkStatus();
      }));
  mediapipe_custom::Packet output_b;
  MP_ASSERT_OK(calculator_graph.ObserveOutputStream(
      "passed_b", [&](const mediapipe_custom::Packet& p) {
        output_b = p;
        return absl::OkStatus();
      }));
  MP_ASSERT_OK(
      calculator_graph.StartRun({{"side", mediapipe_custom::MakePacket<int>(256)}}));

  // Using input of the same type as in the graph builder.
  MP_ASSERT_OK(calculator_graph.AddPacketToInputStream(
      "a", mediapipe_custom::MakePacket<int>(42).At(Timestamp(1000))));
  MP_ASSERT_OK(calculator_graph.AddPacketToInputStream(
      "b", mediapipe_custom::MakePacket<std::string>("str").At(Timestamp(1000))));
  MP_ASSERT_OK(calculator_graph.WaitUntilIdle());

  MP_ASSERT_OK_AND_ASSIGN(mediapipe_custom::Packet passed_side,
