...
#include "mediapipe_custom/framework/test_calculators.pb.h"
 ...
 public:
  ...
  absl::Status Close(CalculatorContext* cc) final {
    cc->Outputs().Index(0).Add(
        new int(mediapipe_custom::MathUtil::SafeRound<int, double>(
...
class ForwardNestedPacketOrEmitBoundUpdateCalculator : public CalculatorBase {
 public:
  static absl::Status GetContract(CalculatorContract* cc) {
    cc->Inputs().Index(0).Set<mediapipe_custom::Packet>();
    cc->Outputs().Index(0).SetAny();
    return absl::OkStatus();
  }

  absl::Status Process(CalculatorContext* cc) override {
    const auto& nested_packet = cc->Inputs().Index(0).Get<mediapipe_custom::Packet>();
  ...
  absl::Status Process(CalculatorContext* cc) override {
    if (cc->Inputs().Index(0).IsEmpty()) {
      ...
      cc->Outputs().Index(0).AddPacket(
          mediapipe_custom::MakePacket<Timestamp>(bound).At(bound));
    }
    return absl::OkStatus();
  }
};
