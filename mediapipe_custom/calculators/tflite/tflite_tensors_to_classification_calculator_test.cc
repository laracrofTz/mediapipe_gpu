...
#include "mediapipe_custom/calculators/tflite/tflite_tensors_to_classification_calculator.pb.h"
...
using Node = ::mediapipe_custom::CalculatorGraphConfig::Node;

class TfLiteTensorsToClassificationCalculatorTest : public ::testing::Test {
 protected:
  void BuildGraph(mediapipe_custom::CalculatorRunner* runner,
    ...
    input_stream_packets.push_back(
        mediapipe_custom::Adopt(tensors.release())
            .At(mediapipe_custom::Timestamp(stream_timestamp++)));
  }
...
TEST_F(TfLiteTensorsToClassificationCalculatorTest, CorrectOutput) {
  mediapipe_custom::CalculatorRunner runner(ParseTextProtoOrDie<Node>(R"pb(
