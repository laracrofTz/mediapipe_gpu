#include "mediapipe_custom/tasks/cc/text/utils/vocab_convert_utils.h"
...
absl::Status ConvertHfTokenizer(const std::string& hf_tokenizer,
  ...
  absl::string_view output_dir = ::mediapipe_custom::file::Dirname(output_vocab_path);
  if (!::mediapipe_custom::file::IsDirectory(output_dir).ok()) {
    MP_RETURN_IF_ERROR(::mediapipe_custom::file::RecursivelyCreateDir(output_dir));
  }
