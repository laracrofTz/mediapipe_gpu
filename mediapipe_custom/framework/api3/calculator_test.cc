...
#include "mediapipe_custom/framework/api3/calculator_test.h"
#include "mediapipe_custom/framework/api3/contract.h"
#include "mediapipe_custom/framework/api3/node.h"
...
TEST(CalculatorTest, CanReadWritePortsAndUseServices) {
  ...
  MP_ASSERT_OK(graph.StartRun(
      {{"side_in", mediapipe_custom::MakePacket<std::string>("side")}}));
  MP_ASSERT_OK(graph.AddPacketToInputStream(
      "in", mediapipe_custom::MakePacket<int>(42).At(Timestamp(0))));
...
TEST(CalculatorTest, CanUseSharedContract) {
  ...
  mediapipe_custom::Packet out_a;
  MP_ASSERT_OK(
      graph.ObserveOutputStream("out_a", [&out_a](const mediapipe_custom::Packet& p) {
        out_a = p;
        return absl::OkStatus();
      }));
  mediapipe_custom::Packet out_b;
  MP_ASSERT_OK(
      graph.ObserveOutputStream("out_b", [&out_b](const mediapipe_custom::Packet& p) {
        out_b = p;
        return absl::OkStatus();
      }));
  ...
  // Verifying outputs.
  MP_ASSERT_OK_AND_ASSIGN(mediapipe_custom::Packet side_out_a,
                          graph.GetOutputSidePacket("side_out_a"));
  ASSERT_FALSE(side_out_a.IsEmpty());
  EXPECT_EQ(side_out_a.Get<std::string>(), "side");

  MP_ASSERT_OK_AND_ASSIGN(mediapipe_custom::Packet side_out_b,
...
TEST(CalculatorTest, TimestampOffsetZeroIsTheDefault) {
  ...
  CalculatorGraph graph;
  MP_ASSERT_OK(graph.Initialize(std::move(config)));
  std::vector<mediapipe_custom::Packet> output_packets;
  MP_ASSERT_OK(graph.ObserveOutputStream(
      "out",
      [&output_packets](const mediapipe_custom::Packet& p) {
  ...
  MP_ASSERT_OK(graph.AddPacketToInputStream(
      "in", mediapipe_custom::MakePacket<int>(43).At(Timestamp(1))));
  MP_ASSERT_OK(graph.WaitUntilIdle());
...
TEST(CalculatorTest, CanRunGeneratorCalculator) {
  ...
  // Verifying outputs.
  MP_ASSERT_OK_AND_ASSIGN(mediapipe_custom::Packet value,
