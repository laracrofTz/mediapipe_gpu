...
namespace mediapipe_custom::debug {
...
// [min_range, max_range].
void LogTensorChannel(const mediapipe_custom::Tensor& tensor, int channel,
...
void LogTensor(const mediapipe_custom::Tensor& tensor,
               absl::string_view name = "tensor", float min_range = 0.0f,
               float max_range = 1.0f);

// Logs the given image as a color or ASCII image, depending on terminal
// capabilities.
void LogImage(const mediapipe_custom::ImageFrame& image,
