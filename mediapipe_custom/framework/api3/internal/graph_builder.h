...
#include "mediapipe_custom/framework/port/any_proto.h"
...
namespace mediapipe_custom::api3::builder {
...
OptionsT& GetOptions(std::optional<mediapipe_custom::MediaPipeOptions>& options) {
  if (!options.has_value()) {
    options = mediapipe_custom::MediaPipeOptions();
  }
...
class Executor {
 public:
  template <typename OptionsT>
  OptionsT& GetOptions() {
    return ::mediapipe_custom::api3::builder::GetOptions<OptionsT>(options_);
  }
 ...
 private:
  ...
  std::optional<mediapipe_custom::MediaPipeOptions> options_;
...
class PacketGeneratorBuilder {
 ...
 private:
  ...
  TagIndexMap<SideDestination> in_sides_;
  TagIndexMap<SideSource> out_sides_;
  mediapipe_custom::PacketGeneratorOptions options_;
