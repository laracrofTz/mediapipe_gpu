...
TEST(SplitTest, SplitToRanges2Ranges) {
  Graph graph;
  Stream<std::vector<mediapipe_custom::Tensor>> tensors =
      graph.In("TENSORS").Cast<std::vector<mediapipe_custom::Tensor>>();
  std::vector<Stream<std::vector<mediapipe_custom::Tensor>>> result =
...
TEST(SplitTest, Split2Items) {
  ...
  Stream<std::vector<mediapipe_custom::Tensor>> tensors =
      graph.In("TENSORS").Cast<std::vector<mediapipe_custom::Tensor>>();
  std::vector<Stream<mediapipe_custom::Tensor>> result = Split(tensors, {0, 1}, graph);
  EXPECT_EQ(result.size(), 2);
  EXPECT_THAT(
      graph.GetConfig(),
      EqualsProto(mediapipe_custom::ParseTextProtoOrDie<CalculatorGraphConfig>(R"pb(
...
TEST(SplitTest, SplitAndCombineRanges) {
  ...
  Stream<std::vector<mediapipe_custom::Tensor>> tensors =
      graph.In("TENSORS").Cast<std::vector<mediapipe_custom::Tensor>>();
  Stream<std::vector<mediapipe_custom::Tensor>> result =
