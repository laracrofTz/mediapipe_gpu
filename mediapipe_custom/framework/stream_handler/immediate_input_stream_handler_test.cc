...
class ImmediateInputStreamHandlerTest : public ::testing::Test {
 protected:
  ...
  void SetupInputStreamHandler(
    ...
    absl::StatusOr<std::unique_ptr<mediapipe_custom::InputStreamHandler>>
  ...
  void ExpectPackets(
    ...
    for (const auto& name_and_id : name_to_id_) {
      ...
      if (mediapipe_custom::ContainsKey(expected_values, name_and_id.first)) {
        ASSERT_FALSE(input_stream.Value().IsEmpty());
        EXPECT_EQ(input_stream.Value().Get<std::string>(),
                  mediapipe_custom::FindOrDie(expected_values, name_and_id.first));
      } else {
