...
#include "mediapipe_custom/calculators/audio/resample_time_series_calculator.h"
...
void CopyChannelToVector(const mediapipe_custom::Matrix& matrix, int channel,
...
void CopyVectorToChannel(const std::vector<float>& vec,
                         mediapipe_custom::Matrix* matrix, int channel) {
  if (matrix->cols() == 0) {
...
absl::Status ResampleTimeSeriesCalculatorImpl::Open(CalculatorContext* cc) {
  ...
  kOutput(cc).SetHeader(mediapipe_custom::api2::FromOldPacket(Adopt(output_header)));
