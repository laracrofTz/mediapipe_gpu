...
#include "mediapipe_custom/gpu/gl_surface_sink_calculator.pb.h"
...
class GlSurfaceSinkCalculator : public Node {
 public:
  static constexpr Input<
      OneOf<mediapipe_custom::Image, mediapipe_custom::GpuBuffer>>::Optional kInVideo{
      "VIDEO"};
  static constexpr Input<
      OneOf<mediapipe_custom::Image, mediapipe_custom::GpuBuffer>>::Optional kIn{""};
  static constexpr SideInput<std::unique_ptr<mediapipe_custom::EglSurfaceHolder>>
      kSurface{"SURFACE"};
 ...
 private:
  mediapipe_custom::GlCalculatorHelper helper_;
  mediapipe_custom::EglSurfaceHolder* surface_holder_;
  bool initialized_ = false;
  std::unique_ptr<mediapipe_custom::QuadRenderer> renderer_;
  mediapipe_custom::FrameScaleMode scale_mode_ =
      mediapipe_custom::FrameScaleMode::kFillAndCrop;
};
...
absl::Status GlSurfaceSinkCalculator::Open(CalculatorContext* cc) {
  ...
  scale_mode_ = FrameScaleModeFromProto(
      cc->Options<mediapipe_custom::GlSurfaceSinkCalculatorOptions>()
          .frame_scale_mode(),
      mediapipe_custom::FrameScaleMode::kFillAndCrop);
...
absl::Status GlSurfaceSinkCalculator::Process(CalculatorContext* cc) {
  return helper_.RunInGlContext([this, &cc]() -> absl::Status {
    ...
    mediapipe_custom::GpuBuffer input;
    if (packet.ValidateAsType<mediapipe_custom::GpuBuffer>().ok())
      input = packet.Get<mediapipe_custom::GpuBuffer>();
    if (packet.ValidateAsType<mediapipe_custom::Image>().ok())
      input = packet.Get<mediapipe_custom::Image>().GetGpuBuffer();

    if (!initialized_) {
      renderer_ = std::make_unique<mediapipe_custom::QuadRenderer>();
    ...
    MP_RETURN_IF_ERROR(
        renderer_->GlRender(src.width(), src.height(), dst_width, dst_height,
                            scale_mode_, mediapipe_custom::FrameRotation::kNone,
...
GlSurfaceSinkCalculator::~GlSurfaceSinkCalculator() {
  if (renderer_) {
    ...
    mediapipe_custom::QuadRenderer* renderer = renderer_.release();
