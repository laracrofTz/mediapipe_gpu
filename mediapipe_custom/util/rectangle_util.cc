...
absl::StatusOr<Rectangle_f> ToRectangle(
    const mediapipe_custom::NormalizedRect& input) {
...
absl::StatusOr<bool> DoesRectOverlap(
    const mediapipe_custom::NormalizedRect& new_rect,
    absl::Span<const mediapipe_custom::NormalizedRect> existing_rects,
    float min_similarity_threshold) {
  MP_ASSIGN_OR_RETURN(Rectangle_f new_rectangle, ToRectangle(new_rect));

  for (const mediapipe_custom::NormalizedRect& existing_rect : existing_rects) {
