...
#include "mediapipe_custom/calculators/core/flow_limiter_calculator.pb.h"
...
class SleepCalculator : public CalculatorBase {
 public:
  static absl::Status GetContract(CalculatorContract* cc) {
    ...
    cc->InputSidePackets().Tag(kClockTag).Set<mediapipe_custom::Clock*>();
    cc->SetTimestampOffset(0);
    return absl::OkStatus();
  }

  absl::Status Open(CalculatorContext* cc) final {
    clock_ = cc->InputSidePackets().Tag(kClockTag).Get<mediapipe_custom::Clock*>();
    return absl::OkStatus();
  }
 ...
 private:
  ::mediapipe_custom::Clock* clock_ = nullptr;
  int packet_count = 0;
};
...
TEST_F(FlowLimiterCalculatorTest, FinishedTimestamps) {
  ...
  std::map<std::string, Packet> side_packets = {
      ...
      {"clock", MakePacket<mediapipe_custom::Clock*>(clock_)},
  };
