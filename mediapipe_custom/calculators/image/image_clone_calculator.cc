...
class ImageCloneCalculator : public Node {
 public:
...
#if MEDIAPIPE_DISABLE_GPU
    if (cc->Options<mediapipe_custom::ImageCloneCalculatorOptions>().output_on_gpu()) {
  ...
  absl::Status Open(CalculatorContext* cc) override {
    const auto& options = cc->Options<mediapipe_custom::ImageCloneCalculatorOptions>();
  ...
  absl::Status Process(CalculatorContext* cc) override {
    ...
    if (input_on_gpu) {
      ...
      mediapipe_custom::Packet* packet_copy_ptr =
          new mediapipe_custom::Packet(kIn(cc).packet());
      ...
      output = std::make_unique<Image>(std::make_unique<mediapipe_custom::ImageFrame>(
