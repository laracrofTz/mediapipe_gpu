...
#include "mediapipe_custom/calculators/tflite/tflite_custom_op_resolver_calculator.pb.h"
...
#include "mediapipe_custom/util/tflite/cpu_op_resolver.h"
#include "mediapipe_custom/util/tflite/op_resolver.h"
...
class TfLiteCustomOpResolverCalculator : public CalculatorBase {
 public:
  ...
  absl::Status Open(CalculatorContext* cc) override {
    ...
    if (options.use_gpu()) {
      op_resolver = absl::make_unique<mediapipe_custom::OpResolver>();
    } else {
      op_resolver = absl::make_unique<mediapipe_custom::CpuOpResolver>();
    }

    if (cc->OutputSidePackets().HasTag(kOpResolverTag)) {
      cc->OutputSidePackets()
          .Tag(kOpResolverTag)
          .Set(mediapipe_custom::api2::PacketAdopting<tflite::OpResolver>(
