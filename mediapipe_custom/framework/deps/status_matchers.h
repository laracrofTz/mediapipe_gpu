...
#define MP_EXPECT_OK(expression) EXPECT_THAT(expression, mediapipe_custom::IsOk())
#define MP_ASSERT_OK(expression) ASSERT_THAT(expression, mediapipe_custom::IsOk())
...
inline void MpInternalAssertAddFatalFailure(
    absl::string_view expression, const mediapipe_custom::StatusBuilder& builder) {
