...
#include "mediapipe_custom/framework/tool/template_expander.h"
...
#include "mediapipe_custom/framework/port/numbers.h"
...
using mediapipe_custom::proto_ns::MessageLite;
using mediapipe_custom::tool::ProtoUtilLite;
    ...
    for (int i = 0; i < args.size(); ++i) {
      if (args[i].has_dict()) {
        ...
        std::string text_value = args[i].has_num()
                                     ? mediapipe_custom::SimpleDtoa(args[i].num())
                                     : args[i].str();
