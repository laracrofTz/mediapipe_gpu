...
using ::mediapipe_custom::CalculatorGraphConfig;
using ::mediapipe_custom::Image;
using ::mediapipe_custom::NormalizedRect;
using ::mediapipe_custom::tasks::vision::image_segmenter::ImageSegmenterResult;
using ImageSegmenterGraphOptionsProto = ::mediapipe_custom::tasks::vision::
...
absl::StatusOr<ImageSegmenterResult> InteractiveSegmenter::Segment(
    mediapipe_custom::Image image, const RegionOfInterest& roi,
  ...
  MP_ASSIGN_OR_RETURN(
      auto output_packets,
      ProcessImageData(
          {{kImageInStreamName, mediapipe_custom::MakePacket<Image>(std::move(image))},
           {kRoiStreamName,
            mediapipe_custom::MakePacket<RenderData>(std::move(roi_as_render_data))},
           {kNormRectStreamName,
