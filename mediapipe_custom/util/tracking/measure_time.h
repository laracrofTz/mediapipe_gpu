...
#define MEASURE_TIME_IMPL(show_output, line, file)                      \
  std::unique_ptr<mediapipe_custom::ScopedWallTimer> scoped_wall_timer_##line; \
  const bool activated##line = show_output;                             \
  if (activated##line) {                                                \
    static mediapipe_custom::ScopedWallTimer::Accumulator*                     \
        scoped_wall_timer_accum_##line =                                \
            new mediapipe_custom::ScopedWallTimer::Accumulator;                \
    scoped_wall_timer_##line.reset(new mediapipe_custom::ScopedWallTimer(      \
