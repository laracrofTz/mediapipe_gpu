...
namespace mediapipe_custom::api2 {

absl::Status InferenceOnDiskCacheHelper::Init(
    const mediapipe_custom::InferenceCalculatorOptions& options,
    const mediapipe_custom::InferenceCalculatorOptions::Delegate::Gpu&
  ...
  if (use_kernel_caching_) {
    ...
    cached_kernel_filename_ =
        mediapipe_custom::file::JoinPath(gpu_delegate_options.cached_kernel_path(),
                                  absl::StrCat(basename, ".ker"));
  }
  if (use_serialized_model_) {
    serialized_model_path_ =
        mediapipe_custom::file::JoinPath(gpu_delegate_options.serialized_model_dir(),
                                  gpu_delegate_options.model_token());
  }
  cache_writing_behavior_ = gpu_delegate_options.has_cache_writing_behavior()
                                ? gpu_delegate_options.cache_writing_behavior()
                                : mediapipe_custom::InferenceCalculatorOptions::
...
absl::Status InferenceOnDiskCacheHelper::SaveGpuCachesBasedOnBehavior(
  ...
  switch (cache_writing_behavior_) {
    case mediapipe_custom::InferenceCalculatorOptions::Delegate::Gpu::NO_WRITE:
      return absl::OkStatus();
    case mediapipe_custom::InferenceCalculatorOptions::Delegate::Gpu::TRY_WRITE: {
    ...
    case mediapipe_custom::InferenceCalculatorOptions::Delegate::Gpu::WRITE_OR_ERROR:
...
absl::Status InferenceOnDiskCacheHelper::SaveGpuCaches(
  ...
  if (use_serialized_model_ && gpu_runner.CanGenerateSerializedModel()) {
    ...
    MP_RETURN_IF_ERROR(
        mediapipe_custom::file::SetContents(serialized_model_path_, serialized_model));
  }
  return absl::OkStatus();
}
...
absl::Status InferenceOnDiskCacheHelper::ReadGpuCaches(
    tflite::gpu::TFLiteGPURunner& gpu_runner) const {
  if (use_kernel_caching_ &&
      mediapipe_custom::file::Exists(cached_kernel_filename_).ok()) {
  ...
  if (use_serialized_model_ &&
      mediapipe_custom::file::Exists(serialized_model_path_).ok()) {
