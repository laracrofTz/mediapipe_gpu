...
std::vector<::mediapipe_custom::Location> GetBBoxAt(
    const std::string& prefix, const tensorflow::SequenceExample& sequence,
    int index) {
  std::vector<::mediapipe_custom::Location> bboxes;
  ...
  for (int i = 0; i < xmins.size(); ++i) {
    bboxes.push_back(::mediapipe_custom::Location::CreateRelativeBBoxLocation(
...
void AddBBox(const std::string& prefix,
             const std::vector<::mediapipe_custom::Location>& bboxes,
             tensorflow::SequenceExample* sequence) {
...
std::unique_ptr<mediapipe_custom::Matrix> GetAudioFromFeatureAt(
    const std::string& prefix, const tensorflow::SequenceExample& sequence,
    int index) {
  ...
  auto output = absl::make_unique<mediapipe_custom::Matrix>(
...
void AddAudioAsFeature(const std::string& prefix,
                       const mediapipe_custom::Matrix& audio,
                       tensorflow::SequenceExample* sequence) {
  auto* value_list =
      MutableFeatureList(merge_prefix(prefix, kFeatureFloatsKey), sequence)
          ...
          ->mutable_float_list()
          ->mutable_value();
  mediapipe_custom::proto_ns::RepeatedField<float>(
