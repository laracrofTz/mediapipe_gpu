...
// mediapipe_custom::Packet; creating an api::Packet<T> that refers to an existing
...
#include "mediapipe_custom/framework/api2/tuple.h"
...
using Timestamp = mediapipe_custom::Timestamp;
using HolderBase = mediapipe_custom::packet_internal::HolderBase;
...
class PacketBase {
 public:
  ...
  operator mediapipe_custom::Packet() const& { return ToOldPacket(*this); }
  operator mediapipe_custom::Packet() && { return ToOldPacket(std::move(*this)); }
 ...
 protected:
  ...
  friend PacketBase FromOldPacket(const mediapipe_custom::Packet& op);
  friend PacketBase FromOldPacket(mediapipe_custom::Packet&& op);
  friend mediapipe_custom::Packet ToOldPacket(const PacketBase& p);
  friend mediapipe_custom::Packet ToOldPacket(PacketBase&& p);
};
...
using Generic = ::mediapipe_custom::api2::AnyType;
