...
#include "mediapipe_custom/calculators/image/bilateral_filter_calculator.pb.h"
...
class BilateralFilterCalculator : public CalculatorBase {
 ...
 private:
  ...
  mediapipe_custom::BilateralFilterCalculatorOptions options_;
...
absl::Status BilateralFilterCalculator::GetContract(CalculatorContract* cc) {
...
#if !MEDIAPIPE_DISABLE_GPU
  if (cc->Inputs().HasTag(kInputFrameTagGpu)) {
    cc->Inputs().Tag(kInputFrameTagGpu).Set<mediapipe_custom::GpuBuffer>();
    use_gpu |= true;
  }
#endif  // !MEDIAPIPE_DISABLE_GPU
  if (cc->Inputs().HasTag(kInputFrameTag)) {
  ...
  if (cc->Inputs().HasTag(kInputGuideTagGpu)) {
    cc->Inputs().Tag(kInputGuideTagGpu).Set<mediapipe_custom::GpuBuffer>();
    use_gpu |= true;
  }
#endif  // !MEDIAPIPE_DISABLE_GPU
  ...
  if (cc->Outputs().HasTag(kOutputFrameTagGpu)) {
    cc->Outputs().Tag(kOutputFrameTagGpu).Set<mediapipe_custom::GpuBuffer>();
    use_gpu |= true;
  }
#endif  // !MEDIAPIPE_DISABLE_GPU
...
absl::Status BilateralFilterCalculator::Open(CalculatorContext* cc) {
  ...
  options_ = cc->Options<mediapipe_custom::BilateralFilterCalculatorOptions>();
...
absl::Status BilateralFilterCalculator::RenderCpu(CalculatorContext* cc) {
  ...
  auto input_mat = mediapipe_custom::formats::MatView(&input_frame);
  ...
  if (has_guide_image) {
    ...
    auto output_mat = mediapipe_custom::formats::MatView(output_frame.get());
...
absl::Status BilateralFilterCalculator::RenderGpu(CalculatorContext* cc) {
  ...
  const auto& input_frame =
      cc->Inputs().Tag(kInputFrameTagGpu).Get<mediapipe_custom::GpuBuffer>();
  auto input_texture = gpu_helper_.CreateSourceTexture(input_frame);

  mediapipe_custom::GlTexture output_texture;
  ...
  if (has_guide_image) {
    ...
    const auto& guide_image =
        cc->Inputs().Tag(kInputGuideTagGpu).Get<mediapipe_custom::GpuBuffer>();
  ...
  auto output_frame = output_texture.GetFrame<mediapipe_custom::GpuBuffer>();
  cc->Outputs()
...
absl::Status BilateralFilterCalculator::GlSetup(CalculatorContext* cc) {
  ...
  const std::string frag_src =
      std::string(mediapipe_custom::kMediaPipeFragmentShaderPreamble) + R"(
  ...
  if (has_guide_image) {
    ...
    mediapipe_custom::GlhCreateProgram(
        mediapipe_custom::kBasicVertexShader, joint_frag_src.c_str(), NUM_ATTRIBUTES,
  ...
  glBufferData(GL_ARRAY_BUFFER, 4 * 2 * sizeof(GLfloat),
               mediapipe_custom::kBasicSquareVertices, GL_STATIC_DRAW);
  ...
  glBufferData(GL_ARRAY_BUFFER, 4 * 2 * sizeof(GLfloat),
               mediapipe_custom::kBasicTextureVertices, GL_STATIC_DRAW);
