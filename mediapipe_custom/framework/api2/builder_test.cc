...
#include "mediapipe_custom/framework/api2/tag.h"
...
using ::mediapipe_custom::api2::test::Bar;
using ::mediapipe_custom::api2::test::FloatAdder;
using ::mediapipe_custom::api2::test::Foo;
using ::mediapipe_custom::api2::test::Foo2;
using ::mediapipe_custom::api2::test::FooBar1;
...
TEST(BuilderTest, AnyAndSameTypeHandledProperly) {
  ...
  CalculatorGraphConfig expected = mediapipe_custom::ParseTextProtoOrDie<
...
TEST(GetOptionsTest, AddProto3Options) {
  ...
  auto& foo = graph.AddNode("Foo");
  foo.GetOptions<mediapipe_custom::SkyLightCalculatorOptions>();
  base >> foo.In("BASE");
  side >> foo.SideIn("SIDE");
  Stream<AnyType> foo_out = foo.Out("OUT");
...
TEST(GetOptionsTest, AddProto2Options) {
  ...
  auto& foo = graph.AddNode("Foo");
  foo.GetOptions<mediapipe_custom::NightLightCalculatorOptions>();
  base >> foo.In("BASE");
  side >> foo.SideIn("SIDE");
  Stream<AnyType> foo_out = foo.Out("OUT");
