...
#include "mediapipe_custom/calculators/util/packet_latency_calculator.pb.h"
...
absl::Status PacketLatencyCalculator::GetContract(CalculatorContract* cc) {
  ...
  if (cc->InputSidePackets().HasTag(kClockTag)) {
    cc->InputSidePackets()
        .Tag(kClockTag)
        .Set<std::shared_ptr<::mediapipe_custom::Clock>>();
  }
...
absl::Status PacketLatencyCalculator::Open(CalculatorContext* cc) {
  ...
  if (cc->InputSidePackets().HasTag(kClockTag)) {
    clock_ = cc->InputSidePackets()
                 .Tag(kClockTag)
                 .Get<std::shared_ptr<::mediapipe_custom::Clock>>();
  } else {
