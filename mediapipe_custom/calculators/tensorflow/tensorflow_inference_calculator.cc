...
#include "mediapipe_custom/calculators/tensorflow/tensorflow_inference_calculator.pb.h"
#include "mediapipe_custom/calculators/tensorflow/tensorflow_session.h"
...
#include "mediapipe_custom/framework/deps/monotonic_clock.h"
...
class TensorFlowInferenceCalculator : public CalculatorBase {
 public:
  ...
  TensorFlowInferenceCalculator() : session_(nullptr) {
    clock_ = std::unique_ptr<mediapipe_custom::Clock>(
        mediapipe_custom::MonotonicClock::CreateSynchronizedMonotonicClock());
  }
  ...
  static absl::Status GetContract(CalculatorContract* cc) {
    ...
    for (const std::string& tag : cc->Inputs().GetTags()) {
      ...
      if (!options.batched_input()) {
        ...
        cc->Inputs().Tag(tag).Set<std::vector<mediapipe_custom::Packet>>();
      }
    }
    ...
    // A mediapipe_custom::TensorFlowSession with a model loaded and ready for use.
  ...
  absl::Status Open(CalculatorContext* cc) override {
    ...
    for (const auto& tag_pair : options_.recurrent_tag_pair()) {
      ...
      RET_CHECK(mediapipe_custom::ContainsKey(tag_to_tensor_map_, tags[0]))
          << "Can't find tag '" << tags[0] << "' in signature "
          << options_.signature_name() << "; instead found tags "
          << absl::StrJoin(tag_to_tensor_map_, ", ", TagFormatter);
      RET_CHECK(mediapipe_custom::ContainsKey(tag_to_tensor_map_, tags[1]))
    ...
    for (const std::string& tag : cc->Inputs().GetTags()) {
      RET_CHECK(mediapipe_custom::ContainsKey(tag_to_tensor_map_, tag))
  ...
  absl::Status AggregateTensorPacket(
    ...
    if (mediapipe_custom::ContainsKey(recurrent_feed_tags_, tag_name)) {
    ...
    std::unique_ptr<InferenceState> inference_state_to_process;
    {
      ...
      for (const std::string& tag_as_node_name : cc->Inputs().GetTags()) {
        if (cc->Inputs().Tag(tag_as_node_name).IsEmpty()) {
          ...
          if (!mediapipe_custom::ContainsKey(recurrent_feed_tags_, tag_as_node_name)) {
  ...
  absl::Status OutputBatch(CalculatorContext* cc,
    ...
    std::vector<std::pair<mediapipe_custom::ProtoString, tf::Tensor>> input_tensors;
    ...
    for (auto& keyed_tensors : inference_state->input_tensor_batches_) {
      if (options_.batch_size() == 1) {
        ...
        if (!keyed_tensors.second.empty()) {
          ...
          RET_CHECK(
              mediapipe_custom::ContainsKey(recurrent_feed_tags_, keyed_tensors.first))
    ...
    std::vector<mediapipe_custom::ProtoString> output_tensor_names;
 ...
 private:
  ...
  std::unique_ptr<mediapipe_custom::Clock> clock_;
