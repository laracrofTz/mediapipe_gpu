...
#include "mediapipe_custom/framework/deps/status_macros.h"
...
inline StatusBuilder RetCheckImpl(const absl::Status& status,
  ...
  if (ABSL_PREDICT_TRUE(status.ok()))
    return mediapipe_custom::StatusBuilder(absl::OkStatus(), location);
...
#define RET_CHECK(cond)               \
  ...
  return mediapipe_custom::RetCheckFailSlowPath(MEDIAPIPE_LOC, #cond)

#define RET_CHECK_OK(status) \
  MP_RETURN_IF_ERROR(mediapipe_custom::RetCheckImpl((status), #status, MEDIAPIPE_LOC))

#define RET_CHECK_FAIL() return mediapipe_custom::RetCheckFailSlowPath(MEDIAPIPE_LOC)
