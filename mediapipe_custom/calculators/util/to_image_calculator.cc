...
#if MEDIAPIPE_DISABLE_GPU
// Just a placeholder to not have to depend on mediapipe_custom::GpuBuffer.
class Nothing {};
using GpuBuffer = Nothing;
#else
using GpuBuffer = mediapipe_custom::GpuBuffer;
#endif  // MEDIAPIPE_DISABLE_GPU
...
class ToImageCalculator : public Node {
 public:
  ...
  static constexpr Input<
      OneOf<mediapipe_custom::Image, mediapipe_custom::ImageFrame, GpuBuffer>>::Optional kIn{
      "IMAGE"};
  static constexpr Input<mediapipe_custom::ImageFrame>::Optional kInCpu{"IMAGE_CPU"};
  static constexpr Input<GpuBuffer>::Optional kInGpu{"IMAGE_GPU"};
  static constexpr Output<mediapipe_custom::Image> kOut{"IMAGE"};
...
absl::StatusOr<Packet<Image>> FromImageFrame(Packet<ImageFrame> packet) {
  ...
  return MakePacket<Image, std::shared_ptr<mediapipe_custom::ImageFrame>>(
      std::const_pointer_cast<mediapipe_custom::ImageFrame>(std::move(shared_ptr)));
}
...
absl::StatusOr<Packet<Image>> ToImageCalculator::GetInputImage(
  ...
  if (kIn(cc).IsConnected()) {
    return kIn(cc).Visit(
        [&](const mediapipe_custom::Image&) {
          return absl::StatusOr<Packet<Image>>(kIn(cc).As<Image>());
        },
        [&](const mediapipe_custom::ImageFrame&) {
