#include "mediapipe_custom/framework/profiler/reporter/reporter.h"
...
typedef std::map<std::pair<int64_t, int32_t>,
                 const mediapipe_custom::GraphTrace::CalculatorTrace*>
...
int64_t RecursePacketStartTime(
    ...
    const mediapipe_custom::GraphProfile& profile,
    const mediapipe_custom::GraphTrace::CalculatorTrace& trace,
...
void CacheNodeNameLookup(const mediapipe_custom::GraphProfile& profile,
                         NameLookup* result) {
...
void CacheOutputTraceLookup(const mediapipe_custom::GraphProfile& profile,
  ...
  for (const auto& graph_trace : profile.graph_trace()) {
    for (const auto& calc_trace : graph_trace.calculator_trace()) {
      if (calc_trace.event_type() != mediapipe_custom::GraphTrace_EventType_PROCESS) {
        continue;
      }
...
void Reporter::Accumulate(const mediapipe_custom::GraphProfile& profile) {
