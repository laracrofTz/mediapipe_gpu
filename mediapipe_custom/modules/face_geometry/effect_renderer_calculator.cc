...
#include "mediapipe_custom/framework/port/opencv_core_inc.h"       // NOTYPO
#include "mediapipe_custom/framework/port/opencv_imgcodecs_inc.h"  // NOTYPO
#include "mediapipe_custom/framework/port/opencv_imgproc_inc.h"    // NOTYPO
...
#include "mediapipe_custom/modules/face_geometry/effect_renderer_calculator.pb.h"
#include "mediapipe_custom/modules/face_geometry/libs/effect_renderer.h"
#include "mediapipe_custom/modules/face_geometry/libs/validation_utils.h"
#include "mediapipe_custom/modules/face_geometry/protos/environment.pb.h"
#include "mediapipe_custom/modules/face_geometry/protos/face_geometry.pb.h"
#include "mediapipe_custom/modules/face_geometry/protos/mesh_3d.pb.h"
...
class EffectRendererCalculator : public CalculatorBase {
 public:
  static absl::Status GetContract(CalculatorContract* cc) {
    ...
    return mediapipe_custom::GlCalculatorHelper::UpdateContract(cc);
  }
  ...
  absl::Status Process(CalculatorContext* cc) override {
    ...
    return gpu_helper_.RunInGlContext([this, cc]() -> absl::Status {
      ...
      cc->Outputs()
          .Tag(kImageGpuTag)
          .AddPacket(mediapipe_custom::Adopt<GpuBuffer>(output_gpu_buffer.release())
 ...
 private:
  static absl::StatusOr<ImageFrame> ReadTextureFromFile(
    ...
    MP_ASSIGN_OR_RETURN(std::unique_ptr<mediapipe_custom::Resource> texture_blob,
  ...
  static absl::StatusOr<face_geometry::Mesh3d> ReadMesh3dFromFile(
    ...
    MP_ASSIGN_OR_RETURN(std::unique_ptr<mediapipe_custom::Resource> mesh_3d_blob,
  ...
  static absl::StatusOr<std::unique_ptr<mediapipe_custom::Resource>>
  ReadContentBlobFromFile(CalculatorContext* cc,
                          const std::string& unresolved_path) {
    MP_ASSIGN_OR_RETURN(
        std::string resolved_path,
        mediapipe_custom::PathToResourceAsFile(unresolved_path),
