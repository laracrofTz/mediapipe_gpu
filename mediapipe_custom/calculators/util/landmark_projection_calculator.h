...
#include "mediapipe_custom/calculators/util/landmark_projection_calculator.pb.h"
...
struct LandmarkProjectionNode : Node<kLandmarkProjectionNodeName> {
  ...
  struct Contract {
    ...
    Repeated<Input<S, mediapipe_custom::NormalizedLandmarkList>> input_landmarks{
        "NORM_LANDMARKS"};
    ...
    Repeated<Output<S, mediapipe_custom::NormalizedLandmarkList>> output_landmarks{
        "NORM_LANDMARKS"};

    // Node options.
    Options<S, mediapipe_custom::LandmarkProjectionCalculatorOptions> options;
