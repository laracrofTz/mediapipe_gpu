...
#else
#include "mediapipe_custom/framework/profiler/graph_profiler_stub.h"
#endif

// Enabling this flag, will require specific platform implementation for the
// methods mediapipe_custom::PlatformSpecificTraceEventBegin() and
// mediapipe_custom::PlatformSpecificTraceEventEnd().
...
#define PLATFORM_SPECIFIC_PROFILER(name, id, packet_timestamp)       \
  mediapipe_custom::PlatformSpecificProfilingScope platform_specific_scope( \
...
#define PLATFORM_SPECIFIC_PROCESS_PROFILER(name, id, method_name,    \
                                           packet_timestamp)         \
  mediapipe_custom::PlatformSpecificProfilingScope platform_specific_scope( \
      name, id, packet_timestamp, mediapipe_custom::TraceEvent::method_name);
