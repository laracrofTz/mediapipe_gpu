...
#include "mediapipe_custom/calculators/util/annotation_overlay_calculator.pb.h"
...
#include "mediapipe_custom/framework/formats/image_opencv.h"
...
#include "mediapipe_custom/framework/port/vector.h"
#include "mediapipe_custom/util/annotation_renderer.h"
...
#include "mediapipe_custom/gpu/gpu_buffer_format.h"
...
// Future Image type.
inline bool HasImageTag(mediapipe_custom::CalculatorContext* cc) {
...
#if !MEDIAPIPE_DISABLE_GPU
  mediapipe_custom::GlCalculatorHelper gpu_helper_;
  GLuint program_ = 0;
...
absl::Status AnnotationOverlayCalculator::GetContract(CalculatorContract* cc) {
  ...
  if (cc->Inputs().HasTag(kGpuBufferTag)) {
    cc->Inputs().Tag(kGpuBufferTag).Set<mediapipe_custom::GpuBuffer>();
  ...
  if (cc->Inputs().HasTag(kImageTag)) {
    cc->Inputs().Tag(kImageTag).Set<mediapipe_custom::Image>();
  ...
  if (cc->Outputs().HasTag(kGpuBufferTag)) {
    cc->Outputs().Tag(kGpuBufferTag).Set<mediapipe_custom::GpuBuffer>();
  }
#endif  // !MEDIAPIPE_DISABLE_GPU
  ...
  if (cc->Outputs().HasTag(kImageTag)) {
    cc->Outputs().Tag(kImageTag).Set<mediapipe_custom::Image>();
  }

  if (use_gpu) {
#if !MEDIAPIPE_DISABLE_GPU
    MP_RETURN_IF_ERROR(mediapipe_custom::GlCalculatorHelper::UpdateContract(cc));
#endif  // !MEDIAPIPE_DISABLE_GPU
  }
...
absl::Status AnnotationOverlayCalculator::Process(CalculatorContext* cc) {
  ...
  if (HasImageTag(cc)) {
    use_gpu_ = cc->Inputs().Tag(kImageTag).Get<mediapipe_custom::Image>().UsesGpu();
  }
    ...
    if (!gpu_initialized_) {
      MP_RETURN_IF_ERROR(
          gpu_helper_.RunInGlContext([this, cc]() -> absl::Status {
            if (HasImageTag(cc)) {
              return GlSetup<mediapipe_custom::Image, kImageTag>(cc);
            }
            return GlSetup<mediapipe_custom::GpuBuffer, kGpuBufferTag>(cc);
          }));
      gpu_initialized_ = true;
    }
    if (HasImageTag(cc)) {
      MP_RETURN_IF_ERROR(
          (CreateRenderTargetGpu<mediapipe_custom::Image, kImageTag>(cc, image_mat)));
    }
    if (cc->Inputs().HasTag(kGpuBufferTag)) {
      MP_RETURN_IF_ERROR(
          (CreateRenderTargetGpu<mediapipe_custom::GpuBuffer, kGpuBufferTag>(
              cc, image_mat)));
    }
#endif  // !MEDIAPIPE_DISABLE_GPU
  } else {
  ...
  if (use_gpu_) {
    ...
    MP_RETURN_IF_ERROR(
        gpu_helper_.RunInGlContext([this, cc, image_mat_ptr]() -> absl::Status {
          if (HasImageTag(cc)) {
            return RenderToGpu<mediapipe_custom::Image, kImageTag>(cc, image_mat_ptr);
          }
          return RenderToGpu<mediapipe_custom::GpuBuffer, kGpuBufferTag>(
              cc, image_mat_ptr);
        }));
#endif  // !MEDIAPIPE_DISABLE_GPU
  } else {
...
absl::Status AnnotationOverlayCalculator::RenderToCpu(
  ...
  if (HasImageTag(cc)) {
    auto out = std::make_unique<mediapipe_custom::Image>(std::move(output_frame));
...
absl::Status AnnotationOverlayCalculator::RenderToGpu(CalculatorContext* cc,
  ...
  auto output_texture = gpu_helper_.CreateDestinationTexture(
      ...
      mediapipe_custom::GpuBufferFormat::kBGRA32);
...
absl::Status AnnotationOverlayCalculator::CreateRenderTargetCpuImage(
  ...
  if (image_frame_available_) {
    const auto& input_frame =
        cc->Inputs().Tag(kImageTag).Get<mediapipe_custom::Image>();
...
absl::Status AnnotationOverlayCalculator::CreateRenderTargetGpu(
  ...
  if (image_frame_available_) {
    const auto& input_frame = cc->Inputs().Tag(Tag).Get<Type>();
    const mediapipe_custom::ImageFormat::Format format =
        mediapipe_custom::ImageFormatForGpuBufferFormat(input_frame.format());
...
absl::Status AnnotationOverlayCalculator::GlSetup(CalculatorContext* cc) {
  ...
  const std::string frag_src = absl::StrCat(
      mediapipe_custom::kMediaPipeFragmentShaderPreamble, defines, kFragSrcBody);

  // Create shader program and set parameters
  mediapipe_custom::GlhCreateProgram(mediapipe_custom::kBasicVertexShader, frag_src.c_str(),
