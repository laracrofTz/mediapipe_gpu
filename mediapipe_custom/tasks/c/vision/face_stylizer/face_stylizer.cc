...
#include "mediapipe_custom/tasks/c/vision/face_stylizer/face_stylizer.h"
...
#include "mediapipe_custom/tasks/cc/vision/face_stylizer/face_stylizer.h"
#include "mediapipe_custom/tasks/cc/vision/utils/image_utils.h"

namespace mediapipe_custom::tasks::c::vision::face_stylizer {

namespace {

using ::mediapipe_custom::tasks::c::core::CppConvertToBaseOptions;
using ::mediapipe_custom::tasks::vision::CreateImageFromBuffer;
using ::mediapipe_custom::tasks::vision::face_stylizer::FaceStylizer;
...
FaceStylizer* CppFaceStylizerCreate(const FaceStylizerOptions& options,
  ...
  auto cpp_options = std::make_unique<
      ::mediapipe_custom::tasks::vision::face_stylizer::FaceStylizerOptions>();
...
void* face_stylizer_create(struct FaceStylizerOptions* options,
                           char** error_msg) {
  return mediapipe_custom::tasks::c::vision::face_stylizer::CppFaceStylizerCreate(
      *options, error_msg);
}

int face_stylizer_stylize_image(void* stylizer, const MpImage* image,
                                MpImage* result, char** error_msg) {
  return mediapipe_custom::tasks::c::vision::face_stylizer::CppFaceStylizerStylize(
      stylizer, image, result, error_msg);
}

void face_stylizer_close_result(MpImage* result) {
  mediapipe_custom::tasks::c::vision::face_stylizer::CppFaceStylizerCloseResult(
      result);
}

int face_stylizer_close(void* stylizer, char** error_ms) {
  return mediapipe_custom::tasks::c::vision::face_stylizer::CppFaceStylizerClose(
      stylizer, error_ms);
}
