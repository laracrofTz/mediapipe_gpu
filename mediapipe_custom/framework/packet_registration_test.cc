...
class TestSinkCalculator : public CalculatorBase {
 public:
  static absl::Status GetContract(CalculatorContract* cc) {
    cc->Inputs().Tag(kInTag).Set<mediapipe_custom::InputOnlyProto>();
    cc->Outputs().Tag(kOutTag).Set<int>();
    return absl::OkStatus();
  }

  absl::Status Process(CalculatorContext* cc) override {
    int x = cc->Inputs().Tag(kInTag).Get<mediapipe_custom::InputOnlyProto>().x();
...
TEST(PacketRegistrationTest, InputTypeRegistration) {
  ...
  ASSERT_EQ(mediapipe_custom::InputOnlyProto{}.GetTypeName(),
...
TEST(PacketRegistrationTest, AdoptingRegisteredProtoWorks) {
  ...
  {
    Graph graph;
    Stream<mediapipe_custom::InputOnlyProto> input =
        graph.In(0).SetName("in").Cast<mediapipe_custom::InputOnlyProto>();
  ...
  int value = 10;
  auto proto = std::make_unique<mediapipe_custom::InputOnlyProto>();
  proto->set_x(value);
