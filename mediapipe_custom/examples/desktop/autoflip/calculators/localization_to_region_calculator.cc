...
#include "mediapipe_custom/examples/desktop/autoflip/calculators/localization_to_region_calculator.pb.h"
...
class LocalizationToRegionCalculator : public mediapipe_custom::CalculatorBase {
 public:
...
void FillSalientRegion(const mediapipe_custom::Detection& detection,
...
absl::Status LocalizationToRegionCalculator::GetContract(
  ...
  cc->Inputs().Tag(kDetectionsTag).Set<std::vector<mediapipe_custom::Detection>>();
...
absl::Status LocalizationToRegionCalculator::Process(
  ...
  const auto& annotations =
      cc->Inputs().Tag(kDetectionsTag).Get<std::vector<mediapipe_custom::Detection>>();
