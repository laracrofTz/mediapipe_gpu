...
#include "mediapipe_custom/java/com/google/mediapipe/framework/jni/graph_texture_frame_jni.h"
...
using mediapipe_custom::GlTextureBufferSharedPtr;
...
JNIEXPORT jlong JNICALL GRAPH_TEXTURE_FRAME_METHOD(
  ...
  GlTextureBufferSharedPtr* buffer =
      reinterpret_cast<GlTextureBufferSharedPtr*>(nativeHandle);
  mediapipe_custom::GlSyncToken* token = nullptr;
  ...
  if (context_for_deletion) {
    auto sync = mediapipe_custom::GlContext::CreateSyncTokenForCurrentExternalContext(
    ...
    // null too.
    if (sync) {
      token = new mediapipe_custom::GlSyncToken(std::move(sync));
    }
  }
...
JNIEXPORT jlong JNICALL GRAPH_TEXTURE_FRAME_METHOD(
  ...
  return reinterpret_cast<jlong>(
      mediapipe_custom::GlContext::GetCurrentNativeContext());
}
...
JNIEXPORT void JNICALL GRAPH_TEXTURE_FRAME_METHOD(nativeDidRead)(
  ...
  GlTextureBufferSharedPtr* buffer =
      reinterpret_cast<GlTextureBufferSharedPtr*>(nativeHandle);
  mediapipe_custom::GlSyncToken& token =
      *reinterpret_cast<mediapipe_custom::GlSyncToken*>(consumerSyncToken);
