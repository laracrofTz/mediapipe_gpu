...
#include "mediapipe_custom/framework/counter_factory.h"
#include "mediapipe_custom/framework/delegating_executor.h"
#include "mediapipe_custom/framework/deps/clock.h"
#include "mediapipe_custom/framework/executor.h"
#include "mediapipe_custom/framework/graph_output_stream.h"
#include "mediapipe_custom/framework/graph_service_manager.h"
#include "mediapipe_custom/framework/input_stream_manager.h"
#include "mediapipe_custom/framework/mediapipe_profiling.h"
#include "mediapipe_custom/framework/output_side_packet_impl.h"
#include "mediapipe_custom/framework/output_stream_manager.h"
...
#include "mediapipe_custom/framework/packet_generator.pb.h"
#include "mediapipe_custom/framework/packet_set.h"
#include "mediapipe_custom/framework/packet_type.h"
#include "mediapipe_custom/framework/port.h"
...
#include "mediapipe_custom/framework/port/map_util.h"
#include "mediapipe_custom/framework/port/ret_check.h"
#include "mediapipe_custom/framework/port/source_location.h"
...
#include "mediapipe_custom/framework/scheduler.h"
#include "mediapipe_custom/framework/status_handler.h"
#include "mediapipe_custom/framework/status_handler.pb.h"
#include "mediapipe_custom/framework/thread_pool_executor.h"
#include "mediapipe_custom/framework/thread_pool_executor.pb.h"
#include "mediapipe_custom/framework/timestamp.h"
#include "mediapipe_custom/framework/tool/fill_packet_set.h"
#include "mediapipe_custom/framework/tool/status_util.h"
#include "mediapipe_custom/framework/tool/tag_map.h"
#include "mediapipe_custom/framework/tool/validate.h"
#include "mediapipe_custom/framework/tool/validate_name.h"
#include "mediapipe_custom/framework/validated_graph_config.h"
#include "mediapipe_custom/framework/vlog_overrides.h"
#include "mediapipe_custom/gpu/gpu_service.h"
#include "mediapipe_custom/gpu/graph_support.h"
#include "mediapipe_custom/util/cpu_util.h"
  ...
  if (cc != nullptr) {
    ...
    for (const auto& [key, packet] : parent_service_packets) {
      ...
      if (key == kGpuService.key) {
        ...
        auto resources = mediapipe_custom::GpuResources::Create(
            *packet.Get<std::shared_ptr<mediapipe_custom::GpuResources>>());
        ABSL_CHECK_OK(resources);
        service_packets[key] =
            MakePacket<std::shared_ptr<mediapipe_custom::GpuResources>>(*resources);
        continue;
      }
#endif  // !MEDIAPIPE_DISABLE_GPU
...
absl::Status CalculatorGraph::InitializeExecutors() {
  ...
  for (const ExecutorConfig& executor_config :
    ...
    if (mediapipe_custom::ContainsKey(executors_, executor_config.name())) {
      if (!executor_config.type().empty()) {
        return mediapipe_custom::InvalidArgumentErrorBuilder(MEDIAPIPE_LOC)
  ...
  if (!mediapipe_custom::ContainsKey(executors_, "")) {
...
absl::Status CalculatorGraph::ObserveOutputStream(
  ...
  if (output_stream_index < 0) {
    return mediapipe_custom::NotFoundErrorBuilder(MEDIAPIPE_LOC)
...
absl::StatusOr<Packet> CalculatorGraph::GetOutputSidePacket(
  ...
  if (output_packet.IsEmpty()) {
    ...
    if (base_packet_iter != base_packets.end() &&
      ...
      return mediapipe_custom::UnavailableErrorBuilder(MEDIAPIPE_LOC)
...
absl::Status CalculatorGraph::SetGpuResources(
    std::shared_ptr<::mediapipe_custom::GpuResources> resources) {
...
std::shared_ptr<::mediapipe_custom::GpuResources> CalculatorGraph::GetGpuResources()
    const {
...
absl::Status CalculatorGraph::MaybeSetUpGpuServiceFromLegacySidePacket(
  ...
  if (gpu_resources) {
    ABSL_LOG(WARNING)
        << "::mediapipe_custom::GpuSharedData provided as a side packet while the "
        << "graph already had one; ignoring side packet";
    return absl::OkStatus();
  }
  gpu_resources = legacy_sp.Get<::mediapipe_custom::GpuSharedData*>()->gpu_resources;
...
std::map<std::string, Packet> CalculatorGraph::MaybeCreateLegacyGpuSidePacket(
  ...
  if (gpu_resources &&
      (legacy_sp.IsEmpty() ||
       legacy_sp.Get<::mediapipe_custom::GpuSharedData*>()->gpu_resources !=
           gpu_resources)) {
    legacy_gpu_shared_ =
        std::make_unique<mediapipe_custom::GpuSharedData>(gpu_resources);
    additional_side_packets[kGpuSharedSidePacketName] =
        MakePacket<::mediapipe_custom::GpuSharedData*>(legacy_gpu_shared_.get());
  }
...
absl::Status CalculatorGraph::PrepareForRun(
  ...
  for (const auto& name_max : graph_input_stream_max_queue_size_) {
    std::unique_ptr<GraphInputStream>* stream =
        mediapipe_custom::FindOrNull(graph_input_streams_, name_max.first);
...
absl::StatusOr<GraphRuntimeInfo> CalculatorGraph::GetGraphRuntimeInfo() {
  ...
  const absl::Time time_now = mediapipe_custom::Clock::RealClock()->TimeNow();
...
absl::Status CalculatorGraph::SetInputStreamTimestampBound(
  ...
  std::unique_ptr<GraphInputStream>* stream =
      mediapipe_custom::FindOrNull(graph_input_streams_, stream_name);
  RET_CHECK(stream).SetNoLogging() << absl::Substitute(
...
absl::Status CalculatorGraph::AddPacketToInputStreamInternal(
  ...
  ABSL_CHECK_GE(node_id, validated_graph_->CalculatorInfos().size());
  {
    ...
    if (full_input_streams_.empty()) {
      return mediapipe_custom::FailedPreconditionErrorBuilder(MEDIAPIPE_LOC)
...
bool CalculatorGraph::HasInputStream(const std::string& stream_name) {
  return mediapipe_custom::FindOrNull(graph_input_streams_, stream_name) != nullptr;
}
...
void CalculatorGraph::CallStatusHandlers(GraphRunState graph_run_state,
  ...
  for (int status_handler_index = 0;
    ...
    if (!packet_set_statusor.ok()) {
      RecordError(mediapipe_custom::StatusBuilder(
    ...
    if (!handler_result.ok()) {
      mediapipe_custom::StatusBuilder builder(std::move(handler_result),
                                       MEDIAPIPE_LOC);
...

  {
    ...
    if (*stream_was_full != stream_is_full) {
      for (int node_id : *upstream_nodes) {
        ...
        mediapipe_custom::LogEvent(profiler_.get(),
...
absl::Status CalculatorGraph::SetExecutorInternal(
  ...
  if (!inserted) {
    ...
    return mediapipe_custom::AlreadyExistsErrorBuilder(MEDIAPIPE_LOC)
