...
#include "mediapipe_custom/calculators/image/image_cropping_calculator.h"
...
absl::Status ImageCroppingCalculator::GetContract(CalculatorContract* cc) {
  ...
  if (cc->Options<mediapipe_custom::ImageCroppingCalculatorOptions>()
          .has_norm_width() &&
      cc->Options<mediapipe_custom::ImageCroppingCalculatorOptions>()
          .has_norm_height()) {
    ++flags;
  }
  if (cc->Options<mediapipe_custom::ImageCroppingCalculatorOptions>().has_width() &&
      cc->Options<mediapipe_custom::ImageCroppingCalculatorOptions>().has_height()) {
    ++flags;
  }
...
absl::Status ImageCroppingCalculator::Open(CalculatorContext* cc) {
  ...
  options_ = cc->Options<mediapipe_custom::ImageCroppingCalculatorOptions>();
  output_max_width_ =
...
absl::Status ImageCroppingCalculator::ValidateBorderModeForGPU(
    CalculatorContext* cc) {
  mediapipe_custom::ImageCroppingCalculatorOptions options =
      cc->Options<mediapipe_custom::ImageCroppingCalculatorOptions>();

  switch (options.border_mode()) {
    case mediapipe_custom::ImageCroppingCalculatorOptions::BORDER_ZERO:
      ABSL_LOG(WARNING)
          << "BORDER_ZERO mode is not supported by GPU "
          << "implementation and will fall back into BORDER_REPLICATE";
      break;
    case mediapipe_custom::ImageCroppingCalculatorOptions::BORDER_REPLICATE:
      break;
    default:
