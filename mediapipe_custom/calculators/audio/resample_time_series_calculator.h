...
#include "mediapipe_custom/calculators/audio/resample_time_series_calculator.pb.h"
...
struct ResampleTimeSeriesCalculator : public mediapipe_custom::api2::NodeIntf {
  // Sequence of Matrices, each column describing a particular time frame, each
  // row a feature dimension, with TimeSeriesHeader.
  static constexpr mediapipe_custom::api2::Input<Matrix> kInput{""};
  static constexpr mediapipe_custom::api2::SideInput<double>::Optional
      kSideInputTargetSampleRate{"TARGET_SAMPLE_RATE"};
  // Sequence of Matrices, each column describing a particular time frame, each
  // row a feature dimension, with TimeSeriesHeader.
  static constexpr mediapipe_custom::api2::Output<Matrix> kOutput{""};
  MEDIAPIPE_NODE_INTERFACE(ResampleTimeSeriesCalculator, kInput, kOutput,
                           kSideInputTargetSampleRate,
                           mediapipe_custom::api2::TimestampChange::Arbitrary());
};
...
class ResampleTimeSeriesCalculatorImpl
    : public mediapipe_custom::api2::NodeImpl<ResampleTimeSeriesCalculator,
