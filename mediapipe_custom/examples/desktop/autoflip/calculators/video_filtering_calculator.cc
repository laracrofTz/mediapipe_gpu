...
#include "mediapipe_custom/examples/desktop/autoflip/calculators/video_filtering_calculator.pb.h"
...
absl::Status VideoFilteringCalculator::Process(CalculatorContext* cc) {
  ...
  cv::Mat frame_mat = mediapipe_custom::formats::MatView(&frame);
  ...
  if (options.fail_if_any()) {
    return mediapipe_custom::UnknownErrorBuilder(MEDIAPIPE_LOC) << absl::Substitute(
