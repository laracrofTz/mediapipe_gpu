...
#include "mediapipe_custom/gpu/gpu_buffer_storage_image_frame.h"
#include "mediapipe_custom/gpu/image_frame_view.h"
...
class Image {
 public:
  ...
  explicit Image(CFHolder<CVPixelBufferRef> pixel_buffer)
      : Image(mediapipe_custom::GpuBuffer(std::move(pixel_buffer))) {}
  explicit Image(CVPixelBufferRef pixel_buffer)
      : Image(mediapipe_custom::GpuBuffer(pixel_buffer)) {}
#else
  explicit Image(mediapipe_custom::GlTextureBufferSharedPtr texture_buffer)
      : Image(mediapipe_custom::GpuBuffer(std::move(texture_buffer))) {}
#endif  // MEDIAPIPE_GPU_BUFFER_USE_CV_PIXEL_BUFFER
  explicit Image(mediapipe_custom::GpuBuffer gpu_buffer) {
    use_gpu_ = true;
  ...
  CVPixelBufferRef GetCVPixelBufferRef() const {
    ...
    return mediapipe_custom::GetCVPixelBufferRef(gpu_buffer_);
  }
#else
  mediapipe_custom::GlTextureBufferSharedPtr GetGlTextureBufferSharedPtr() const {
    if (use_gpu_ == false) ConvertToGpu();
    return gpu_buffer_.internal_storage<mediapipe_custom::GlTextureBuffer>();
  }
#endif  // MEDIAPIPE_GPU_BUFFER_USE_CV_PIXEL_BUFFER
#endif  // !MEDIAPIPE_DISABLE_GPU
  ...
  const mediapipe_custom::GpuBuffer GetGpuBuffer(bool upload_to_gpu = true) const {
  ...
  mediapipe_custom::GpuBufferFormat format() const;
 ...
 private:
  mutable mediapipe_custom::GpuBuffer gpu_buffer_;
...
inline ImageFormat::Format Image::image_format() const {
  return mediapipe_custom::ImageFormatForGpuBufferFormat(gpu_buffer_.format());
}

inline mediapipe_custom::GpuBufferFormat Image::format() const {
