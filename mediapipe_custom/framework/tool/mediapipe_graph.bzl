...
Example:
  mediapipe_binary_graph(
    ...
    graph = "//mediapipe_custom/framework/tool/testdata:test_graph",
...
load("//mediapipe_custom/framework:encode_binary_proto.bzl", "encode_binary_proto", "generate_proto_descriptor_set")
load("//mediapipe_custom/framework:transitive_protos.bzl", "transitive_proto_cc_libs", "transitive_proto_descriptor_sets", "transitive_protos")
load("//mediapipe_custom/framework/deps:expand_template.bzl", "expand_template")
load("//mediapipe_custom/framework/tool:build_defs.bzl", "clean_dep")

# buildifier: disable=out-of-order-load
# buildifier: disable=same-origin-load
load("//mediapipe_custom/framework/deps:descriptor_set.bzl", "direct_descriptor_set", "transitive_descriptor_set")
...
def mediapipe_binary_graph(name, graph = None, output_name = None, deps = [], testonly = None, compatible_with = None, **kwargs):
    ...
    generate_proto_descriptor_set(
        ...
        deps = [
            name + "_gather_protos",
            clean_dep("//mediapipe_custom/framework:calculator_proto"),
        ],
        testonly = testonly,
        compatible_with = compatible_with,
    )
    encode_binary_proto(
        ...
        deps = [
            name + "_gather_protos",
            clean_dep("//mediapipe_custom/framework:calculator_proto"),
            clean_dep("//mediapipe_custom/framework/stream_handler:fixed_size_input_stream_handler_proto"),
        ],
...
def data_as_c_string(
    ...
    if outs == None:
        outs = [name]
    encode_as_c_string = clean_dep("//mediapipe_custom/framework/tool:encode_as_c_string")
    native.genrule(
        name = name,
        srcs = srcs,
        outs = outs,
...
def mediapipe_simple_subgraph(
    ...
    expand_template(
        ...
        template = clean_dep("//mediapipe_custom/framework/tool:simple_subgraph_template.cc"),
        out = name + "_linked.cc",
        substitutions = {
    ...
    if not tflite_deps:
        cc_library(
            ...
            deps = [
                clean_dep("//mediapipe_custom/framework:calculator_framework"),
                clean_dep("//mediapipe_custom/framework:subgraph"),
            ] + deps,
            alwayslink = 1,
...
def mediapipe_options_library(
    ...
    native.genrule(
        ...
        cmd = ("$(location " + "//mediapipe_custom/framework/tool:message_type_util" + ") " +
        ...
        tools = ["//mediapipe_custom/framework/tool:message_type_util"],
        visibility = visibility,
        testonly = testonly,
        compatible_with = compatible_with,
    )
    expand_template(
        name = name + "_cc",
        template = clean_dep("//mediapipe_custom/framework/tool:options_lib_template.cc"),
        out = name + ".cc",
        substitutions = {
    ...
    cc_library(
        ...
        deps = [
            clean_dep("//mediapipe_custom/framework:calculator_framework"),
            clean_dep("//mediapipe_custom/framework/port:advanced_proto"),
            clean_dep("//mediapipe_custom/framework/tool:options_registry"),
