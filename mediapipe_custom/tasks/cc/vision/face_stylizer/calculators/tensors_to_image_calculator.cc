...
#include "mediapipe_custom/calculators/tensor/image_to_tensor_utils.h"
...
#include "mediapipe_custom/framework/formats/tensor_mtl_buffer_view.h"
...
using ::mediapipe_custom::api2::Node;
...
//
// Input streams:
//   TENSORS - std::vector<mediapipe_custom::Tensor> that only contains one element.
//
// Output streams:
//   OUTPUT - mediapipe_custom::Image.
...
#else
  GLuint program_ = 0;
  std::unique_ptr<mediapipe_custom::QuadRenderer> gl_renderer_;
#endif  // MEDIAPIPE_OPENGL_ES_VERSION >= MEDIAPIPE_OPENGL_ES_31
#endif  // MEDIAPIPE_METAL_ENABLED
#endif  // !MEDIAPIPE_DISABLE_GPU
};
MEDIAPIPE_REGISTER_NODE(::mediapipe_custom::tasks::TensorsToImageCalculator);
...
absl::Status TensorsToImageCalculator::CpuProcess(CalculatorContext* cc) {
  ...
  auto format = mediapipe_custom::ImageFormat::SRGB;
  ...
  cv::Mat output_matview = mediapipe_custom::formats::MatView(output_frame.get());
...
absl::Status TensorsToImageCalculator::MetalProcess(CalculatorContext* cc) {
  ...
  auto input_view =
      mediapipe_custom::MtlBufferView::GetReadView(input_tensor, command_buffer);
  [compute_encoder setBuffer:input_view.buffer() offset:0 atIndex:0];

  mediapipe_custom::GpuBuffer output =
...
absl::Status TensorsToImageCalculator::MetalSetup(CalculatorContext* cc) {
  ...
  return mediapipe_custom::OkStatus();
}
...
absl::Status TensorsToImageCalculator::GlSetup(CalculatorContext* cc) {
  ...
  if (options.gpu_origin() != mediapipe_custom::GpuOrigin::TOP_LEFT) {
  ...
  const std::string src =
      absl::StrCat(mediapipe_custom::kMediaPipeFragmentShaderPreamble,
                   kFragColorOutputDeclaration, maybe_flip_y_define, kBody);
  gl_renderer_ = std::make_unique<mediapipe_custom::QuadRenderer>();
...
absl::Status TensorsToImageCalculator::GlProcess(CalculatorContext* cc) {
  ...
  return gl_helper_.RunInGlContext([this, cc]() -> absl::Status {
    ...
    auto texture_buffer = mediapipe_custom::GlTextureBuffer::Wrap(
        ...
        [ptr = out_texture.release()](
            std::shared_ptr<mediapipe_custom::GlSyncPoint> sync_token) mutable {
          delete ptr;
        });

    auto output =
        std::make_unique<mediapipe_custom::GpuBuffer>(std::move(texture_buffer));
    kOutputImage(cc).Send(Image(*output));
    ...
    MP_RETURN_IF_ERROR(gl_renderer_->GlRender(
        ...
        output_texture.height(), mediapipe_custom::FrameScaleMode::kStretch,
        mediapipe_custom::FrameRotation::kNone,
