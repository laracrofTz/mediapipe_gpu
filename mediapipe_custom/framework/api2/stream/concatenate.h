...
#include "mediapipe_custom/calculators/core/concatenate_vector_calculator.pb.h"
...
GenericNode& AddConcatenateVectorNode(Graph& graph) {
  if constexpr (std::is_same_v<T, mediapipe_custom::LandmarkList>) {
...
Stream<PayloadT> Concatenate(StreamsT& streams,
  ...
  auto& concatenator_opts =
      concatenator
          .template GetOptions<mediapipe_custom::ConcatenateVectorCalculatorOptions>();
