...
#include "mediapipe_custom/framework/port/aligned_malloc_and_free.h"
#include "mediapipe_custom/framework/port/port.h"
...
void RescaleImageFrame(const ImageFrame& source_frame, const int width,
  ...
  cv::Mat source_mat = ::mediapipe_custom::formats::MatView(&source_frame);
  destination_frame->Reset(source_frame.Format(), width, height,
                           alignment_boundary);
  cv::Mat destination_mat = ::mediapipe_custom::formats::MatView(destination_frame);
...
void SrgbToMpegYCbCr(const uint8_t r, const uint8_t g, const uint8_t b,  //
  ...
  *y = static_cast<uint8_t>(
      mediapipe_custom::MathUtil::FastIntRound(16.0 +                 //
                                        65.481 * r / 255.0 +   //
                                        128.553 * g / 255.0 +  //
                                        24.966 * b / 255.0));
  *cb = static_cast<uint8_t>(
      mediapipe_custom::MathUtil::FastIntRound(128.0 +                //
                                        -37.797 * r / 255.0 +  //
                                        -74.203 * g / 255.0 +  //
                                        112.0 * b / 255.0));
...
void MpegYCbCrToSrgb(const uint8_t y, const uint8_t cb, const uint8_t cr,  //
  ...
  // to an sRGB value.
  *r = mediapipe_custom::MathUtil::SafeRound<uint8_t, double>(  //
      255.0 / 219.0 * (y - 16.0) +                       //
      255.0 / 112.0 * 0.701 * (cr - 128.0));
  *g = mediapipe_custom::MathUtil::SafeRound<uint8_t, double>(
      255.0 / 219.0 * (y - 16.0) -                            //
      255.0 / 112.0 * 0.886 * 0.114 / 0.587 * (cb - 128.0) -  //
      255.0 / 112.0 * 0.701 * 0.299 / 0.587 * (cr - 128.0));
  *b = mediapipe_custom::MathUtil::SafeRound<uint8_t, double>(  //
      255.0 / 219.0 * (y - 16.0) +                       //
