...
#         exclude=['mediapipe_custom.examples.desktop.*', 'mediapipe_custom.model_maker.*']),
...
# CRITICAL: Update all paths to use mediapipe_custom
MP_DIR_INIT_PY = os.path.join(MP_ROOT_PATH, 'mediapipe_custom/__init__.py')
MP_THIRD_PARTY_BUILD = os.path.join(MP_ROOT_PATH, 'third_party/BUILD')
MP_ROOT_INIT_PY = os.path.join(MP_ROOT_PATH, '__init__.py')
...
def _add_mp_init_files():
  """Add __init__.py to mediapipe_custom root directories."""
  open(MP_ROOT_INIT_PY, 'w').close()
  # Save the original mediapipe_custom/__init__.py file.
  shutil.copyfile(MP_DIR_INIT_PY, _get_backup_file(MP_DIR_INIT_PY))
  mp_dir_init_file = open(MP_DIR_INIT_PY, 'a')
  mp_dir_init_file.writelines([
      '\n', 'from mediapipe_custom.python import *\n',
...
class GeneratePyProtos(build_ext.build_ext):
  ...
  def run(self):
    ...
    # Add __init__.py to mediapipe_custom proto directories
    proto_dirs = ['mediapipe_custom/calculators'] + [
        x[0] for x in os.walk('mediapipe_custom/modules')
    ] + [x[0] for x in os.walk('mediapipe_custom/tasks/cc')]
    ...
    # Build framework and calculator py protos with updated paths
    for pattern in [
        'mediapipe_custom/framework/**/*.proto', 
        'mediapipe_custom/calculators/**/*.proto',
        'mediapipe_custom/gpu/**/*.proto', 
        'mediapipe_custom/modules/**/*.proto',
        'mediapipe_custom/tasks/cc/**/*.proto', 
        'mediapipe_custom/util/**/*.proto'
    ]:
      for proto_file in glob.glob(pattern, recursive=True):
        if proto_file.endswith('test.proto'):
          continue
...
class BuildModules(build_ext.build_ext):
  ...
  def run(self):
    _check_bazel()
    # Update external file paths to use mediapipe_custom
    external_files = [
        'face_detection/face_detection_full_range_sparse.tflite',
        'face_detection/face_detection_short_range.tflite',
        'face_landmark/face_landmark.tflite',
    ...
    for elem in external_files:
      external_file = os.path.join('mediapipe_custom/modules/', elem)
      sys.stderr.write('downloading file: %s\n' % external_file)
      self._download_external_file(external_file)
    ...
    for elem in binary_graphs:
      binary_graph = os.path.join('mediapipe_custom/modules/', elem)
      sys.stderr.write('generating binarypb: %s\n' % binary_graph)
      self._generate_binary_graph(binary_graph)
  ...
  def run(self):
    for target in [
      ...
      bazel_command = [
          ...
          '--compilation_mode=opt',
          '--action_env=PYTHON_BIN_PATH=' + _normalize_path(sys.executable),
          '//mediapipe_custom/tasks/metadata:' + target,  # Updated path
      ] + GPU_OPTIONS

      _invoke_shell_command(bazel_command)
      _copy_to_build_lib_dir(
          self.build_lib,
          'mediapipe_custom/tasks/metadata/' + target + '_generated.py')
    
    for schema_file in [
        'mediapipe_custom/tasks/metadata/metadata_schema.fbs',
        'mediapipe_custom/tasks/metadata/object_detector_metadata_schema.fbs',
        'mediapipe_custom/tasks/metadata/image_segmenter_metadata_schema.fbs',
    ]:
      shutil.copyfile(schema_file,
                      os.path.join(self.build_lib + '/', schema_file))
...
class Restore(setuptools.Command):
  """Restore the modified mediapipe_custom source files."""
  ...
  def run(self):
    ...
    if os.path.exists(_get_backup_file(MP_THIRD_PARTY_BUILD)):
      os.remove(MP_THIRD_PARTY_BUILD)
      shutil.move(_get_backup_file(MP_THIRD_PARTY_BUILD), MP_THIRD_PARTY_BUILD)
    if os.path.exists(MP_ROOT_INIT_PY):
      os.remove(MP_ROOT_INIT_PY)

setuptools.setup(
    name='mediapipe-custom',  # Use hyphen for PyPI package name
    version=MP_VERSION,
    url='https://github.com/google/mediapipe',
    description='Custom MediaPipe package with modifications',
    author='The MediaPipe Authors (Modified)',
    ...
    ext_modules=[
        # Updated Bazel targets to use mediapipe_custom paths
        BazelExtension('//mediapipe_custom/python:_framework_bindings'),
        BazelExtension(
            '//mediapipe_custom/tasks/cc/metadata/python:_pywrap_metadata_version'),
        BazelExtension(
            '//mediapipe_custom/tasks/python/metadata/flatbuffers_lib:_pywrap_flatbuffers'
        ),
    ],
    zip_safe=False,
    include_package_data=True,
