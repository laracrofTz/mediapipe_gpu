...
#include "mediapipe_custom/framework/deps/status.h"
...
class ABSL_MUST_USE_RESULT StatusBuilder {
 public:
  ...
  // enabled, it will use `location` as the location from which the log message
  // occurs.  A typical user will call this with `MEDIAPIPE_LOC`.
  StatusBuilder(absl::StatusCode code, mediapipe_custom::source_location location)
 ...
 private:
  struct Impl {
    ...
    Impl(const absl::Status& status, mediapipe_custom::source_location location);
    Impl(absl::Status&& status, mediapipe_custom::source_location location);
    Impl(const Impl&);
...
inline StatusBuilder InternalErrorBuilder(mediapipe_custom::source_location location) {
...
inline StatusBuilder NotFoundErrorBuilder(mediapipe_custom::source_location location) {
...
inline StatusBuilder UnknownErrorBuilder(mediapipe_custom::source_location location) {
