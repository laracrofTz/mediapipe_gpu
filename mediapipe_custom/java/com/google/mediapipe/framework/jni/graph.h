...
#if !MEDIAPIPE_DISABLE_GPU
  // Gets the shared mediapipe_custom::GpuResources. Only valid once the graph is
  // running.
  mediapipe_custom::GpuResources* GetGpuResources() const;
#endif  // !MEDIAPIPE_DISABLE_GPU
  // Adds a surface output for a given stream name.
  // Multiple outputs can be attached to the same stream.
  // Returns a native packet handle for the mediapipe_custom::EglSurfaceHolder, or 0 in
  // case of failure.
...
#if !MEDIAPIPE_DISABLE_GPU
  // mediapipe_custom::GpuResources used by the graph.
  // Note: this class does not create a CalculatorGraph until StartRunningGraph
  // is called, and we may have to create the mediapipe_custom::GpuResources before
  // that time, e.g. before a SurfaceOutput is associated with a Surface.
  std::shared_ptr<mediapipe_custom::GpuResources> gpu_resources_;
#endif  // !MEDIAPIPE_DISABLE_GPU
