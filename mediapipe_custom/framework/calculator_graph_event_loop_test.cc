...
TEST_F(CalculatorGraphEventLoopTest, UnthrottleSources) {
  ...
  mediapipe_custom::OutputStreamPoller& poller = poller_status.value();
  ...
  // Start capturing VLOG messages from the mediapipe_custom::Scheduler.
  TextMessageLogSink log_listener;
  mediapipe_custom::AddLogSink(&log_listener);
  ...
  mediapipe_custom::RemoveLogSink(&log_listener);
