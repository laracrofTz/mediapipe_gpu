...
#include "mediapipe_custom/calculators/tensorflow/tensorflow_session_from_frozen_graph_calculator.pb.h"
...
class TensorFlowSessionFromFrozenGraphCalculator : public CalculatorBase {
 public:
  ...
  absl::Status Open(CalculatorContext* cc) override {
    auto clock = std::unique_ptr<mediapipe_custom::Clock>(
    ...
    std::vector<mediapipe_custom::ProtoString> initialization_op_names;
    ...
    if (cc->InputSidePackets().HasTag(kStringModelTag)) {
      ...
      const std::string& frozen_graph = cc->InputSidePackets()
                                            ...
                                            .Get<std::string>();
      RET_CHECK_OK(
          mediapipe_custom::file::GetContents(frozen_graph, &graph_def_serialized));
    } else {
      RET_CHECK_OK(mediapipe_custom::file::GetContents(options.graph_proto_path(),
                                                &graph_def_serialized));
    }
