...
#include "mediapipe_custom/framework/formats/image_file_properties.pb.h"
...
#include "mediapipe_custom/graphs/iris_tracking/calculators/iris_to_depth_calculator.pb.h"
...
class IrisToDepthCalculator : public CalculatorBase {
 ...
 private:
  ...
  void GetRightIris(const NormalizedLandmarkList& lds,
                    NormalizedLandmarkList* iris);
  ::mediapipe_custom::IrisToDepthCalculatorOptions options_;
};
...
absl::Status IrisToDepthCalculator::Open(CalculatorContext* cc) {
  ...
  options_ = cc->Options<::mediapipe_custom::IrisToDepthCalculatorOptions>();
  return absl::OkStatus();
}
