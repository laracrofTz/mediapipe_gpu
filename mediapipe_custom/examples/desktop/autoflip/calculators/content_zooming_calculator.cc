...
#include "mediapipe_custom/examples/desktop/autoflip/autoflip_messages.pb.h"
#include "mediapipe_custom/examples/desktop/autoflip/calculators/content_zooming_calculator.pb.h"
#include "mediapipe_custom/examples/desktop/autoflip/calculators/content_zooming_calculator_state.h"
...
#include "mediapipe_custom/framework/formats/location_data.pb.h"
#include "mediapipe_custom/framework/packet.h"
#include "mediapipe_custom/framework/port/ret_check.h"
#include "mediapipe_custom/framework/port/status_builder.h"
...
class ContentZoomingCalculator : public CalculatorBase {
 public:
  ...
  static absl::Status GetContract(mediapipe_custom::CalculatorContract* cc);
  absl::Status Open(mediapipe_custom::CalculatorContext* cc) override;
  absl::Status Process(mediapipe_custom::CalculatorContext* cc) override;
  absl::Status Close(mediapipe_custom::CalculatorContext* cc) override;
 ...
 private:
  ...
  absl::Status MaybeLoadState(mediapipe_custom::CalculatorContext* cc, int frame_width,
                              int frame_height);
  // Saves state to a state-cache, if provided.
  absl::Status SaveState(mediapipe_custom::CalculatorContext* cc) const;
  // Returns the factor for maximum zoom based on options and the
  // kMaxZoomFactorPercent input (if present).
  double GetMaxZoomFactor(mediapipe_custom::CalculatorContext* cc) const;
  // Returns the factor for scale based on options and the
  // kScaleFactorPercent input (if present).
  double GetScaleFactor(mediapipe_custom::CalculatorContext* cc) const;
  // Initializes the calculator for the given frame size, creating path solvers
  // and resetting history like last measured values.
  absl::Status InitializeState(mediapipe_custom::CalculatorContext* cc,
                               int frame_width, int frame_height);
  // Adjusts state to work with an updated frame size.
  absl::Status UpdateForResolutionChange(mediapipe_custom::CalculatorContext* cc,
                                         int frame_width, int frame_height);
  ...
  absl::StatusOr<mediapipe_custom::Rect> GetAnimationRect(
  ...
  absl::Status GetDetectionsBox(mediapipe_custom::CalculatorContext* cc, float* xmin,
  ...
  mediapipe_custom::NormalizedRect first_rect_;
...
absl::Status ContentZoomingCalculator::GetContract(
    mediapipe_custom::CalculatorContract* cc) {
  RET_CHECK(
  ...
  if (cc->Inputs().HasTag(kVideoFrame)) {
    ...
    return mediapipe_custom::UnknownErrorBuilder(MEDIAPIPE_LOC)
  ...
  if (cc->Inputs().HasTag(kDetections)) {
    cc->Inputs().Tag(kDetections).Set<std::vector<mediapipe_custom::Detection>>();
  }
  ...
  if (cc->Outputs().HasTag(kCropRect)) {
    cc->Outputs().Tag(kCropRect).Set<mediapipe_custom::Rect>();
  }
  if (cc->Outputs().HasTag(kNormalizedCropRect)) {
    cc->Outputs().Tag(kNormalizedCropRect).Set<mediapipe_custom::NormalizedRect>();
  }
  if (cc->Outputs().HasTag(kFirstCropRect)) {
    cc->Outputs().Tag(kFirstCropRect).Set<mediapipe_custom::NormalizedRect>();
  }
...
absl::Status ContentZoomingCalculator::Open(mediapipe_custom::CalculatorContext* cc) {
...
absl::Status ContentZoomingCalculator::Close(mediapipe_custom::CalculatorContext* cc) {
  if (initialized_) {
...
namespace {
mediapipe_custom::LocationData::RelativeBoundingBox ShiftDetection(
    const mediapipe_custom::LocationData::RelativeBoundingBox& relative_bounding_box,
...
mediapipe_custom::autoflip::RectF ShiftDetection(
    const mediapipe_custom::autoflip::RectF& relative_bounding_box,
    const float y_offset_percent, const float x_offset_percent) {
  auto shifted_bb = relative_bounding_box;
...
absl::Status UpdateRanges(const mediapipe_custom::Detection& detection,
                          const float shift_vertical,
                          const float shift_horizontal,
                          const float pad_vertical, const float pad_horizontal,
                          float* xmin, float* xmax, float* ymin, float* ymax) {
  RET_CHECK(detection.location_data().format() ==
            mediapipe_custom::LocationData::RELATIVE_BOUNDING_BOX)
...
absl::Status GetVideoResolution(mediapipe_custom::CalculatorContext* cc,
                                int* frame_width, int* frame_height) {
  if (cc->Inputs().HasTag(kVideoFrame)) {
...
absl::Status ContentZoomingCalculator::MaybeLoadState(
    mediapipe_custom::CalculatorContext* cc, int frame_width, int frame_height) {
...
absl::Status ContentZoomingCalculator::SaveState(
    mediapipe_custom::CalculatorContext* cc) const {
  auto* state_cache =
      cc->InputSidePackets().HasTag(kStateCache)
          ? cc->InputSidePackets().Tag(kStateCache).Get<StateCacheType*>()
          : nullptr;
...
absl::StatusOr<mediapipe_custom::Rect> ContentZoomingCalculator::GetAnimationRect(
    int frame_width, int frame_height, const Timestamp& timestamp) const {
  ...
  mediapipe_custom::Rect gpu_rect;
...
absl::Status ContentZoomingCalculator::Process(
    mediapipe_custom::CalculatorContext* cc) {
  ...
  if (cc->Outputs().HasTag(kCropRect)) {
    std::unique_ptr<mediapipe_custom::Rect> gpu_rect;
    if (is_animating) {
      auto rect =
          GetAnimationRect(frame_width, frame_height, cc->InputTimestamp());
      MP_RETURN_IF_ERROR(rect.status());
      gpu_rect = absl::make_unique<mediapipe_custom::Rect>(*rect);
    } else {
      gpu_rect = absl::make_unique<mediapipe_custom::Rect>();
  ...
  if (cc->Outputs().HasTag(kNormalizedCropRect)) {
    std::unique_ptr<mediapipe_custom::NormalizedRect> gpu_rect =
        absl::make_unique<mediapipe_custom::NormalizedRect>();
  ...
  if (cc->Outputs().HasTag(kFirstCropRect)) {
    cc->Outputs()
        .Tag(kFirstCropRect)
        .Add(new mediapipe_custom::NormalizedRect(first_rect_),
             Timestamp(cc->InputTimestamp()));
  }
...
absl::Status ContentZoomingCalculator::GetDetectionsBox(
    mediapipe_custom::CalculatorContext* cc, float* xmin, float* xmax, float* ymin,
  ...
  if (cc->Inputs().HasTag(kDetections)) {
    if (cc->Inputs().Tag(kDetections).IsEmpty()) {
      if (last_only_required_detection_ == 0) {
        ...
        if (cc->Outputs().HasTag(kCropRect)) {
          auto default_rect = absl::make_unique<mediapipe_custom::Rect>();
        ...
        if (cc->Outputs().HasTag(kNormalizedCropRect)) {
          auto default_rect = absl::make_unique<mediapipe_custom::NormalizedRect>();
        ...
        if (cc->Outputs().HasTag(kFirstCropRect)) {
          cc->Outputs()
              .Tag(kFirstCropRect)
              .Add(new mediapipe_custom::NormalizedRect(),
                   Timestamp(cc->InputTimestamp()));
        }
      ...
      auto raw_detections = cc->Inputs()
                                .Tag(kDetections)
                                .Get<std::vector<mediapipe_custom::Detection>>();
