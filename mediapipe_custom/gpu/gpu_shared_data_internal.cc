...
#include "mediapipe_custom/gpu/gl_context_options.pb.h"
#include "mediapipe_custom/gpu/graph_support.h"
#include "mediapipe_custom/gpu/multi_pool.h"

#if MEDIAPIPE_METAL_ENABLED
#include "mediapipe_custom/gpu/metal_shared_resources.h"
...
static const std::string& SharedContextKey() {
  static const mediapipe_custom::NoDestructor<std::string> kSharedContextKey("");
...
absl::Status GpuResources::PrepareGpuNode(CalculatorNode* node) {
  ...
  const auto& options =
      node->GetCalculatorState().Options<mediapipe_custom::GlContextOptions>();
