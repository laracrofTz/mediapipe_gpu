...
#include "mediapipe_custom/java/com/google/mediapipe/framework/jni/packet_getter_jni.h"
...
#include "mediapipe_custom/framework/port/proto_ns.h"
...
const T& GetFromNativeHandle(int64_t packet_handle) {
  return mediapipe_custom::android::Graph::GetPacketFromHandle(packet_handle).Get<T>();
}

bool CopyImageDataToByteBuffer(JNIEnv* env, const mediapipe_custom::ImageFrame& image,
                               jobject byte_buffer) {
...
void CheckImageSizeInImageList(JNIEnv* env,
                               const std::vector<mediapipe_custom::Image>& image_list,
...
JNIEXPORT jlong JNICALL PACKET_GETTER_METHOD(nativeGetPacketFromReference)(
  ...
  mediapipe_custom::Packet mediapipe_packet =
      mediapipe_custom::android::Graph::GetPacketFromHandle(packet)
          .Get<std::unique_ptr<mediapipe_custom::SyncedPacket>>()
          ->Get();
  auto mediapipe_graph =
      mediapipe_custom::android::Graph::GetContextFromHandle(packet);
...
JNIEXPORT jlongArray JNICALL PACKET_GETTER_METHOD(nativeGetPairPackets)(
  ...
  auto pair_packets =
      GetFromNativeHandle<std::pair<mediapipe_custom::Packet, mediapipe_custom::Packet>>(
          packet);
  auto mediapipe_graph =
      mediapipe_custom::android::Graph::GetContextFromHandle(packet);
  int64_t handles[2];
...
JNIEXPORT jlongArray JNICALL PACKET_GETTER_METHOD(nativeGetVectorPackets)(
    JNIEnv* env, jobject thiz, jlong packet) {
  auto vector_packets =
      GetFromNativeHandle<std::vector<mediapipe_custom::Packet>>(packet);
  auto mediapipe_graph =
      mediapipe_custom::android::Graph::GetContextFromHandle(packet);
...
JNIEXPORT jobjectArray JNICALL PACKET_GETTER_METHOD(nativeGetProtoVector)(
  ...
  if (!get_proto_vector.ok()) {
    env->Throw(mediapipe_custom::android::CreateMediaPipeException(
        env, get_proto_vector.status()));
  }
  const std::vector<const ::mediapipe_custom::proto_ns::MessageLite*>& proto_vector =
  ...
  for (int i = 0; i < proto_vector.size(); ++i) {
    const ::mediapipe_custom::proto_ns::MessageLite* proto_message = proto_vector[i];
...
JNIEXPORT jint JNICALL PACKET_GETTER_METHOD(nativeGetImageWidth)(JNIEnv* env,
  ...
  mediapipe_custom::Packet mediapipe_packet =
      mediapipe_custom::android::Graph::GetPacketFromHandle(packet);
  const bool is_image =
      mediapipe_packet.ValidateAsType<mediapipe_custom::Image>().ok();
  const mediapipe_custom::ImageFrame& image =
      is_image ? *GetFromNativeHandle<mediapipe_custom::Image>(packet)
                      .GetImageFrameSharedPtr()
                      .get()
               : GetFromNativeHandle<mediapipe_custom::ImageFrame>(packet);
  return image.Width();
}
...
JNIEXPORT jobject JNICALL PACKET_GETTER_METHOD(nativeGetImageDataDirect)(
  ...
  mediapipe_custom::ImageFrame& mutable_image =
      const_cast<mediapipe_custom::ImageFrame&>(image);
...
JNIEXPORT jint JNICALL PACKET_GETTER_METHOD(nativeGetImageListSize)(
  ...
  const auto& image_list =
      GetFromNativeHandle<std::vector<mediapipe_custom::Image>>(packet);
...
JNIEXPORT jboolean JNICALL PACKET_GETTER_METHOD(nativeGetRgbaFromRgb)(
  ...
  mediapipe_custom::android::RgbToRgba(image.PixelData(), image.WidthStep(),
...
JNIEXPORT jint JNICALL PACKET_GETTER_METHOD(nativeGetVideoHeaderWidth)(
    JNIEnv* env, jobject thiz, jlong packet) {
  return GetFromNativeHandle<mediapipe_custom::VideoHeader>(packet).width;
}

JNIEXPORT jint JNICALL PACKET_GETTER_METHOD(nativeGetVideoHeaderHeight)(
    JNIEnv* env, jobject thiz, jlong packet) {
  return GetFromNativeHandle<mediapipe_custom::VideoHeader>(packet).height;
}

JNIEXPORT jint JNICALL PACKET_GETTER_METHOD(
    nativeGetTimeSeriesHeaderNumChannels)(JNIEnv* env, jobject thiz,
                                          jlong packet) {
  return GetFromNativeHandle<mediapipe_custom::TimeSeriesHeader>(packet)
      .num_channels();
}
...
JNIEXPORT jdouble JNICALL PACKET_GETTER_METHOD(
    nativeGetTimeSeriesHeaderSampleRate)(JNIEnv* env, jobject thiz,
                                         jlong packet) {
  return GetFromNativeHandle<mediapipe_custom::TimeSeriesHeader>(packet).sample_rate();
}

JNIEXPORT jbyteArray JNICALL PACKET_GETTER_METHOD(nativeGetAudioData)(
    JNIEnv* env, jobject thiz, jlong packet) {
  const mediapipe_custom::Matrix& audio_mat =
      GetFromNativeHandle<mediapipe_custom::Matrix>(packet);
...
JNIEXPORT jint JNICALL PACKET_GETTER_METHOD(nativeGetMatrixRows)(JNIEnv* env,
                                                                 jobject thiz,
                                                                 jlong packet) {
  return GetFromNativeHandle<mediapipe_custom::Matrix>(packet).rows();
}

JNIEXPORT jint JNICALL PACKET_GETTER_METHOD(nativeGetMatrixCols)(JNIEnv* env,
                                                                 jobject thiz,
                                                                 jlong packet) {
  return GetFromNativeHandle<mediapipe_custom::Matrix>(packet).cols();
}
...
JNIEXPORT jint JNICALL PACKET_GETTER_METHOD(nativeGetGpuBufferName)(
    JNIEnv* env, jobject thiz, jlong packet) {
  const mediapipe_custom::GpuBuffer& gpu_buffer =
      GetFromNativeHandle<mediapipe_custom::GpuBuffer>(packet);
  ...
  return static_cast<jint>(
      gpu_buffer.internal_storage<mediapipe_custom::GlTextureBuffer>()->name());
}
...
JNIEXPORT jlong JNICALL PACKET_GETTER_METHOD(nativeGetGpuBuffer)(
  ...
  mediapipe_custom::Packet mediapipe_packet =
      mediapipe_custom::android::Graph::GetPacketFromHandle(packet);
  mediapipe_custom::GlTextureBufferSharedPtr ptr;
  if (mediapipe_packet.ValidateAsType<mediapipe_custom::Image>().ok()) {
    auto mediapipe_graph =
        mediapipe_custom::android::Graph::GetContextFromHandle(packet);
    auto gl_context = mediapipe_graph->GetGpuResources()->gl_context();
    auto status =
        gl_context->Run([gl_context, mediapipe_packet, &ptr]() -> absl::Status {
          const mediapipe_custom::Image& buffer =
              mediapipe_packet.Get<mediapipe_custom::Image>();
          ptr = buffer.GetGlTextureBufferSharedPtr();
          return absl::OkStatus();
        });
  } else {
    const mediapipe_custom::GpuBuffer& buffer =
        mediapipe_packet.Get<mediapipe_custom::GpuBuffer>();
    ptr = buffer.internal_storage<mediapipe_custom::GlTextureBuffer>();
  }
  if (wait_on_cpu) {
    ptr->WaitUntilComplete();
  }
  return reinterpret_cast<intptr_t>(
      new mediapipe_custom::GlTextureBufferSharedPtr(ptr));
}
