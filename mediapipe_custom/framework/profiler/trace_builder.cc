...
#include "mediapipe_custom/framework/profiler/trace_builder.h"
...
template <>
struct hash<mediapipe_custom::TaskId> {
  std::size_t operator()(const mediapipe_custom::TaskId& task_id) const {
    return task_id.hash();
  }
};
}  // namespace std
