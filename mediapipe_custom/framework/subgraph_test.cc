...
#include "mediapipe_custom/framework/port/status_matchers.h"  // NOLINT(build/deprecated)
...
class EmitFileStringTestSubgraph : public Subgraph {
 public:
  absl::StatusOr<CalculatorGraphConfig> GetConfig(
      mediapipe_custom::SubgraphContext* sc) override {
    MP_ASSIGN_OR_RETURN(
        std::unique_ptr<mediapipe_custom::Resource> data,
        sc->GetResources().Get(
            "mediapipe/framework/testdata/resource_subgraph.data"));
    Graph graph;
    auto& node = graph.AddNode("ConstantSidePacketCalculator");
    node.GetOptions<mediapipe_custom::ConstantSidePacketCalculatorOptions>()
        .add_packet()
