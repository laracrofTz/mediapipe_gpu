...
TEST(GetRoi, WholeImageNormRect) {
  mediapipe_custom::NormalizedRect norm_rect;
...
TEST(ValidateOptionOutputDims, ImageToTensorCalcOptions) {
  const auto float_options =
      mediapipe_custom::ParseTextProtoOrDie<mediapipe_custom::ImageToTensorCalculatorOptions>(
          kValidFloatProto);
...
TEST(GetBorderMode, GetBorderMode) {
  ...
  auto border_mode =
      mediapipe_custom::ImageToTensorCalculatorOptions_BorderMode_BORDER_UNSPECIFIED;
  EXPECT_EQ(BorderMode::kReplicate, GetBorderMode(border_mode));

  // Set to ZERO.
  border_mode =
      mediapipe_custom::ImageToTensorCalculatorOptions_BorderMode_BORDER_ZERO;
