...
#include "mediapipe_custom/graphs/instant_motion_tracking/calculators/transformations.h"
 ...
 public:
  static absl::Status GetContract(CalculatorContract* cc) {
    ...
    if (cc->Inputs().HasTag(kBoxesInputTag)) {
      cc->Inputs().Tag(kBoxesInputTag).Set<mediapipe_custom::TimedBoxProtoList>();
    }

    cc->Outputs().Tag(kAnchorsTag).Set<std::vector<Anchor>>();
    cc->Outputs().Tag(kBoxesOutputTag).Set<mediapipe_custom::TimedBoxProtoList>();
...
absl::Status TrackedAnchorManagerCalculator::Process(CalculatorContext* cc) {
  mediapipe_custom::Timestamp timestamp = cc->InputTimestamp();
  ...
  auto pos_boxes = absl::make_unique<mediapipe_custom::TimedBoxProtoList>();
  std::vector<Anchor> tracked_scaled_anchor_data;

  // Delete any boxes being tracked without an associated anchor
  for (const mediapipe_custom::TimedBoxProto& box :
       cc->Inputs()
           .Tag(kBoxesInputTag)
           .Get<mediapipe_custom::TimedBoxProtoList>()
           .box()) {
  ...
  for (const Anchor& anchor : current_anchor_data) {
    ...
    if (sticker_sentinel == anchor.sticker_id) {
      ...
      // Add a tracking box
      mediapipe_custom::TimedBoxProto* box = pos_boxes->add_box();
      ...
      const mediapipe_custom::TimedBoxProtoList box_list =
          cc->Inputs().Tag(kBoxesInputTag).Get<mediapipe_custom::TimedBoxProtoList>();
