...
#include "mediapipe_custom/framework/testdata/proto3_options.pb.h"
...
TEST(OptionsMapTest, QueryNotFound) {
  ...
  EXPECT_FALSE(options.Has<mediapipe_custom::NightLightCalculatorOptions>());
  EXPECT_FALSE(options.Has<mediapipe_custom::Proto3Options>());
}
...
TEST(OptionsMapTest, Proto2QueryFound) {
  ...
  OptionsMap options;
  options.Initialize(node);
  EXPECT_TRUE(options.Has<mediapipe_custom::NightLightCalculatorOptions>());
  EXPECT_EQ(
      options.Get<mediapipe_custom::NightLightCalculatorOptions>().base_timestamp()[0],
      123);
}
...
TEST(MutableOptionsMapTest, InsertProto2AndQueryFound) {
  ...
  options.Initialize(node);
  EXPECT_FALSE(options.Has<mediapipe_custom::NightLightCalculatorOptions>());
  mediapipe_custom::NightLightCalculatorOptions night_light_options;
...
TEST(OptionsMapTest, Proto3QueryFound) {
  ...
  OptionsMap options;
  options.Initialize(node);
  EXPECT_TRUE(options.Has<mediapipe_custom::Proto3Options>());
  EXPECT_EQ(options.Get<mediapipe_custom::Proto3Options>().test_value(), 123);
}
...
TEST(MutableOptionsMapTest, InsertProto3AndQueryFound) {
  ...
  options.Initialize(node);
  EXPECT_FALSE(options.Has<mediapipe_custom::Proto3Options>());
  mediapipe_custom::Proto3Options proto3_options;
...
TEST(OptionsMapTest, BothProto2AndProto3QueriesFound) {
  ...
  EXPECT_EQ(options.Get<mediapipe_custom::Proto3Options>().test_value(), 123);
  EXPECT_TRUE(options.Has<mediapipe_custom::NightLightCalculatorOptions>());
  EXPECT_EQ(options.Get<mediapipe_custom::NightLightCalculatorOptions>().jitter(),
            321);
}
