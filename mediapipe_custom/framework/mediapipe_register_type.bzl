...
Example usage:
    ...
    load("//mediapipe_custom/framework:mediapipe_register_type.bzl",
         "mediapipe_register_type")

    # Creates rules "foo_registration"
    mediapipe_register_type(
        base_name = "foo"
        include_headers = ["mediapipe_custom/framework/formats/foo.proto.h""],
        types = [
            "::mediapipe::Foo",
            "::mediapipe::FooList",
...
load("//mediapipe_custom/framework/tool:build_defs.bzl", "clean_dep")
...
def mediapipe_register_type(
    ...
    native.cc_library(
        ...
        deps = depset(deps + [
            clean_dep("//mediapipe_custom/framework:type_map"),
        ]),
