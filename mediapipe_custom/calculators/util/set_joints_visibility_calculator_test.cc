...
TEST_P(SetJointsVisibilityTest, SetJointsVisibilityTest) {
  ...
  // Prepare graph.
  mediapipe_custom::CalculatorRunner runner(ParseTextProtoOrDie<Node>(absl::Substitute(
      R"(
  ...
  runner.MutableInputs()->Tag("JOINTS").packets.push_back(
      MakePacket<JointList>(std::move(in_joints)).At(mediapipe_custom::Timestamp(0)));
