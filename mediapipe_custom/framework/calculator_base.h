...
// SOFT DEPRECATION: use mediapipe_custom::api2::NodeIntf/Impl instead.
//
...
class CalculatorBaseFactoryFor : public CalculatorBaseFactory {
  static_assert(std::is_base_of<mediapipe_custom::CalculatorBase, T>::value,
                "Classes registered with REGISTER_CALCULATOR must be "
                "subclasses of mediapipe_custom::CalculatorBase.");
};

template <class T>
class CalculatorBaseFactoryFor<
    T,
    typename std::enable_if<std::is_base_of<mediapipe_custom::CalculatorBase, T>{} &&
                            !std::is_base_of<mediapipe_custom::api2::Node, T>{}>::type>
