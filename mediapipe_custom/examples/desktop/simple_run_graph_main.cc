...
absl::Status OutputStreamToLocalFile(mediapipe_custom::OutputStreamPoller& poller) {
  std::ofstream file;
...
absl::Status OutputSidePacketsToLocalFile(mediapipe_custom::CalculatorGraph& graph) {
...
absl::Status RunMPPGraph() {
  ...
  if (!absl::GetFlag(FLAGS_input_side_packets).empty()) {
    ...
    for (const std::string& kv_pair : kv_pairs) {
      ...
      RET_CHECK(!mediapipe_custom::ContainsKey(input_side_packets, name_and_value[0]));
      input_side_packets[name_and_value[0]] =
          mediapipe_custom::MakePacket<std::string>(name_and_value[1]);
    }
  }
