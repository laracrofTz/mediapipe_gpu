...
#include "mediapipe_custom/calculators/image/rotation_mode.pb.h"
#include "mediapipe_custom/framework/calculator_framework.h"
#include "mediapipe_custom/framework/formats/image_frame.h"
#include "mediapipe_custom/framework/formats/image_frame_opencv.h"
#include "mediapipe_custom/framework/formats/video_stream_header.h"
#include "mediapipe_custom/framework/packet.h"
#include "mediapipe_custom/framework/port/opencv_core_inc.h"
#include "mediapipe_custom/framework/port/opencv_imgproc_inc.h"
...
#include "mediapipe_custom/gpu/scale_mode.pb.h"

#if !MEDIAPIPE_DISABLE_GPU
#include "mediapipe_custom/gpu/gl_base.h"
#include "mediapipe_custom/gpu/gl_calculator_helper.h"
#include "mediapipe_custom/gpu/gl_quad_renderer.h"
#include "mediapipe_custom/gpu/gl_simple_shaders.h"
#include "mediapipe_custom/gpu/shader_util.h"
...
#define DEFAULT_SCALE_MODE mediapipe_custom::ScaleMode_Mode_STRETCH
...
int RotationModeToDegrees(mediapipe_custom::RotationMode_Mode rotation) {
  switch (rotation) {
    case mediapipe_custom::RotationMode::UNKNOWN:
    case mediapipe_custom::RotationMode::ROTATION_0:
      return 0;
    case mediapipe_custom::RotationMode::ROTATION_90:
      return 90;
    case mediapipe_custom::RotationMode::ROTATION_180:
      return 180;
    case mediapipe_custom::RotationMode::ROTATION_270:
      return 270;
  }
}
mediapipe_custom::RotationMode_Mode DegreesToRotationMode(int degrees) {
  switch (degrees) {
    case 0:
      return mediapipe_custom::RotationMode::ROTATION_0;
    case 90:
      return mediapipe_custom::RotationMode::ROTATION_90;
    case 180:
      return mediapipe_custom::RotationMode::ROTATION_180;
    case 270:
      return mediapipe_custom::RotationMode::ROTATION_270;
    default:
      return mediapipe_custom::RotationMode::UNKNOWN;
  }
}
mediapipe_custom::ScaleMode_Mode ParseScaleMode(
    mediapipe_custom::ScaleMode_Mode scale_mode,
    mediapipe_custom::ScaleMode_Mode default_mode) {
  switch (scale_mode) {
    case mediapipe_custom::ScaleMode::DEFAULT:
      return default_mode;
    case mediapipe_custom::ScaleMode::STRETCH:
      return scale_mode;
    case mediapipe_custom::ScaleMode::FIT:
      return scale_mode;
    case mediapipe_custom::ScaleMode::FILL_AND_CROP:
      return scale_mode;
    default:
      return default_mode;
  }
...
class ImageTransformationCalculator : public CalculatorBase {
 ...
 private:
  ...
  mediapipe_custom::RotationMode_Mode rotation_;
  mediapipe_custom::ScaleMode_Mode scale_mode_;
...
absl::Status ImageTransformationCalculator::GetContract(
  ...
  if (cc->Inputs().HasTag(kVideoPrestreamTag)) {
    ...
    cc->Inputs().Tag(kVideoPrestreamTag).Set<mediapipe_custom::VideoHeader>();
    cc->Outputs().Tag(kVideoPrestreamTag).Set<mediapipe_custom::VideoHeader>();
  }
...
absl::Status ImageTransformationCalculator::Process(CalculatorContext* cc) {
  ...
  if (cc->Inputs().HasTag(kVideoPrestreamTag) &&
    ...
    mediapipe_custom::VideoHeader header =
        cc->Inputs().Tag(kVideoPrestreamTag).Get<mediapipe_custom::VideoHeader>();
    ...
    cc->Outputs()
        ...
        .AddPacket(mediapipe_custom::MakePacket<mediapipe_custom::VideoHeader>(header).At(
            mediapipe_custom::Timestamp::PreStream()));
  }
...
absl::Status ImageTransformationCalculator::RenderCpu(CalculatorContext* cc) {
  ...
  if (output_width_ > 0 && output_height_ > 0) {
    cv::Mat scaled_mat;
    if (scale_mode_ == mediapipe_custom::ScaleMode::STRETCH) {
      ...
      if (scale_mode_ == mediapipe_custom::ScaleMode::FIT) {
...
absl::Status ImageTransformationCalculator::RenderGpu(CalculatorContext* cc) {
  ...
  if (scale_mode_ == mediapipe_custom::ScaleMode::FILL_AND_CROP) {
    const float scale =
        std::min(static_cast<float>(output_width_) / input_width,
                 static_cast<float>(output_height_) / input_height);
  ...
  if (input.format() == GpuBufferFormat::kBiPlanar420YpCbCr8VideoRange ||
    ...
    if (!yuv_renderer_) {
      ...
      MP_RETURN_IF_ERROR(
          yuv_renderer_->GlSetup(::mediapipe_custom::kYUV2TexToRGBFragmentShader,
    ...
    if (src1.target() == GL_TEXTURE_EXTERNAL_OES) {
      if (!ext_rgb_renderer_) {
        ...
        MP_RETURN_IF_ERROR(ext_rgb_renderer_->GlSetup(
            ::mediapipe_custom::kBasicTexturedFragmentShaderOES, {"video_frame"}));
      }
  ...
  mediapipe_custom::FrameScaleMode scale_mode = mediapipe_custom::FrameScaleModeFromProto(
      scale_mode_, mediapipe_custom::FrameScaleMode::kStretch);
  mediapipe_custom::FrameRotation rotation =
      mediapipe_custom::FrameRotationFromDegrees(RotationModeToDegrees(rotation_));
...
void ImageTransformationCalculator::ComputeOutputDimensions(
  ...
  } else if (rotation_ == mediapipe_custom::RotationMode::ROTATION_90 ||
             rotation_ == mediapipe_custom::RotationMode::ROTATION_270) {
...
void ImageTransformationCalculator::ComputeOutputLetterboxPadding(
  ...
  padding->fill(0.f);
  if (scale_mode_ == mediapipe_custom::ScaleMode::FIT) {
    if (rotation_ == mediapipe_custom::RotationMode::ROTATION_90 ||
        rotation_ == mediapipe_custom::RotationMode::ROTATION_270) {
