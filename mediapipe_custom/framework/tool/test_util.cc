...
std::string GetBinaryDirectory() {
  ...
  return std::string(
      ::mediapipe_custom::file::Dirname(absl::string_view(full_path, length)));
}
#endif
...
absl::StatusOr<std::unique_ptr<ImageFrame>> LoadTestImage(
  ...
  std::string encoded;
  MP_RETURN_IF_ERROR(mediapipe_custom::file::GetContents(path, &encoded));
...
absl::StatusOr<std::string> SavePngTestOutput(
    const mediapipe_custom::ImageFrame& image, absl::string_view prefix) {
