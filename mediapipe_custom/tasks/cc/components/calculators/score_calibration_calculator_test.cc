...
void BuildGraph(CalculatorRunner* runner, std::vector<float> scores,
  ...
  input_scores_packets.push_back(
      mediapipe_custom::Adopt(scores_tensors.release()).At(mediapipe_custom::Timestamp(0)));
  ...
  if (indices.has_value()) {
    ...
    input_indices_packets.push_back(mediapipe_custom::Adopt(indices_tensors.release())
