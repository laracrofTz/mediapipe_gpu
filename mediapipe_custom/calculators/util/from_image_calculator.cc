...
absl::Status FromImageCalculator::GetContract(CalculatorContract* cc) {
  ...
  if (cc->Outputs().HasTag(kImageFrameTag)) {
    cc->Outputs().Tag(kImageFrameTag).Set<mediapipe_custom::ImageFrame>();
  }
...
absl::Status FromImageCalculator::Process(CalculatorContext* cc) {
  if (check_image_source_) {
    auto& input = cc->Inputs().Tag(kImageTag).Get<mediapipe_custom::Image>();
    cc->Outputs()
        .Tag(kSourceOnGpuTag)
  ...
  if (gpu_output_) {
    ...
    MP_RETURN_IF_ERROR(gpu_helper_.RunInGlContext([&cc]() -> absl::Status {
      ...
      auto output =
          std::make_unique<mediapipe_custom::GpuBuffer>(input.GetGpuBuffer());
      cc->Outputs()
          .Tag(kGpuBufferTag)
    ...
    std::unique_ptr<mediapipe_custom::ImageFrame> output =
        std::make_unique<mediapipe_custom::ImageFrame>(
