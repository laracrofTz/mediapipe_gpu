...
#include "mediapipe_custom/framework/profiler/circular_buffer.h"
...
TEST_F(CircularBufferTest, SequentialWriteAndRead) {
  mediapipe_custom::CircularBuffer<std::string> my_buffer(100);
...
TEST_F(CircularBufferTest, ParallelWriteAndRead) {
  mediapipe_custom::CircularBuffer<std::string> buffer(100);
  auto first = buffer.begin();
  std::atomic_int read_sum(0);
  std::atomic_int read_count(0);
  {
    mediapipe_custom::ThreadPool pool(12);
    pool.StartWorkers();
...
TEST_F(CircularBufferTest, SequentialGetWraps) {
  mediapipe_custom::CircularBuffer<int> buffer(3);
  buffer.push_back(2);
