...
#include "mediapipe_custom/calculators/tflite/tflite_inference_calculator.pb.h"
...
#include "mediapipe_custom/util/tflite/config.h"

#if !defined(__EMSCRIPTEN__) || defined(__EMSCRIPTEN_PTHREADS__)
#include "mediapipe_custom/util/cpu_util.h"
#endif  // !__EMSCRIPTEN__ || __EMSCRIPTEN_PTHREADS__

#include "mediapipe_custom/util/tflite/tflite_model_loader.h"
...
#include "mediapipe_custom/util/android/file/base/file.h"
#include "mediapipe_custom/util/android/file/base/filesystem.h"
#include "mediapipe_custom/util/android/file/base/helpers.h"
#endif  // ANDROID

#if MEDIAPIPE_TFLITE_GL_INFERENCE
#include "mediapipe_custom/gpu/gl_calculator_helper.h"
#include "mediapipe_custom/gpu/gpu_buffer.h"
#include "mediapipe_custom/util/tflite/tflite_gpu_runner.h"
...
#include "mediapipe_custom/gpu/MPPMetalUtil.h"
#include "mediapipe_custom/gpu/gpu_buffer.h"
#include "tensorflow/lite/delegates/gpu/common/shape.h"
...
int GetXnnpackNumThreads(
    const mediapipe_custom::TfLiteInferenceCalculatorOptions& opts) {
...
class TfLiteInferenceCalculator : public CalculatorBase {
 ...
 private:
  ...
  mediapipe_custom::TfLiteInferenceCalculatorOptions::Delegate::Gpu::Api
      tflite_gpu_runner_api_;
  mediapipe_custom::TfLiteInferenceCalculatorOptions::Delegate::Gpu::InferenceUsage
...
#if MEDIAPIPE_TFLITE_GPU_SUPPORTED
  const auto& options =
      cc->template Options<::mediapipe_custom::TfLiteInferenceCalculatorOptions>();
...
absl::Status TfLiteInferenceCalculator::GetContract(CalculatorContract* cc) {
  ...
  const auto& options =
      cc->Options<::mediapipe_custom::TfLiteInferenceCalculatorOptions>();
...
absl::Status TfLiteInferenceCalculator::Open(CalculatorContext* cc) {
  ...
  if (use_kernel_caching_) {
    ...
    cached_kernel_filename_ = options.delegate().gpu().cached_kernel_path() +
                              mediapipe_custom::File::Basename(options.model_path()) +
                              ".ker";
...
absl::Status TfLiteInferenceCalculator::WriteKernelsToFile() {
  ...
  if (use_kernel_caching_) {
    ...
    MP_RETURN_IF_ERROR(
        mediapipe_custom::file::SetContents(cached_kernel_filename_, cache_str));
  }
#endif  // MEDIAPIPE_TFLITE_GL_INFERENCE && MEDIAPIPE_ANDROID
  return absl::OkStatus();
}
...
absl::Status TfLiteInferenceCalculator::ReadKernelsFromFile() {
  ...
  if (use_kernel_caching_) {
    ...
    if (mediapipe_custom::File::Exists(cached_kernel_filename_)) {
      std::string cache_str;
      MP_RETURN_IF_ERROR(
          mediapipe_custom::file::GetContents(cached_kernel_filename_, &cache_str));
...
absl::Status TfLiteInferenceCalculator::InitTFLiteGPURunner(
  ...
  switch (tflite_gpu_runner_usage_) {
    case mediapipe_custom::TfLiteInferenceCalculatorOptions::Delegate::Gpu::
        FAST_SINGLE_ANSWER: {
  ...
  switch (tflite_gpu_runner_api_) {
    case mediapipe_custom::TfLiteInferenceCalculatorOptions::Delegate::Gpu::OPENGL: {
      tflite_gpu_runner_->ForceOpenGL();
      break;
    }
    case mediapipe_custom::TfLiteInferenceCalculatorOptions::Delegate::Gpu::OPENCL: {
      tflite_gpu_runner_->ForceOpenCL();
      break;
    }
    case mediapipe_custom::TfLiteInferenceCalculatorOptions::Delegate::Gpu::ANY: {
