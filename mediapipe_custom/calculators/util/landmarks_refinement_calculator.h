...
struct LandmarksRefinementNode : Node<kLandmarksRefinementNodeName> {
  ...
  struct Contract {
    ...
    Repeated<Input<S, mediapipe_custom::NormalizedLandmarkList>> landmarks{
        "LANDMARKS"};
    ...
    // mapping).
    Output<S, mediapipe_custom::NormalizedLandmarkList> refined_landmarks{
        "REFINED_LANDMARKS"};

    Options<S, mediapipe_custom::LandmarksRefinementCalculatorOptions> options;
  };
};
