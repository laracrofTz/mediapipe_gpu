...
#include "mediapipe_custom/java/com/google/mediapipe/framework/jni/surface_output_jni.h"

// TODO: CHECK in JNI does not work. Raise exception instead.

namespace {
mediapipe_custom::EglSurfaceHolder* GetSurfaceHolder(jlong packet) {
  return mediapipe_custom::android::Graph::GetPacketFromHandle(packet)
      .Get<std::unique_ptr<mediapipe_custom::EglSurfaceHolder>>()
      .get();
}

mediapipe_custom::GlContext* GetGlContext(jlong context) {
  auto mediapipe_graph = reinterpret_cast<mediapipe_custom::android::Graph*>(context);
  mediapipe_custom::GpuResources* gpu_resources = mediapipe_graph->GetGpuResources();
...
JNIEXPORT void JNICALL MEDIAPIPE_SURFACE_OUTPUT_METHOD(nativeSetFlipY)(
  ...
  mediapipe_custom::EglSurfaceHolder* surface_holder = GetSurfaceHolder(packet);
...
#ifdef __ANDROID__
  mediapipe_custom::GlContext* gl_context = GetGlContext(context);
