...
#include "mediapipe_custom/objc/util.h"
...
@implementation MPPGraph {
  ...
  // details.
  std::unique_ptr<mediapipe_custom::CalculatorGraph> _graph;
  /// Input side packets that will be added to the graph when it is started.
  std::map<std::string, mediapipe_custom::Packet> _inputSidePackets;
  /// Packet headers that will be added to the graph when it is started.
  std::map<std::string, mediapipe_custom::Packet> _streamHeaders;
  /// Service packets to be added to the graph when it is started.
  std::map<const mediapipe_custom::GraphServiceBase*, mediapipe_custom::Packet>
      _servicePackets;
  ...
  mediapipe_custom::Timestamp _frameTimestamp;
  int64_t _frameNumber;

  // Graph config modified to expose requested output streams.
  mediapipe_custom::CalculatorGraphConfig _config;
...
- (instancetype)initWithGraphConfig:
    (const mediapipe_custom::CalculatorGraphConfig&)config {
  self = [super init];
  if (self) {
    ...
    _graph = absl::make_unique<mediapipe_custom::CalculatorGraph>();
    _config = config;
  }
  return self;
}

- (mediapipe_custom::ProfilingContext*)getProfiler {
  return _graph->profiler();
}

- (mediapipe_custom::CalculatorGraph::GraphInputStreamAddMode)packetAddMode {
  return _graph->GetGraphInputStreamAddMode();
}

- (void)setPacketAddMode:
    (mediapipe_custom::CalculatorGraph::GraphInputStreamAddMode)mode {
...
- (void)addFrameOutputStream:(const std::string&)outputStreamName
  ...
  mediapipe_custom::tool::AddCallbackCalculator(outputStreamName, &_config,
                                         &callbackInputName,
  ...
  _inputSidePackets[callbackInputName] =
      mediapipe_custom::MakePacket<std::function<void(const mediapipe_custom::Packet&)>>(
          [wrapperVoid, outputStreamName,
           packetType](const mediapipe_custom::Packet& packet) {
...
void CallFrameDelegate(void* wrapperVoid, const std::string& streamName,
  ...
  @autoreleasepool {
    if (packetType == MPPPacketTypeRaw) {
      ...
      const auto& frame = packet.Get<mediapipe_custom::ImageFrame>();
      mediapipe_custom::ImageFormat::Format format = frame.Format();

      if (format == mediapipe_custom::ImageFormat::SRGBA ||
      ...
      if (packetType == MPPPacketTypePixelBuffer)
        pixelBuffer =
            mediapipe_custom::GetCVPixelBufferRef(packet.Get<mediapipe_custom::GpuBuffer>());
      else
        pixelBuffer = packet.Get<mediapipe_custom::Image>().GetCVPixelBufferRef();
      if ([wrapper.delegate
              respondsToSelector:@selector
              (mediapipeGraph:didOutputPixelBuffer:fromStream:timestamp:)]) {
        [wrapper.delegate mediapipeGraph:wrapper
...
- (void)setHeaderPacket:(const mediapipe_custom::Packet&)packet
...
- (void)setSidePacket:(const mediapipe_custom::Packet&)packet
...
- (void)setServicePacket:(mediapipe_custom::Packet&)packet
              forService:(const mediapipe_custom::GraphServiceBase&)service {
  _GTMDevAssert(!_started, @"%@ must be called before the graph is started",
                NSStringFromSelector(_cmd));
...
- (void)addSidePackets:
    (const std::map<std::string, mediapipe_custom::Packet>&)extraSidePackets {
  _GTMDevAssert(!_started, @"%@ must be called before the graph is started",
                NSStringFromSelector(_cmd));
...
- (BOOL)movePacket:(mediapipe_custom::Packet&&)packet
...
- (BOOL)sendPacket:(const mediapipe_custom::Packet&)packet
        intoStream:(const std::string&)streamName
             error:(NSError**)error {
...
- (mediapipe_custom::Packet)packetWithPixelBuffer:(CVPixelBufferRef)imageBuffer
  ...
  if (packetType == MPPPacketTypeImageFrame ||
    ...
    packet = mediapipe_custom::Adopt(frame.release());
#if MEDIAPIPE_GPU_BUFFER_USE_CV_PIXEL_BUFFER
  } else if (packetType == MPPPacketTypePixelBuffer) {
    packet = mediapipe_custom::MakePacket<mediapipe_custom::GpuBuffer>(imageBuffer);
#endif  // MEDIAPIPE_GPU_BUFFER_USE_CV_PIXEL_BUFFER
  } else if (packetType == MPPPacketTypeImage) {
#if MEDIAPIPE_GPU_BUFFER_USE_CV_PIXEL_BUFFER
    // GPU
    packet = mediapipe_custom::MakePacket<mediapipe_custom::Image>(imageBuffer);
#else
    // CPU
    auto frame = CreateImageFrameForCVPixelBuffer(imageBuffer,
                                                  /* canOverwrite = */ false,
                                                  /* bgrAsRgb = */ false);
    packet = mediapipe_custom::MakePacket<mediapipe_custom::Image>(std::move(frame));
#endif  // MEDIAPIPE_GPU_BUFFER_USE_CV_PIXEL_BUFFER
  } else {
...
- (mediapipe_custom::Packet)imagePacketWithPixelBuffer:(CVPixelBufferRef)pixelBuffer {
...
- (BOOL)sendPixelBuffer:(CVPixelBufferRef)imageBuffer
             ...
             packetType:(MPPPacketType)packetType
              timestamp:(const mediapipe_custom::Timestamp&)timestamp
...
- (BOOL)sendPixelBuffer:(CVPixelBufferRef)imageBuffer
             intoStream:(const std::string&)inputName
             packetType:(MPPPacketType)packetType {
  _GTMDevAssert(_frameTimestamp < mediapipe_custom::Timestamp::Done(),
                @"Trying to send frame after stream is done.");
  if (_frameTimestamp < mediapipe_custom::Timestamp::Min()) {
    _frameTimestamp = mediapipe_custom::Timestamp::Min();
  } else {
    _frameTimestamp++;
  }
  return [self sendPixelBuffer:imageBuffer
...
- (void)debugPrintGlInfo {
  std::shared_ptr<mediapipe_custom::GpuResources> gpu_resources =
