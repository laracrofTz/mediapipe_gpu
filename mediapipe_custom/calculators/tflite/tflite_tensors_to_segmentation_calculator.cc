...
#include "mediapipe_custom/calculators/tflite/tflite_tensors_to_segmentation_calculator.pb.h"
...
class TfLiteTensorsToSegmentationCalculator : public CalculatorBase {
 ...
 private:
  ...
  ::mediapipe_custom::TfLiteTensorsToSegmentationCalculatorOptions options_;
...
absl::Status TfLiteTensorsToSegmentationCalculator::GetContract(
  ...
  if (cc->Inputs().HasTag(kPrevMaskGpuTag)) {
    cc->Inputs().Tag(kPrevMaskGpuTag).Set<mediapipe_custom::GpuBuffer>();
    use_gpu |= true;
  }
  if (cc->Inputs().HasTag(kSizeImageGpuTag)) {
    cc->Inputs().Tag(kSizeImageGpuTag).Set<mediapipe_custom::GpuBuffer>();
    use_gpu |= true;
  }
#endif  // !MEDIAPIPE_DISABLE_GPU
  ...
  if (cc->Outputs().HasTag(kMaskGpuTag)) {
    cc->Outputs().Tag(kMaskGpuTag).Set<mediapipe_custom::GpuBuffer>();
    use_gpu |= true;
  }
#endif  // !MEDIAPIPE_DISABLE_GPU
...
absl::Status TfLiteTensorsToSegmentationCalculator::ProcessGpu(
  ...
  const auto& input_mask =
      has_prev_mask
          ? cc->Inputs().Tag(kPrevMaskGpuTag).Get<mediapipe_custom::GpuBuffer>()
          : mediapipe_custom::GpuBuffer();
  int output_width = tensor_width_, output_height = tensor_height_;
  if (cc->Inputs().HasTag(kSizeImageGpuTag)) {
    const auto& input_image =
        cc->Inputs().Tag(kSizeImageGpuTag).Get<mediapipe_custom::GpuBuffer>();
    output_width = input_image.width();
    output_height = input_image.height();
  }
  RET_CHECK_EQ(input_tensors.size(), 1);
  ...
  // Upsample small mask into output.
  mediapipe_custom::GlTexture output_texture = gpu_helper_.CreateDestinationTexture(
      output_width, output_height,
      mediapipe_custom::GpuBufferFormat::kBGRA32);  // actually GL_RGBA8
  ...
  auto output_image = output_texture.GetFrame<mediapipe_custom::GpuBuffer>();
  cc->Outputs()
      .Tag(kMaskGpuTag)
...
absl::Status TfLiteTensorsToSegmentationCalculator::LoadOptions(
  ...
  options_ =
      cc->Options<::mediapipe_custom::TfLiteTensorsToSegmentationCalculatorOptions>();
...
void main() {
  ...
  void main() {
    ...
    mediapipe_custom::GlhCreateProgram(
        mediapipe_custom::kBasicVertexShader, upsample_shader_base.c_str(),
