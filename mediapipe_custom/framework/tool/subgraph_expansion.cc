...
absl::Status FindCorrespondingStreams(
  ...
  for (const auto& it : dst_map->Mapping()) {
    ...
    const TagMap::TagData* src_tag_data =
        mediapipe_custom::FindOrNull(src_map->Mapping(), tag);
    if (!src_tag_data) {
...
absl::Status ConnectSubgraphStreams(
  ...
  auto replace_names = [&name_map](absl::string_view s) {
    ...
    std::string* replacement = mediapipe_custom::FindOrNull(*name_map, original);
...
absl::Status ExpandSubgraphs(CalculatorGraphConfig* config,
  ...
  MP_RETURN_IF_ERROR(mediapipe_custom::tool::DefineGraphOptions(
    ...
    for (auto it = subgraph_nodes_start; it != nodes->end(); ++it) {
      ...
      MP_ASSIGN_OR_RETURN(
          ...
          graph_registry->CreateByName(config->package(), node.calculator(),
                                       &subgraph_context));
      MP_RETURN_IF_ERROR(mediapipe_custom::tool::DefineGraphOptions(node, &subgraph));
