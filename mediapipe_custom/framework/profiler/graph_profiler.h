...
#include "mediapipe_custom/framework/profiler/sharded_map.h"
...
class GraphProfiler : public std::enable_shared_from_this<ProfilingContext> {
 public:
  ...
  void SetClock(const std::shared_ptr<mediapipe_custom::Clock>& clock)
      ABSL_LOCKS_EXCLUDED(profiler_mutex_);

  // Gets the profiler clock.
  const std::shared_ptr<mediapipe_custom::Clock> GetClock() const
      ABSL_LOCKS_EXCLUDED(profiler_mutex_);
  ...
  absl::Status Start(mediapipe_custom::Executor* executor);
 ...
 private:
  ...
  std::shared_ptr<mediapipe_custom::Clock> clock_;
