...
#include "mediapipe_custom/calculators/tensor/tensor_converter_cpu.h"
#include "mediapipe_custom/calculators/tensor/tensor_converter_gpu.h"
...
#include "mediapipe_custom/calculators/tensor/tensor_converter_metal.h"
...
#include "mediapipe_custom/calculators/tensor/tensor_converter_gl31.h"
#else
#include "mediapipe_custom/calculators/tensor/tensor_converter_gl30.h"
...
absl::StatusOr<bool> ShouldFlipVertically(
    const mediapipe_custom::TensorConverterCalculatorOptions& options, bool use_gpu) {
  ...
  return mediapipe_custom::IsGpuOriginAtBottom(options.gpu_origin());
}
...
absl::StatusOr<std::optional<Tensor>> TensorConverterCalculator::ProcessGPU(
  ...
  const auto& input =
      cc->Inputs().Tag(kGpuBufferTag).Get<mediapipe_custom::GpuBuffer>();
#if MEDIAPIPE_METAL_ENABLED
...
absl::Status TensorConverterCalculator::InitGpu(CalculatorContext* cc) {
  ...
  const auto& input =
      cc->Inputs().Tag(kGpuBufferTag).Get<mediapipe_custom::GpuBuffer>();
  mediapipe_custom::GpuBufferFormat format = input.format();
  const bool include_alpha = (max_num_channels_ == 4);
  const bool single_channel = (max_num_channels_ == 1);

  RET_CHECK(format == mediapipe_custom::GpuBufferFormat::kBGRA32 ||
            format == mediapipe_custom::GpuBufferFormat::kRGB24 ||
            format == mediapipe_custom::GpuBufferFormat::kRGBA32 ||
            format == mediapipe_custom::GpuBufferFormat::kRGBAFloat128 ||
            format == mediapipe_custom::GpuBufferFormat::kRGBAHalf64 ||
            format == mediapipe_custom::GpuBufferFormat::kGrayFloat32 ||
            format == mediapipe_custom::GpuBufferFormat::kGrayHalf16 ||
            format == mediapipe_custom::GpuBufferFormat::kOneComponent8)
...
absl::Status TensorConverterCalculator::LoadOptions(CalculatorContext* cc,
  ...
  const auto& options =
      cc->Options<::mediapipe_custom::TensorConverterCalculatorOptions>();
