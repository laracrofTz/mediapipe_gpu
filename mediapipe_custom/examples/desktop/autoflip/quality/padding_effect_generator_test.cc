...
#include "mediapipe_custom/examples/desktop/autoflip/quality/padding_effect_generator.h"
...
void TestWithAspectRatio(const double aspect_ratio,
  ...
  if (!process_arbitrary_image) {
    std::string test_image_path = mediapipe_custom::file::JoinPath("./", kTestImage);
    MP_ASSERT_OK(mediapipe_custom::file::GetContents(test_image_path, &test_image));
  } else {
    MP_ASSERT_OK(mediapipe_custom::file::GetContents(absl::GetFlag(FLAGS_input_image),
                                              &test_image));
  }
  ...
  switch (decoded_mat.channels()) {
    ...
    case 3:
      ...
      break;
    case 4:
      MP_ASSERT_OK(mediapipe_custom::UnimplementedErrorBuilder(MEDIAPIPE_LOC)
                   << "4-channel image isn't supported yet");
      break;
    default:
      MP_ASSERT_OK(mediapipe_custom::FailedPreconditionErrorBuilder(MEDIAPIPE_LOC)
  ...
  if (!cv::imencode(".jpg", input_mat, encode_buffer, parameters)) {
    MP_ASSERT_OK(mediapipe_custom::InternalErrorBuilder(MEDIAPIPE_LOC)
  ...
  if (!process_arbitrary_image) {
    std::string result_string_path = mediapipe_custom::file::JoinPath(
        "./", absl::StrCat(kResultImagePrefix, aspect_ratio,
                           background_color_in_rgb ? "_solid_background" : "",
                           ".jpg"));
    std::string result_image;
    MP_ASSERT_OK(
        mediapipe_custom::file::GetContents(result_string_path, &result_image));
    ...
    std::string output_string_path = mediapipe_custom::file::JoinPath(
        absl::GetFlag(FLAGS_output_folder),
        absl::StrCat("result_", aspect_ratio,
                     background_color_in_rgb ? "_solid_background" : "",
                     ".jpg"));
    MP_ASSERT_OK(
        mediapipe_custom::file::SetContents(output_string_path, output_string));
  }
}
