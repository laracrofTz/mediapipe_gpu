...
CVPixelBufferRef CreateCVPixelBufferForImageFramePacket(
    const mediapipe_custom::Packet& image_frame_packet) {
...
absl::Status CreateCVPixelBufferForImageFramePacket(
    const mediapipe_custom::Packet& image_frame_packet, bool can_overwrite,
    CFHolder<CVPixelBufferRef>* out_buffer) {
  if (!out_buffer) {
    return ::mediapipe_custom::InvalidArgumentErrorBuilder(MEDIAPIPE_LOC)
           << "out_buffer cannot be NULL";
  }
  auto image_frame = std::const_pointer_cast<mediapipe_custom::ImageFrame>(
      mediapipe_custom::SharedPtrWithPacket<mediapipe_custom::ImageFrame>(
          image_frame_packet));
...
absl::StatusOr<CFHolder<CVPixelBufferRef>> CreateCVPixelBufferForImageFrame(
    std::shared_ptr<mediapipe_custom::ImageFrame> image_frame, bool can_overwrite) {
  ...
  mediapipe_custom::ImageFormat::Format image_format = frame.Format();
  OSType pixel_format = 0;
  CVReturn status;
  switch (image_format) {
    case mediapipe_custom::ImageFormat::SRGBA: {
    ...
    case mediapipe_custom::ImageFormat::GRAY8:
      pixel_format = kCVPixelFormatType_OneComponent8;
      break;

    case mediapipe_custom::ImageFormat::VEC32F1:
      pixel_format = kCVPixelFormatType_OneComponent32Float;
      break;

    case mediapipe_custom::ImageFormat::VEC32F2:
      pixel_format = kCVPixelFormatType_TwoComponent32Float;
      break;

    case mediapipe_custom::ImageFormat::VEC32F4:
...
absl::StatusOr<CFHolder<CVPixelBufferRef>> CreateCVPixelBufferCopyingImageFrame(
    const mediapipe_custom::ImageFrame& image_frame) {
  CFHolder<CVPixelBufferRef> pixel_buffer;
  OSType pixel_format = 0;
  ...
  mediapipe_custom::ImageFormat::Format image_format = image_frame.Format();
  switch (image_format) {
    case mediapipe_custom::ImageFormat::SRGBA:
      pixel_format = kCVPixelFormatType_32BGRA;
...
std::unique_ptr<mediapipe_custom::ImageFrame> CreateImageFrameForCVPixelBuffer(
...
std::unique_ptr<mediapipe_custom::ImageFrame> CreateImageFrameForCVPixelBuffer(
  ...
  size_t width = CVPixelBufferGetWidth(image_buffer);
  size_t height = CVPixelBufferGetHeight(image_buffer);
  std::unique_ptr<mediapipe_custom::ImageFrame> frame;

  CVPixelBufferRetain(image_buffer);

  OSType pixel_format = CVPixelBufferGetPixelFormatType(image_buffer);
  mediapipe_custom::ImageFormat::Format image_format = mediapipe_custom::ImageFormat::UNKNOWN;
  switch (pixel_format) {
    case kCVPixelFormatType_32BGRA: {
      image_format = mediapipe_custom::ImageFormat::SRGBA;
      if (!bgr_as_rgb) {
        // Swap R and B channels.
        ...
        if (can_overwrite) {
          ...
          frame = absl::make_unique<mediapipe_custom::ImageFrame>(image_format, width,
                                                           height);
    ...
    case kCVPixelFormatType_24RGB:
      image_format = mediapipe_custom::ImageFormat::SRGB;
      break;

    case kCVPixelFormatType_OneComponent8:
      image_format = mediapipe_custom::ImageFormat::GRAY8;
      break;
  ...
  if (frame) {
    ...
    frame = absl::make_unique<mediapipe_custom::ImageFrame>(
