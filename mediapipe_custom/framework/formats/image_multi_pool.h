...
#include "mediapipe_custom/framework/formats/image_frame_pool.h"

#if !MEDIAPIPE_DISABLE_GPU
#include "mediapipe_custom/gpu/gpu_buffer.h"

#ifdef __APPLE__
#include "mediapipe_custom/gpu/pixel_buffer_pool_util.h"
#endif  // __APPLE__

#if !MEDIAPIPE_GPU_BUFFER_USE_CV_PIXEL_BUFFER
#include "mediapipe_custom/gpu/gl_texture_buffer_pool.h"
...
class ImageMultiPool {
 public:
  ...
  // reusable buffers.
  void RegisterTextureCache(mediapipe_custom::CVTextureCacheType cache);

  // Remove a texture cache from the list of caches to be flushed.
  void UnregisterTextureCache(mediapipe_custom::CVTextureCacheType cache);
  ...
  struct IBufferSpec {
    IBufferSpec(int w, int h, mediapipe_custom::ImageFormat::Format f)
 ...
 private:
...
#else
  typedef std::shared_ptr<mediapipe_custom::GlTextureBufferPool> SimplePoolGpu;
#endif  // MEDIAPIPE_GPU_BUFFER_USE_CV_PIXEL_BUFFER
  ...
  std::vector<CFHolder<mediapipe_custom::CVTextureCacheType>> texture_caches_
